...
try{
  ...
  await page.waitForFunction(()=>window.RWARenkoV6?.version==='6.1.0'&&window.RWARenkoV3?.state?.symbols?.length>=500,{timeout:30000});
  ...
  await page.waitForFunction(()=>window.RWARenkoV6?.state?.bricks?.length>=50&&!window.RWARenkoV6?.state?.fetching,{timeout:60000});
  await page.waitForTimeout(400);
  const initial=await page.evaluate(()=>({version:window.RWARenkoV6.version,mode:window.RWARenkoV6.mode,source:window.RWARenkoV6.historicalSource,raw:window.RWARenkoV6.rawAudit,generation:window.RWARenkoV6.state.generation,pairs:window.RWARenkoV6.state.selectedUniverseCount,rows:document.querySelectorAll('.pair-row').length,history:document.querySelector('#fastHistoryCount')?.textContent,coverage:document.querySelector('#fastHistoryCoverage')?.textContent,status:document.querySelector('#fastArchiveStatus')?.textContent,display:document.querySelector('#lazyHistoryCanvas')?.style.display,rect:document.querySelector('#lazyHistoryCanvas')?.getBoundingClientRect().toJSON(),feed:document.querySelector('#feedPill b')?.textContent,sourceText:document.querySelector('#sourceText')?.textContent}));
  assert.equal(initial.version,'6.1.0');assert.equal(initial.mode,'tradingview-compatible-lazy-50');assert.equal(initial.source,'binance-1m-close');assert.equal(initial.raw,'binance-vision-individual-trades');assert.ok(initial.generation>=1);assert.ok(initial.pairs>=500);assert.equal(initial.rows,500);assert.match(String(initial.history),/50 BRICKS VISIBLE/i);assert.match(String(initial.coverage),/50-BRICK WINDOW/i);assert.match(String(initial.status),/50 BRICKS READY|50 CACHED BRICKS/i);assert.notEqual(initial.display,'none');assert.ok(initial.rect?.width>700&&initial.rect?.height>250);assert.match(String(initial.feed),/^LIVE/);assert.match(String(initial.sourceText),/1m CLOSE/);
