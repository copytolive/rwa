...
if(localExact){
  await page.route('https://data-api.binance.vision/**',async route=>{
    try{const req=route.request(),headers={...req.headers()};delete headers.origin;delete headers.referer;const r=await fetch(req.url(),{method:req.method(),headers,signal:AbortSignal.timeout(30000)}),body=Buffer.from(await r.arrayBuffer());await route.fulfill({status:r.status,headers:{'content-type':r.headers.get('content-type')||'application/json','cache-control':'no-store','access-control-allow-origin':'*'},body})}catch{await route.abort('failed').catch(()=>{})}
  });
}
...
try{
  ...
  // Discover the browser's actual physical min/max edge (range direction differs
  // across engines), reset to the physical min edge, then drag the real thumb to
  // the physical max/5Y edge. No JS value assignment is used for the 5Y move.
  const range=page.locator('#renko5yRange'),bb=await range.boundingBox();if(!bb)throw new Error('5Y slider has no physical bounding box');
  ...
  await page.mouse.click(minX,y);await page.waitForTimeout(20);
  const dragStarted=Date.now();await page.mouse.move(minX,y);await page.mouse.down();await page.mouse.move(maxX,y,{steps:24});await page.mouse.up();
  let sliderValue=Number(await range.inputValue()),dragReachedMax=sliderValue>=max-1,fallbackClick=false;
  if(!dragReachedMax){fallbackClick=true;await page.mouse.click(maxX,y);sliderValue=Number(await range.inputValue())}
  const physicalMoveMs=Date.now()-dragStarted;if(sliderValue<max-1)throw new Error(`physical 5Y move failed left=${leftVal} right=${rightVal} final=${sliderValue} max=${max}`);
  const readyStarted=Date.now();await page.waitForFunction(()=>document.documentElement.dataset.renko5yStatus==='ready',null,{timeout:60000});
  ...
  Object.assign(history,{leftVal,rightVal,dragReachedMax,fallbackClick,physicalMoveMs,readyWaitMs,wallMs});
  ...
  const report={schema:'renko-presets-five-values-5y-browser-v4',generatedAt:new Date().toISOString(),base,mode,viewport,status:'PASS',httpStatus:response?.status()||null,contract:{presetValues:P,atrMeaning:'ATR length',traditionalMeaning:'fixed absolute box size',zeroMsMeaning:'interaction ACK <=1ms and measured long-task blocking 0ms; chart settle/network are reported separately',historyMeaning:'physical pointer move to virtualized exact Binance fixed-1s window; never fake continuous 5Y in-memory bars',maxHorizon:'5 calendar years',localExactData:localExact?'real data-api.binance.vision bytes relayed only to bypass localhost Origin CORS':'no relay; browser accesses public production source directly'},ui,presetRows,history,stats,pageErrors,consoleErrors};
  fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));
  console.log('RENKO_PRESETS_5Y '+JSON.stringify({status:'PASS',mode,presets:presetRows.length,ackMaxMs:Math.max(...presetRows.map(x=>x.ackMs)),settleMaxMs:Math.max(...presetRows.map(x=>x.settleMs)),blockingMaxMs:Math.max(...presetRows.map(x=>x.blockingMs)),fiveYearPhysicalMoveSeconds:Number((history.physicalMoveMs/1000).toFixed(3)),dragReachedMax:history.dragReachedMax,fallbackClick:history.fallbackClick,fiveYearLoadSeconds:Number((history.loadMs/1000).toFixed(3)),fiveYearWallSeconds:Number((history.wallMs/1000).toFixed(3)),fiveYearBars:history.bars,fiveYearBricks:history.totalBricks,pageErrors}));
}catch(e){fatal=String(e?.stack||e);console.error('RENKO_PRESETS_5Y_FATAL',fatal);try{await page.screenshot({path:path.join(out,`${mode}-FAIL.png`),fullPage:true})}catch{}fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({schema:'renko-presets-five-values-5y-browser-v4',generatedAt:new Date().toISOString(),base,mode,viewport,status:'FAIL',fatal,presetRows,pageErrors,consoleErrors},null,2));await browser.close();process.exit(2)}
await browser.close();
