...
if(localExact){
  // The real Binance public API accepts the deployed Pages Origin, while its
  // CORS policy rejects localhost. Relay the same live bytes only for local
  // exact-checkout testing; public-production testing does not use this relay.
  await page.route('https://data-api.binance.vision/**',async route=>{
    try{
      ...
      const r=await fetch(req.url(),{method:req.method(),headers,signal:AbortSignal.timeout(30000)}),body=Buffer.from(await r.arrayBuffer());
...
try{
  ...
  // Physical browser interaction: drag the RTL history slider from LIVE all the
  // way to the 5Y edge, then time until the exact fixed-1s historical window is
  // fully rendered. This is intentionally not a programmatic value assignment.
  ...
  const max=await range.evaluate(e=>Number(e.max)),dragStarted=Date.now();
  await page.mouse.move(bb.x+bb.width*.96,bb.y+bb.height/2);await page.mouse.down();await page.mouse.move(bb.x+bb.width*.04,bb.y+bb.height/2,{steps:24});await page.mouse.up();
  const dragMs=Date.now()-dragStarted;
  await page.waitForFunction(m=>Number(document.getElementById('renko5yRange')?.value)>=m-1,max,{timeout:5000});
  ...
  Object.assign(history,{physicalDragMs:dragMs,readyWaitMs,wallMs});
