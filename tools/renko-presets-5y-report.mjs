...
try{
  ...
  // On narrow/mobile layouts chartWrap can sit below the initial viewport. Bring
  // the real native range control into the physical viewport before pointer
  // interaction; otherwise Playwright mouse coordinates are outside the screen.
  const range=page.locator('#renko5yRange');await range.scrollIntoViewIfNeeded();await page.waitForTimeout(80);
  ...
  // Some mobile Chromium builds deliberately do not map mouse clicks to a native
  // range thumb. The final fallback is still a real browser/user input path:
  // focus the native control and press End, never JS-set its 5Y value.
  ...
  const report={schema:'renko-presets-five-values-5y-browser-v5',generatedAt:new Date().toISOString(),base,mode,viewport,status:'PASS',httpStatus:response?.status()||null,contract:{presetValues:P,atrMeaning:'ATR length',traditionalMeaning:'fixed absolute box size',zeroMsMeaning:'interaction ACK <=1ms and measured long-task blocking 0ms; chart settle/network are reported separately',historyMeaning:'physical native-control input to virtualized exact Binance fixed-1s window; never fake continuous 5Y in-memory bars',maxHorizon:'5 calendar years',localExactData:localExact?'real data-api.binance.vision bytes relayed only to bypass localhost Origin CORS':'no relay; browser accesses public production source directly'},ui,presetRows,history,stats,pageErrors,consoleErrors};
  fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));
  console.log('RENKO_PRESETS_5Y '+JSON.stringify({status:'PASS',mode,presets:presetRows.length,ackMaxMs:Math.max(...presetRows.map(x=>x.ackMs)),settleMaxMs:Math.max(...presetRows.map(x=>x.settleMs)),blockingMaxMs:Math.max(...presetRows.map(x=>x.blockingMs)),fiveYearPhysicalMoveSeconds:Number((history.physicalMoveMs/1000).toFixed(3)),dragReachedMax:history.dragReachedMax,fallbackClick:history.fallbackClick,keyboardFallback:history.keyboardFallback,fiveYearLoadSeconds:Number((history.loadMs/1000).toFixed(3)),fiveYearWallSeconds:Number((history.wallMs/1000).toFixed(3)),fiveYearBars:history.bars,fiveYearBricks:history.totalBricks,pageErrors}));
}catch(e){fatal=String(e?.stack||e);console.error('RENKO_PRESETS_5Y_FATAL',fatal);try{await page.screenshot({path:path.join(out,`${mode}-FAIL.png`),fullPage:true})}catch{}fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({schema:'renko-presets-five-values-5y-browser-v5',generatedAt:new Date().toISOString(),base,mode,viewport,status:'FAIL',fatal,presetRows,pageErrors,consoleErrors},null,2));await browser.close();process.exit(2)}
await browser.close();
