...
// data-api.binance.vision allows the real public Pages origin but rejects the
// localhost Origin used by exact-checkout CI. Relay only those same real public
// API bytes through the Playwright host process; no fixture or synthesized data.
...
try{
  ...
  const report={schema:'renko-presets-five-values-5y-browser-v2',generatedAt:new Date().toISOString(),base,mode,viewport,status:'PASS',httpStatus:response?.status()||null,contract:{presetValues:P,atrMeaning:'ATR length',traditionalMeaning:'fixed absolute box size',zeroMsMeaning:'interaction ACK <=1ms and measured long-task blocking 0ms; chart settle/network are reported separately',historyMeaning:'virtualized exact Binance fixed-1s window navigation, never fake continuous 5Y in-memory bars',maxHorizon:'5 calendar years',exactCheckoutData:'real data-api.binance.vision bytes relayed only to bypass localhost Origin CORS'},ui,presetRows,history,stats,pageErrors,consoleErrors};
...
}catch(e){fatal=String(e?.stack||e);console.error('RENKO_PRESETS_5Y_FATAL',fatal);try{await page.screenshot({path:path.join(out,`${mode}-FAIL.png`),fullPage:true})}catch{}fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({schema:'renko-presets-five-values-5y-browser-v2',generatedAt:new Date().toISOString(),base,mode,viewport,status:'FAIL',fatal,presetRows,pageErrors,consoleErrors},null,2));await browser.close();process.exit(2)}
await browser.close();
