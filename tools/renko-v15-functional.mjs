...
async function openSymbol(base){
  ...
  await f.waitForFunction(()=>window.RWARenkoV15?.version==='15.1.0'&&window.RWARenkoV15MethodProfiles?.version==='1.0.0',{timeout:70000});
  ...
  const state=await f.evaluate(()=>({v3:RWARenkoV3.state.selected,v15:RWARenkoV15.state.symbol,ticks:RWARenkoV15.state.ticks.length,bricks:RWARenkoV15.state.data.length,formation:RWARenkoV15.formationSource,confirmation:RWARenkoV15.confirmationRule,timeframe:RWARenkoV15.timeframeControl,history:RWARenkoV15.historySource,atrSizing:RWARenkoV15.atrSizing,profile:document.querySelector('[data-v15-profile].active .v15-state')?.textContent||''}));
...
try{
  ...
  const trad=f.locator('#v15TraditionalBox');await trad.fill('37.5');await f.waitForTimeout(700);if(await trad.inputValue()!=='37.5')fail('Traditional draft rewrote itself');
  ...
  const pct=f.locator('#v15Percentage');await pct.fill('0,25');await f.waitForTimeout(600);if(await pct.inputValue()!=='0,25')fail('Percentage draft rewrote itself');await f.locator('[data-v15-apply="percentage"]').click();
  ...
  const atr=f.locator('#v15AtrLength');await atr.fill('21');await f.waitForTimeout(600);if(await atr.inputValue()!=='21')fail('Tick ATR draft rewrote itself');await f.locator('[data-v15-apply="atr"]').click();
  await f.waitForFunction(()=>window.RWARenkoV15.settings.method==='atr'&&Number(window.RWARenkoV15.settings.atrLength)===21&&!window.RWARenkoV15.state.building,{timeout:60000});const aa=await uniform(f,'TICK ATR');if(!(aa.box>0))fail('Tick ATR active box missing');
  const atrState=await f.evaluate(()=>({atr:RWARenkoV15.state.atrValue,sizing:RWARenkoV15.atrSizing,box:RWARenkoV15.state.box}));if(atrState.sizing!=='wilder-absolute-trade-move-no-timeframe'||!(atrState.atr>0))fail(`Tick ATR contract failed ${JSON.stringify(atrState)}`);
  ...
  console.log('RENKO V15.1 FUNCTIONAL PASS',JSON.stringify({before,after,uiHeight:ui.height,atrState}));await page.close();
