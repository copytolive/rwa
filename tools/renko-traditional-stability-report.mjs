...
async function snap(){return page.evaluate(()=>{const T=RWARenkoTV,s=T.state,b=s.base,bars=s.closedBars||[],firstBar=bars[0],lastBar=bars.at(-1);return{symbol:s.symbol,status:s.status,method:T.settings.method,box:Number(s.box),input:Number(document.getElementById('traditionalBox')?.value),visibleBox:(document.getElementById('brickValue')?.textContent||'').trim(),anchor:Number(b?.anchor),total:Number(b?.totalBricks??s.confirmedTotal??0),rendered:Number(s.confirmed?.length||0),source:Number(bars.length||0),firstOpenTime:Number(firstBar?.openTime||firstBar?.time||0),latestCloseTime:Number(lastBar?.closeTime||lastBar?.time||0),lastSource:Number(b?.state?.lastSourceTime||0),exactOwn:Object.prototype.hasOwnProperty.call(T.settings,'_exactBox'),workerActive:!!window.RWARenkoATRFixed1s?.workerActive,fixed:document.documentElement.dataset.renkoTraditionalFixed||'',control:document.documentElement.dataset.renkoTraditionalStatus||''}})}
