...
if len(routes)!=430:
 print(f'EXPECTED_430_ROUTES actual={len(routes)}',file=sys.stderr); sys.exit(1)
...
runtime_errors=[]; console_errors=[]; failed_responses=[]; broken_images=[]; stale_paths=[]; overflows=[]; broken_links=[]; api_requests=[]
control_occurrences=0; distinct={}; screens=[]; current={'vp':'','route':''}
...
def relocate(page,item):
 buttons=page.locator('button:visible'); count=buttons.count()
 if item['index']<count:
  c=buttons.nth(item['index'])
  try:
   if button_label(c)==item['label'] and (c.get_attribute('class') or '')==item['class']: return c
  except Exception: pass
 try:
  idx=buttons.evaluate_all("""(els,t)=>els.findIndex(e=>{const raw=(e.getAttribute('aria-label')||e.innerText||'').trim().replace(/\s+/g,' ').slice(0,120);return raw===t.label&&(e.getAttribute('class')||'')===t.cls})""",{'label':item['label'],'cls':item['class']})
  if isinstance(idx,int) and idx>=0: return buttons.nth(idx)
 except Exception: pass
 return None

def pointer_click(page,item):
 b=relocate(page,item)
 if b is None: return False,{'reason':'not-found'}
 try:
  b.evaluate("el=>el.scrollIntoView({block:'center',inline:'center',behavior:'instant'})"); page.wait_for_timeout(45)
  ...
  if not hit['inViewport'] or not hit['ok']: return False,{'reason':'pointer-intercepted','hit':hit}
  page.mouse.click(hit['x'],hit['y']); return True,hit
 except Exception as e: return False,{'reason':'pointer-exception','detail':str(e)[:800]}
...
with sync_playwright() as pw:
 ...
 # Full route/static/geometry sweep at launch desktop and mobile.
 for vpname,w,h in sweep_viewports:
  page.set_viewport_size({'width':w,'height':h})
  for route in routes:
   current.update(vp=vpname,route=route)
   try:
    resp=page.goto(url_for(route),wait_until='domcontentloaded',timeout=12000); page.wait_for_timeout(100)
   ...
   try:
    dims=page.evaluate("() => ({sw:document.documentElement.scrollWidth,iw:innerWidth,bw:document.body.getBoundingClientRect().width})")
    if dims['sw']>dims['iw']+3:
     offender=page.evaluate("""() => {let best=null;for(const el of document.querySelectorAll('body *')){const r=el.getBoundingClientRect();const excess=Math.max(0,r.right-innerWidth)+Math.max(0,-r.left);if(excess>3&&(!best||excess>best.excess))best={tag:el.tagName,cls:String(el.className||'').slice(0,160),id:el.id||'',left:r.left,right:r.right,width:r.width,excess};}return best;}""")
     overflows.append({'vp':vpname,'route':route,**dims,'offender':offender})
   except Exception as e: runtime_errors.append({'vp':vpname,'route':route,'error':'geometry: '+str(e)})
   ...
   for i in range(buttons.count()):
    try:
     b=buttons.nth(i); label=button_label(b); cls=b.get_attribute('class') or ''; key=(vpname,label,cls)
 ...
 # Screenshot parity matrix, all nine requested viewports.
 for vpname,w,h in viewports:
  page.set_viewport_size({'width':w,'height':h})
  for route in core:
   current.update(vp=vpname,route=route)
   try:
    page.goto(url_for(route),wait_until='domcontentloaded',timeout=12000); page.wait_for_timeout(140)
 ...
 # Execute every globally-distinct visible control representative. Pointer reachability is
 # verified before dispatch; no force click and no DOM .click shortcut are used.
 ...
 for item in distinct.values():
  ...
  try:
   ...
   b=relocate(page,item)
   if b is None:
    if unsafe_rx.search(item['label']): locked_total+=1; locked_fail.append({**item,'reason':'control missing after hydration'})
    else: active_total+=1; unexpected.append({**item,'reason':'control missing after hydration'})
    continue
   ...
   if item.get('selected'):
    if is_locked: locked_total+=1; locked_passed+=1
    else: active_total+=1; active_passed+=1
    continue
   ok,diag=pointer_click(page,item)
   if not ok:
    if is_locked: locked_total+=1; locked_fail.append({**item,**diag})
    else: active_total+=1; unexpected.append({**item,**diag})
    continue
   page.wait_for_timeout(70); after_url=page.url; after=clean_html(page); changed=after_url!=before_url or hashlib.sha256(after.encode()).hexdigest()!=hashlib.sha256(before.encode()).hexdigest()
   if is_locked:
    locked_total+=1; body=page.locator('body').inner_text(); unsafe_success=bool(re.search(r'order filled|purchase confirmed|transaction successful|settlement complete',body,re.I)) and not re.search(r'backend.*not connected|UI DEMO',body,re.I); safe_notice=bool(re.search(r'backend/wallet execution is not connected|UI DEMO',body,re.I))
   ...
   if unsafe_rx.search(item['label']): locked_total+=1; locked_fail.append({**item,'reason':str(e)[:1000]})
   else: active_total+=1; unexpected.append({**item,'reason':str(e)[:1000]})
 browser.close()

runtime_errors=uniq(runtime_errors,lambda x:(x.get('vp'),x.get('route'),x.get('error')))
console_errors=uniq(console_errors,lambda x:(x.get('vp'),x.get('route'),x.get('text')))
failed_responses=uniq(failed_responses,lambda x:(x.get('vp'),x.get('route'),x.get('status'),x.get('url')))
broken_images=uniq(broken_images,lambda x:json.dumps(x,sort_keys=True)); stale_paths=uniq(stale_paths,lambda x:json.dumps(x,sort_keys=True))
overflows=uniq(overflows,lambda x:(x.get('vp'),x.get('route'))); broken_links=uniq(broken_links,lambda x:json.dumps(x,sort_keys=True))
base_host=urlparse(BASE).netloc; external_api=[r for r in api_requests if urlparse(r['url']).netloc and urlparse(r['url']).netloc!=base_host]
...
result={
 'routesTotal':len(routes),'viewports':[x[0] for x in viewports],'coreScreenshots':screens,
 'controlInstances':control_occurrences,'distinctControls':len(distinct),'activeTotal':active_total,'activePassed':active_passed,'lockedTotal':locked_total,'lockedPassed':locked_passed,
 'unexpectedNoop':len(unexpected),'runtimeErrors':len(runtime_errors),'consoleErrors':len(console_errors),'failedResponses':len(failed_responses),'brokenImages':len(broken_images),'stalePublicRoots':len(stale_paths),'horizontalOverflow':len(overflows),'brokenLinks':len(broken_links),'backend':backend,
 'unexpected':unexpected,'lockedFailures':locked_fail,'runtimeErrorDetails':runtime_errors,'consoleErrorDetails':console_errors,'failedResponseDetails':failed_responses,'brokenImageDetails':broken_images,'stalePathDetails':stale_paths,'overflowDetails':overflows,'brokenLinkDetails':broken_links,
}
(ART/'audit.json').write_text(json.dumps(result,indent=2))
print('REALWORLDASSET_TOTAL_UI_AUDIT '+json.dumps({k:result[k] for k in ['routesTotal','controlInstances','distinctControls','activeTotal','activePassed','lockedTotal','lockedPassed','unexpectedNoop','runtimeErrors','consoleErrors','failedResponses','brokenImages','stalePublicRoots','horizontalOverflow','brokenLinks']}))
print('BACKEND_CLASSIFICATION '+classification)
fail=(classification!='NO_RUNTIME_BACKEND_CONNECTION' or runtime_errors or console_errors or failed_responses or broken_images or stale_paths or overflows or broken_links or unexpected or locked_fail or active_passed!=active_total or locked_passed!=locked_total)
if fail:
 for name,items in [('UNEXPECTED',unexpected),('LOCKED_FAILURE',locked_fail),('RUNTIME',runtime_errors),('CONSOLE',console_errors),('FAILED_RESPONSE',failed_responses),('BROKEN_IMAGE',broken_images),('STALE_PATH',stale_paths),('OVERFLOW',overflows),('BROKEN_LINK',broken_links)]:
  if items: print(name+' '+json.dumps(items[:40],ensure_ascii=False),file=sys.stderr)
 sys.exit(1)
print('REALWORLDASSET_TOTAL_UI_PASS')
