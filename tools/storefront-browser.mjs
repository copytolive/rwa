...
await browser.close();if(fail.length){console.error(JSON.stringify({ok:false,contract:'rwa-storefront-browser-v6',fail},null,2));process.exit(1)}console.log(JSON.stringify({ok:true,contract:'rwa-storefront-browser-v6',surfaces:['commerce landing','RWA-linked vs store-token labels','desktop storefront','mobile storefront','physical-store detail','honest preview states','search focus','physical-store draft create + detail bridge','trade mobile commerce link','command palette']},null,2));
