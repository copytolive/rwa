...
async function test(viewport,label){
  ...
  if(label==='desktop'){
    ...
    if(await page.locator('#rwaPhysicalStoreDraft').isVisible().catch(()=>false)){
      ...
      await page.locator('#openRwaStorefront').click();await page.locator('#rwaShopScreen.open').waitFor({state:'visible',timeout:8000}).catch(()=>{});check(await page.locator('[data-local-store-token="STORE-UI"]').isVisible().catch(()=>false),'desktop: local physical-store draft is not visible in storefront');check(await page.locator('[data-local-store-token="STORE-UI"] button:disabled').count().catch(()=>0)>=1,'desktop: local physical-store draft trade must remain disabled');await page.locator('#rwaShopClose').click();
    }
  }
...
await browser.close();if(fail.length){console.error(JSON.stringify({ok:false,contract:'rwa-storefront-browser-v5',fail},null,2));process.exit(1)}console.log(JSON.stringify({ok:true,contract:'rwa-storefront-browser-v5',surfaces:['commerce landing','RWA-linked vs store-token labels','desktop storefront','mobile storefront','honest preview states','search focus','physical-store draft create + storefront bridge','trade mobile commerce link','command palette']},null,2));
