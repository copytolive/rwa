...
const [tradeHtml,tradeCfg,tradePro,margin,overlay,release,rootHtml,quick,suiteUi,suiteCore,social,exec,worker,readiness]=await Promise.all([
  read('trade/index.html'),read('trade/config.js'),read('trade/terminal-pro.js'),read('trade/margin-runtime.js'),read('trade/chart-overlays.js'),read('trade/release-runtime.js'),read('index.html'),read('quick-actions.js'),read('suite-ui.js'),read('suite-v2.js'),read('social-trade-monitor.js'),read('execution-api.js'),read('agent-worker/worker.mjs'),read('launch/readiness.json')
]);
...
if(fail.length){console.error(JSON.stringify({ok:false,contract:'rwa-social-trading-parity-v1',fail},null,2));process.exit(1)}
console.log(JSON.stringify({ok:true,contract:'rwa-social-trading-parity-v1',trade:'one-click+atomic-tpsl+margin+overlays',social:'leaderboard+copy+feed+follow-alerts',security:'single-write+machine-gated-mainnet'},null,2));
