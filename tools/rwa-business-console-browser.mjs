...
try{
  for(const [name,width,height] of viewports){
    const context=await browser.newContext({viewport:{width,height}}),pages=[];context.on('page',p=>pages.push(p));const page=await context.newPage();
    ...
    await page.waitForSelector('#rwaSeablueprintCommerceLaunch',{state:'visible',timeout:15000});const pathBefore=new URL(page.url()).pathname;await page.click('#rwaSeablueprintCommerceLaunch');await page.waitForSelector('#rwaShopScreen.open',{timeout:15000});await page.waitForSelector('#rwaBusinessConsoleLaunch',{state:'visible',timeout:10000});await page.click('#rwaBusinessConsoleLaunch');await page.waitForSelector('#rwaBusinessConsole.open',{timeout:10000});await page.waitForFunction(()=>document.querySelector('#rwaBusinessConsoleBody')?.textContent?.includes('BUSINESS VALIDATION BACKEND LOCKED'));
    const state=await page.evaluate(()=>({path:location.pathname,hash:location.hash,overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,shopOpen:document.querySelector('#rwaShopScreen')?.classList.contains('open')===true,businessOpen:document.querySelector('#rwaBusinessConsole')?.classList.contains('open')===true,audit:window.RWABusinessConsole?.audit?.(),bridge:window.RWASeablueprintCommerceBridge?.audit?.()}));
    if(state.path!==pathBefore)failures.push(`${name}:top-level-path-changed:${state.path}`);if(state.overflow>1)failures.push(`${name}:horizontal-overflow:${state.overflow}`);if(!state.shopOpen||!state.businessOpen)failures.push(`${name}:console-not-open`);if(state.audit?.ok!==true)failures.push(`${name}:audit-failed`);if(state.audit?.apiConfigured!==false)failures.push(`${name}:api-must-remain-fail-closed-in-repo`);if(pages.length)failures.push(`${name}:popup-opened:${pages.length}`);
