...
try{
  const ctx=await browser.newContext({viewport:{width:1600,height:1000},serviceWorkers:'block'});await mocks(ctx);const p=await ctx.newPage();
  const errors=[];p.on('pageerror',e=>errors.push(String(e.message||e)));
  await p.goto(BASE,{waitUntil:'domcontentloaded'});await ready(p);
  await p.evaluate(()=>window.__chromeChartNode=document.querySelector('.chart-wrap'));
  ...
  for(const route of ['intelligence','assets','research','portfolio','institutional']){
    await p.locator(`[data-v5-route="${route}"]`).first().click();await p.waitForTimeout(route==='portfolio'?850:300);report.push(await chromeState(p,`topnav-${route}`));
  }

  for(const level of ['discovery','analysis','action']){
    await p.locator(`#rwaExperienceRail [data-rwa-level="${level}"]`).click();await p.waitForTimeout(level==='action'?1200:450);report.push(await chromeState(p,`rail-${level}`));
  }
  ...
  console.log('GLOBAL_CHROME_REPORT',JSON.stringify(report));
  await ctx.close();

  const mobile=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,serviceWorkers:'block'});await mocks(mobile);const m=await mobile.newPage();
  await m.goto(BASE,{waitUntil:'domcontentloaded'});await ready(m);await m.evaluate(()=>window.__chromeChartNode=document.querySelector('.chart-wrap'));
  ...
  console.log('MOBILE_GLOBAL_HEADER_PERSISTENCE=PASS');
  await mobile.close();
