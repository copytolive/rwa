...
async function ready(page){
  ...
  await page.waitForFunction(()=>window.RWAMarketPerformanceGuard?.version==='1.3.3'&&[...document.querySelectorAll('link[rel="stylesheet"]')].some(x=>(x.getAttribute('href')||'').includes('persistent-market-operability-patch-v1.css?v=10')),{timeout:10000});
...
async function chromeState(page,label){
  ...
  assert.equal(s.cssV10,true,`${label}: V10 operability CSS not loaded`);
