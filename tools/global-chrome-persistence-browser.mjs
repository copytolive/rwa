...
async function ready(page){await page.waitForFunction(()=>window.RWASuperApp?.version==='5.0.0'&&document.getElementById('rwaExperienceRail'),{timeout:20000});await page.waitForTimeout(450)}
...
async function chromeState(page,label){
  const s=await page.evaluate((label)=>{
    ...
    const describe=e=>e?{tag:e.tagName,id:e.id||'',cls:String(e.className||''),text:String(e.textContent||'').trim().slice(0,80),z:getComputedStyle(e).zIndex,pos:getComputedStyle(e).position}:null;
    return{label,vw:innerWidth,path:location.pathname,hash:location.hash,scrollY,topVisible:visible(top),railVisible:visible(rail),chartVisible:visible(chart),sameChart:chart===window.__chromeChartNode,top:tr,rail:rr,workspace:wr,workspacePosition:workspace?getComputedStyle(workspace).position:null,probePoint:{x:px,y:py},probeInsideRail:!!probe?.closest?.('#rwaExperienceRail'),probe:describe(probe),probeAncestors:probe?[...function*(){let e=probe;for(let i=0;e&&i<6;i++,e=e.parentElement)yield describe(e)}()]:[]};
  },label);
  assert.equal(s.path,'/rwa/',`${label}: escaped /rwa/`);assert.equal(s.topVisible,true,`${label}: topbar disappeared`);assert.equal(s.railVisible,true,`${label}: workflow rail disappeared`);assert.equal(s.chartVisible,true,`${label}: chart disappeared`);assert.equal(s.sameChart,true,`${label}: chart DOM was replaced`);
  assert.ok(s.top&&s.top.y<=1.5&&s.top.h>=40,`${label}: topbar is not pinned ${JSON.stringify(s.top)}`);
  assert.ok(s.rail&&s.rail.y>=s.top.b-2&&s.rail.y<=s.top.b+8,`${label}: rail is not pinned under header ${JSON.stringify({top:s.top,rail:s.rail,scrollY:s.scrollY})}`);
  assert.equal(s.probeInsideRail,true,`${label}: another surface covers the workflow rail ${JSON.stringify({point:s.probePoint,probe:s.probe,ancestors:s.probeAncestors,rail:s.rail,workspace:s.workspace,scrollY:s.scrollY})}`);
  if(s.workspace&&s.workspacePosition==='fixed'&&s.vw>=1200)assert.ok(s.workspace.y>=s.rail.b-2,`${label}: workspace overlaps global rail ${JSON.stringify({rail:s.rail,workspace:s.workspace})}`);return s;
}

const browser=await chromium.launch({headless:true});
try{
  const ctx=await browser.newContext({viewport:{width:1600,height:1000},serviceWorkers:'block'});await mocks(ctx);const p=await ctx.newPage();const errors=[];p.on('pageerror',e=>errors.push(String(e.message||e)));await p.goto(BASE,{waitUntil:'domcontentloaded'});await ready(p);await p.evaluate(()=>window.__chromeChartNode=document.querySelector('.chart-wrap'));
  ...
  assert.equal(errors.length,0,`runtime errors: ${errors.join(' | ')}`);console.log('GLOBAL_TOPBAR_PERSISTENCE=PASS');console.log('EXPERIENCE_RAIL_PERSISTENCE=PASS');console.log('WORKSPACE_NEVER_COVERS_GLOBAL_CHROME=PASS');console.log('GLOBAL_CHROME_ALL_CLICK_TARGETS=PASS');console.log('GLOBAL_CHROME_REPORT',JSON.stringify(report));await ctx.close();
  ...
  for(const route of ['intelligence','assets','research','portfolio','institutional']){await m.evaluate(r=>window.RWASuperApp.navigate(r),route);await m.waitForTimeout(route==='portfolio'?800:250);const s=await m.evaluate(()=>{const t=document.querySelector('.topbar')?.getBoundingClientRect(),c=document.querySelector('.chart-wrap');return{top:t&&{y:t.y,h:t.height},display:t&&getComputedStyle(document.querySelector('.topbar')).display,same:c===window.__chromeChartNode}});assert.ok(s.top&&s.top.y<=1.5&&s.top.h>=44,`${route}: mobile header disappeared`);assert.notEqual(s.display,'none',`${route}: mobile header display none`);assert.equal(s.same,true,`${route}: mobile chart replaced`)}
