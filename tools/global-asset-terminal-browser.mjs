...
async function run(viewport){const browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport});
 ...
 const cryptoBadge=await page.locator('.verified-badge').first().textContent();if(!/\bLIVE\b/.test(cryptoBadge||''))throw new Error(`crypto LIVE badge not restored: ${cryptoBadge}`);
 const order=page.locator('.order-section');if(viewport.width<681){if(await order.getAttribute('hidden')!==null)throw new Error('crypto order book remained hidden after stock mode');await page.locator('[data-mobile-view="depth"]').click();await page.waitForFunction(()=>document.body.classList.contains('mview-depth'));if(!(await page.locator('.right').isVisible()))throw new Error('mobile depth panel not reachable after stock mode');}else if(!(await order.isVisible()))throw new Error('crypto order book not restored');
