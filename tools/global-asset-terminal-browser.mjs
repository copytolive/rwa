...
async function run(viewport){const browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport});
 ...
 const stock=page.locator('[data-filter="stocks"]');await stock.waitFor({state:'visible'});await stock.click();await page.waitForSelector('[data-stock-id="US-XNAS-AAPL"]',{timeout:10000});
 ...
 const fbtn=page.locator('.rwa-fundamentals-trigger').first();if(await fbtn.count()){await fbtn.click();await page.waitForSelector('#globalStockFundamentals.open');const txt=await page.locator('#globalStockFundBody').textContent();if(!/No regulator-backed fundamentals|Revenue/.test(txt||''))throw new Error('stock fundamentals overlay failed')}
 await page.locator('[data-filter="crypto"]').click();await page.waitForFunction(()=>window.RWAGlobalAssetTerminal?.state?.mode==='crypto');if(await page.locator('#stockTfBadge').isVisible())throw new Error('10m badge leaked into crypto mode');await browser.close()}
