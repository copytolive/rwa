...
async function run(viewport){const browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport});
 ...
 if(viewport.width<681){await page.evaluate(()=>window.RWAGlobalAssetTerminal.activateCrypto('all'));}else{const crypto=page.locator('[data-filter="crypto"]');await crypto.waitFor({state:'visible'});await crypto.click();}
