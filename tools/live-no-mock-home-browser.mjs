...
async function desktop(){
 ...
 if(JSON.stringify(info.globalNav)!==JSON.stringify(['trade','discover','portfolio','analytics','rewards','more']))fail('V5 agreed global nav mismatch',info.globalNav);
 ...
 const navRects=await page.locator('.topnav>button[data-v5-nav]').evaluateAll(xs=>xs.map(x=>{const r=x.getBoundingClientRect(),s=getComputedStyle(x);return{key:x.dataset.v5Nav,x:r.x,y:r.y,w:r.width,h:r.height,display:s.display,visibility:s.visibility,opacity:s.opacity}}));
 if(navRects.length!==6||navRects.some(r=>r.w<20||r.h<30||r.y<0||r.y>8||r.display==='none'||r.visibility==='hidden'||Number(r.opacity)===0))fail('Agreed desktop nav is not visible',navRects);
 ...
 await page.locator('#rwaV5Bottom [data-v5-bottom="holders"]').click();await page.waitForFunction(()=>/Holders data unavailable|NEEDS HOLDER BACKEND|Holder source not configured|NEEDS AUTHORITATIVE SOURCE|Holders source|SOURCE GATED|VERIFIED/i.test(document.querySelector('[data-v5-bottom-body]')?.innerText||''),{timeout:8000}).catch(()=>{});if(!/Holders data unavailable|NEEDS HOLDER BACKEND|Holder source not configured|NEEDS AUTHORITATIVE SOURCE|Holders source|SOURCE GATED|VERIFIED/i.test(await page.locator('[data-v5-bottom-body]').innerText()))fail('Holders authoritative/source-gated state missing');
 ...
 await shot(page,'desktop-v5-post-interaction');
