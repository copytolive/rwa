...
const write=(status,fatal='')=>fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({schema:'renko-50pair-full-total-history-10x-v2',generatedAt:new Date().toISOString(),base,mode,viewport,status,httpStatus:response?.status?.()||null,contract:'Every one of the 50 launch pairs applies 10 distinct ATR lengths and 10 distinct Traditional minimum-tick multiples through the production runtime controllers. For every case the visible CONFIRMED value must equal the full engine total, while the bounded rendered tail remains nonblank and internally consistent. Fixed 1s remains permanent and the XAUT deep worker stays dormant on these 50 Binance launch pairs.',values:{atr:ATR,traditionalTickMultipliers:MULT},counts:counts(),current,pairs,atrRows,traditionalRows,pageErrors,consoleErrors,fatal},null,2));
try{
  ...
  for(let i=0;i<symbols.length;i++){
    ...
    for(const length of ATR){
      ...
      r.pass=r.actual===symbol&&r.method==='atr'&&r.sourceBars>=800&&r.historySatisfied&&r.historySourceCount>=length&&r.rawBox&&r.exactRaw&&r.totalPass&&r.empty==='none'&&r.canvas>0&&!r.worker&&r.entries===0&&r.interval==='1s'&&!r.selector;
    ...
    for(let j=0;j<boxes.length;j++){
      ...
      r.totalPass=r.total>0&&r.visible===r.total&&r.guardTotal===r.total&&r.rendered>0&&r.data===r.rendered&&r.total>=r.rendered&&(!r.limit||r.rendered<=r.limit);
      r.pass=r.actual===symbol&&r.method==='traditional'&&r.sourceBars>=800&&r.box>0&&r.tick>0&&r.requestPass&&r.inputPass&&r.tickPass&&r.totalPass&&r.empty==='none'&&r.canvas>0&&!r.worker&&r.entries===0&&r.interval==='1s'&&!r.selector&&!r.exactOwn;
    ...
    const pass=a.every(r=>r.pass)&&t.every(r=>r.pass);pairs.push({index:i+1,symbol,atrPass:a.filter(r=>r.pass).length,traditionalPass:t.filter(r=>r.pass).length,pass});
  ...
  write(status);
  console.log('RENKO_HISTORY_TOTAL_10X '+JSON.stringify({status,mode,counts:c,failedPairs:pairs.filter(r=>!r.pass).map(r=>r.symbol),pageErrors}));
  await browser.close();
