...
async function open(viewport){const p=await browser.newPage({viewport});const pageErrors=[];p.on('pageerror',e=>pageErrors.push(String(e)));await p.goto(base,{waitUntil:'domcontentloaded',timeout:30000});await p.waitForFunction(()=>window.RWAUILayoutIntegrity?.version==='16.0.0'&&window.RWASuperApp?.version,{timeout:20000});await p.waitForSelector('#rwaMarketplaceLaunch',{state:'visible'});return{p,pageErrors}}
...
async function assertDock(p,sel,label,expected=440){const d=await rect(p,sel);if(d.display==='none'||d.visibility==='hidden')throw Error(`${label}: dock hidden ${JSON.stringify(d)}`);if(!near(d.width,expected,2))throw Error(`${label}: width ${d.width} != ${expected}`);const vr=await p.evaluate(()=>{const b=document.body?.clientWidth||0,e=document.documentElement?.clientWidth||0;return b>0&&b<=e?b:e||innerWidth});if(!near(d.right,vr,2))throw Error(`${label}: right edge ${d.right} != ${vr}`);return d}
...
async function transitionParity(p,label){
  ...
  await p.locator('[data-asset-tab="fundamentals"]').click();
  await p.waitForSelector('[data-v5-action="legacy-fundamentals"]',{state:'visible'});
  await p.locator('[data-v5-action="legacy-fundamentals"]').click();
  await p.waitForFunction(()=>document.body.classList.contains('rwa-fundamentals-open')&&document.querySelector('#rwaFundamentals')?.classList.contains('open'));
  await p.waitForTimeout(120);
  const fund=await assertDock(p,'#rwaFundamentals',`${label} fundamentals`);const layoutFund=await rect(p,'.layout');
  if(!near(asset.width,fund.width,1)||!near(asset.left,fund.left,1)||!near(asset.right,fund.right,1))throw Error(`${label}: context geometry changed asset=${JSON.stringify(asset)} fund=${JSON.stringify(fund)}`);
  ...
  const restored=await assertDock(p,'#rwaSuperWorkspace',`${label} restored asset`);if(!near(restored.width,asset.width,1)||!near(restored.left,asset.left,1))throw Error(`${label}: restored geometry changed ${JSON.stringify({asset,restored})}`);
