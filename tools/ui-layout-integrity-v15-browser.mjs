...
async function loadFundamentals(p){await p.addStyleTag({url:new URL('rwa-fundamentals.css',base).href});await p.addScriptTag({url:new URL('rwa-fundamentals.js',base).href});await p.waitForFunction(()=>window.RWAFundamentals?.version,{timeout:10000});await p.waitForTimeout(150)}
async function assertFundDrawer(p,label){const d=await p.locator('#rwaFundamentals').evaluate(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return{left:r.left,right:r.right,width:r.width,visibility:s.visibility,z:s.zIndex,viewport:innerWidth,rightVisible:document.querySelector('.right')?getComputedStyle(document.querySelector('.right')).display:null}});if(d.visibility==='hidden'||d.width<380||d.left<0||d.right>d.viewport+2)throw Error(`${label}: fundamentals drawer clipped ${JSON.stringify(d)}`);if(d.rightVisible!=='none')throw Error(`${label}: legacy right rail still competes with fundamentals ${JSON.stringify(d)}`);return d}
async function assertClosedFundHidden(p,label){const d=await p.locator('#rwaFundamentals').evaluate(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return{visibility:s.visibility,left:r.left,right:r.right,width:r.width,viewport:innerWidth}});if(d.visibility!=='hidden')throw Error(`${label}: closed fundamentals still visually present ${JSON.stringify(d)}`)}
...
await desktop.p.waitForSelector('#rwaSuperWorkspace:not([hidden])',{state:'visible'});
await audit(desktop.p,'desktop marketplace');
await desktop.p.screenshot({path:`${proof}/02-desktop-marketplace.png`,fullPage:true});
await desktop.p.evaluate(()=>window.RWASuperApp.navigate('markets'));
...
await desktop.p.waitForFunction(()=>document.body.classList.contains('rwa-super-asset-workspace')&&!document.querySelector('#rwaSuperWorkspace')?.hidden);
...
await desktop.p.waitForFunction(()=>document.body.classList.contains('rwa-fundamentals-open')&&document.querySelector('#rwaFundamentals')?.classList.contains('open'));
...
await medium.p.waitForFunction(()=>document.body.classList.contains('rwa-super-asset-workspace'));
...
await mb.click();await mobile.p.waitForFunction(()=>String(window.RWASuperApp?.route?.()).startsWith('assets'));await mobile.p.waitForSelector('#rwaSuperWorkspace:not([hidden])',{state:'visible'});await audit(mobile.p,'mobile marketplace');
