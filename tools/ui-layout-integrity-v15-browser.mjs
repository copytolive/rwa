...
let a=await audit(desktop.p,'fundamentals exclusive');if(!a.fundamentalsOpen||a.workspaceOpen)throw Error(`fundamentals exclusivity failed ${JSON.stringify(a)}`);
await desktop.p.screenshot({path:`${proof}/04-fundamentals-exclusive.png`,fullPage:true});
await desktop.p.evaluate(()=>window.RWASuperApp.navigate('asset/PENDLE'));
await desktop.p.waitForFunction(()=>document.body.classList.contains('rwa-super-asset-workspace')&&!document.querySelector('#rwaSuperWorkspace')?.hidden);
if(await desktop.p.locator('#rwaFundamentals.open').count())throw Error('fundamentals remained open after asset navigation');
...
const result={ok:true,version:'15.0.0',marketplaceProminent:true,desktop1536:true,medium1280:true,mobile390:true,assetFundamentalsMutuallyExclusive:true,noContextOverlap:true,noRootHorizontalOverflow:true};
