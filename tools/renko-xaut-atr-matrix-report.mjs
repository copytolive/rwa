...
async function run(label,viewport){
  ...
  for(const n of lengths){
    ...
    await page.waitForFunction(n=>window.RWARenkoTV?.state?.atrAppliedLength===n&&window.RWARenkoTV?.state?.atrHistorySatisfied===true&&document.documentElement.dataset.atrEffectiveBoxPass==='true'&&Number(document.documentElement.dataset.atrLength)===n&&window.RWARenkoTV?.settings?.interval==='1s',n,{timeout:90000});
    await sleep(250);
    const row=await page.evaluate(n=>{
      const T=window.RWARenkoTV,s=T.state,br=s.confirmed||[],first=br[0],last=br.at(-1),raw=Number(s.atrRaw),box=Number(s.box),tickSize=Number(s.tickSize),tol=Math.max(1e-12,Math.abs(raw)*1e-10),tickTol=Math.max(1e-12,Math.abs(tickSize)*1e-10),mark=Number(window.__applyMark)||0;
      const tbt=(window.__renkoLongTasks||[]).filter(x=>x.start>=mark).reduce((a,x)=>a+Math.max(0,Number(x.duration)-50),0);
      const zeroFallback=!!s.atrZeroFallback||document.documentElement.dataset.atrZeroFallback==='true';
      const rawEqualsBox=Number.isFinite(raw)&&raw>0&&Math.abs(raw-box)<=tol;
      const zeroFallbackPass=raw===0&&zeroFallback&&Number.isFinite(tickSize)&&tickSize>0&&Math.abs(box-tickSize)<=tickTol;
      return {length:n,method:T.settings.method,interval:T.settings.interval,source:T.settings.source,rawAtr:raw,atr:Number(s.atr),box,tickSize,rawEqualsBox,zeroFallback,zeroFallbackPass,effectiveBoxPass:rawEqualsBox||zeroFallbackPass,atrEqualsBox:Number.isFinite(Number(s.atr))&&Math.abs(Number(s.atr)-box)<=Math.max(1e-12,Math.abs(box)*1e-10),historySatisfied:!!s.atrHistorySatisfied,sourceCount:Number(s.atrHistorySourceCount)||0,historyFrom:Number(s.atrHistoryFrom)||0,historyTo:Number(s.atrHistoryTo)||0,confirmed:Number(br.length)||0,confirmedTotal:Number(s.confirmedTotal||s.base?.totalBricks||br.length)||0,renderLimit:Number(s.base?.renderLimit)||0,firstBrickTime:Number(first?.sourceTime)||0,lastBrickTime:Number(last?.sourceTime)||0,anchor:Number(s.base?.anchor),applyMs:Number(document.documentElement.dataset.atrMatrixApplyMs)||0,runtimeTbtMs:Number(document.documentElement.dataset.atrMatrixTbtMs)||0,observedTbtMs:tbt,provider:s.atrMatrixProvider||'',badge:document.querySelector('.method[data-method="atr"] .method-title span')?.textContent||'',coverage:document.getElementById('tvCoverage')?.textContent||'',selectorExists:!!document.querySelector('#intervalSelect'),fixedInterval:window.RENKO_FIXED_INTERVAL,currentInterval:document.getElementById('currentInterval')?.textContent||'',baseBox:Number(s.base?.box),brickBudget:document.documentElement.dataset.renkoBrickBudget||''};
    },n);
    const positiveGeometryPass=row.rawAtr>0&&row.rawEqualsBox&&row.confirmed>0&&row.confirmed<=12000&&row.confirmedTotal>=row.confirmed&&row.firstBrickTime>0&&row.lastBrickTime>=row.firstBrickTime;
    // A zero latest Wilder ATR legally falls back to the exchange minimum tick.
    // Historical fixed-1s prices may still span one or more minimum-tick boxes,
    // so accept either coherent zero geometry or coherent bounded real geometry.
    const zeroGeometryPass=row.rawAtr===0&&row.zeroFallbackPass&&row.confirmed>=0&&row.confirmed<=12000&&row.confirmedTotal>=row.confirmed&&((row.confirmed===0&&row.confirmedTotal===0&&row.firstBrickTime===0&&row.lastBrickTime===0)||(row.confirmed>0&&row.confirmedTotal>0&&row.firstBrickTime>0&&row.lastBrickTime>=row.firstBrickTime));
    row.pass=row.method==='atr'&&row.interval==='1s'&&row.fixedInterval==='1s'&&!row.selectorExists&&row.effectiveBoxPass&&row.atrEqualsBox&&row.historySatisfied&&row.sourceCount>=n&&(positiveGeometryPass||zeroGeometryPass)&&row.brickBudget==='true'&&row.observedTbtMs===0&&row.runtimeTbtMs===0&&row.applyMs<50&&Math.abs(row.baseBox-row.box)<=Math.max(1e-12,Math.abs(row.box)*1e-10);
    rows.push(row);
  ...
  const persistence=await page.evaluate(()=>{
    const T=window.RWARenkoTV,s=T.state,raw=Number(s.atrRaw),box=Number(s.box),atr=Number(s.atr),tickSize=Number(s.tickSize),tol=Math.max(1e-12,Math.abs(raw)*1e-10),tickTol=Math.max(1e-12,Math.abs(tickSize)*1e-10);
    const zeroFallback=!!s.atrZeroFallback||document.documentElement.dataset.atrZeroFallback==='true';
    const rawEqualsBox=Number.isFinite(raw)&&raw>0&&Math.abs(raw-box)<=tol;
    const zeroFallbackPass=raw===0&&zeroFallback&&Number.isFinite(tickSize)&&tickSize>0&&Math.abs(box-tickSize)<=tickTol;
    return {interval:T.settings.interval,length:Number(T.settings.atrLength),appliedLength:Number(s.atrAppliedLength),rawAtr:raw,atr,box,tickSize,rawEqualsBox,zeroFallback,zeroFallbackPass,effectiveBoxPass:rawEqualsBox||zeroFallbackPass,atrEqualsBox:Number.isFinite(atr)&&Math.abs(atr-box)<=Math.max(1e-12,Math.abs(box)*1e-10),historySatisfied:!!s.atrHistorySatisfied,sourceCount:Number(s.atrHistorySourceCount)||0,revision:Number(s.closedBars.at(-1)?.closeTime)||0,selectorExists:!!document.querySelector('#intervalSelect'),fixedInterval:window.RENKO_FIXED_INTERVAL,confirmed:Number(s.confirmed?.length)||0,confirmedTotal:Number(s.confirmedTotal||s.base?.totalBricks||s.confirmed?.length)||0,brickBudget:document.documentElement.dataset.renkoBrickBudget||'',stableStats:{...(window.RWARenkoStableChartV2?.stats||{})}};
  });
  ...
  const persistencePass=persistence.interval==='1s'&&persistence.fixedInterval==='1s'&&!persistence.selectorExists&&persistence.length===1000000&&persistence.appliedLength===1000000&&persistence.effectiveBoxPass&&persistence.atrEqualsBox&&persistence.historySatisfied&&persistence.sourceCount>=1000000&&persistence.revision>rev0&&persistence.brickBudget==='true'&&persistence.confirmed>0&&persistence.confirmed<=12000&&Math.abs(persistence.box-million.box)<=Math.max(1e-12,Math.abs(million.box)*1e-8);
...
const report={schema:'renko-xaut-fixed-1s-atr-seven-length-browser-v4-zero-safe',generatedAt:new Date().toISOString(),base,lengths,status:results.every(r=>r.pass)?'PASS':'FAIL',contract:'Production timeframe selector is absent and runtime is hard-locked to 1s. Each positive raw Wilder ATR becomes the actual Renko box for the seven requested lengths; a mathematically zero raw Wilder ATR uses only the legal exchange minimum-tick box, without timeframe/data substitution. The matrix is explicitly requested on demand, prepared switches have observed main-thread TBT = 0 ms, rendered geometry is bounded while total Renko count/final state are preserved, and deep ATR persists across later 1s closes.',claimBoundary:'Observable/documented Renko parity only; no claim of TradingView proprietary source-code identity.',results};
