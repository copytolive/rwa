...
async function run(label,viewport){
  ...
  for(const n of lengths){
    ...
    await page.waitForFunction(n=>window.RWARenkoTV?.state?.atrAppliedLength===n&&window.RWARenkoTV?.state?.atrHistorySatisfied===true&&document.documentElement.dataset.atrRawBoxPass==='true'&&Number(document.documentElement.dataset.atrLength)===n,n,{timeout:30000});
    await sleep(350);
    const row=await page.evaluate(n=>{
      ...
      const visibleText=document.getElementById('tvCoverage')?.textContent||'';
      const badge=document.querySelector('.method[data-method="atr"] .method-title span')?.textContent||'';
      return {length:n,method:T.settings.method,interval:T.settings.interval,source:T.settings.source,rawAtr:raw,box,rawEqualsBox:Number.isFinite(raw)&&raw>0&&Math.abs(raw-box)<=tol,historySatisfied:!!s.atrHistorySatisfied,sourceCount:Number(s.atrHistorySourceCount)||0,historyFrom:Number(s.atrHistoryFrom)||0,historyTo:Number(s.atrHistoryTo)||0,confirmed:Number(br.length)||0,firstBrickTime:Number(first?.sourceTime)||0,lastBrickTime:Number(last?.sourceTime)||0,anchor:Number(s.base?.anchor),applyMs:Number(s.atrMatrixLastApplyMs)||0,longTaskDelta:Number(s.atrMatrixLongTaskDelta)||0,warmMs:Number(s.atrMatrixWarmMs)||0,provider:s.atrMatrixProvider||'',badge,coverage:visibleText,matrixEntry:{satisfied:!!e?.satisfied,sourceCount:Number(e?.sourceCount)||0,fromTime:Number(e?.fromTime)||0,toTime:Number(e?.toTime)||0,rawAtr:Number(e?.rawAtr),box:Number(e?.box),brickCount:Number(e?.base?.bricks?.length)||0}};
    },n);
...
const report={schema:'renko-xaut-atr-seven-length-browser-v1',generatedAt:new Date().toISOString(),base,lengths,status:results.every(r=>r.pass)?'PASS':'FAIL',claimBoundary:'PASS means the seven XAUT ATR settings satisfy the documented/observable Wilder-ATR Renko contract on regular Gate Spot 1m OHLC data; it is not a claim of TradingView proprietary source-code identity.',results};
