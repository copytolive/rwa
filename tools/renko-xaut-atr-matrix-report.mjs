...
async function run(label,viewport){
  ...
  await page.waitForFunction(()=>window.RWARenkoTV?.state?.symbol==='XAUTUSDT'&&window.RWARenkoTV?.state?.status==='live'&&window.RWARenkoTV?.settings?.interval==='1m'&&window.RWARenkoTV?.state?.closedBars?.length>100&&document.documentElement.dataset.marketProvider==='gate-spot',null,{timeout:60000});
  await page.waitForFunction(()=>document.documentElement.dataset.atrMatrixReady==='true',null,{timeout:720000});
  await page.waitForFunction(()=>{const b=window.RWARenkoTV?.state?.closedBars?.at?.(-1),rev=Number(b?.closeTime||b?.openTime)||0;return rev>0&&Number(document.documentElement.dataset.atrMatrixRevision)===rev},null,{timeout:60000});
  await sleep(800);
  ...
  for(const n of lengths){
    await page.waitForFunction(()=>{const b=window.RWARenkoTV?.state?.closedBars?.at?.(-1),rev=Number(b?.closeTime||b?.openTime)||0;return Number(document.documentElement.dataset.atrMatrixRevision)===rev},null,{timeout:60000});
    ...
    const row=await page.evaluate(n=>{
      const T=window.RWARenkoTV,s=T.state,e=window.RWARenkoATRParity?.matrixEntries?.get?.(n),br=s.confirmed||[],first=br[0],last=br.at(-1),raw=Number(s.atrRaw),box=Number(s.box),tol=Math.max(1e-12,Math.abs(raw)*1e-10);
  ...
  results.push({label,viewport,url,httpStatus:response?.status(),ready,rows,progressiveHistoryFrom:progressive,deepBrickDatesInsideSourceCoverage:deepActual,applyNoBlock,errors,consoleErrors,providerFailures:providerFailures.slice(0,50),providerFailureCount:providerFailures.length,pass});
  await page.close();
}

try{await run('desktop',{width:1900,height:1000});await run('mobile',{width:390,height:844})}finally{await browser.close()}
const report={schema:'renko-xaut-atr-seven-length-browser-v3',generatedAt:new Date().toISOString(),base,lengths,status:results.every(r=>r.pass)?'PASS':'FAIL',claimBoundary:'PASS means the seven XAUT ATR settings satisfy the documented/observable Wilder-ATR Renko contract on regular Gate Spot 1m OHLC data; it is not a claim of TradingView proprietary source-code identity.',results};
