...
async function run(label,viewport){
  ...
  for(const n of lengths){
    ...
    row.pass=row.method==='atr'&&row.interval==='1s'&&row.fixedInterval==='1s'&&!row.selectorExists&&row.effectiveBoxPass&&row.atrEqualsBox&&row.historySatisfied&&row.sourceCount>=n&&row.confirmed>0&&row.confirmed<=12000&&row.confirmedTotal>=row.confirmed&&row.brickBudget==='true'&&row.firstBrickTime>0&&row.lastBrickTime>=row.firstBrickTime&&row.observedTbtMs===0&&row.runtimeTbtMs===0&&row.applyMs<50&&Math.abs(row.baseBox-row.box)<=Math.max(1e-12,Math.abs(row.box)*1e-10);
    rows.push(row);
