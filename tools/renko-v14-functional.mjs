...
async function openSymbol(base){
  ...
  await f.waitForFunction(()=>window.RWARenkoV14?.version==='14.0.0'&&window.RWARenkoV14MethodProfiles?.version==='1.0.0',{timeout:60000});
  ...
  await f.waitForFunction(()=>window.RWARenkoV14.state.bars.length>0&&!window.RWARenkoV14.state.building,{timeout:90000});
  const state=await f.evaluate(()=>({v3:RWARenkoV3.state.selected,v14:RWARenkoV14.state.symbol,bars:RWARenkoV14.state.bars.length,bricks:RWARenkoV14.state.data.length,ltp:RWARenkoV14.state.ltpSnapshot,rule:RWARenkoV14.percentageRule,realtime:RWARenkoV14.realtimeBehavior}));
...
try{
  ...
  const pct=f.locator('#v14Percentage');await pct.fill('0,25');await f.waitForTimeout(1100);if(await pct.inputValue()!=='0,25')fail('Percentage draft rewrote itself');await f.locator('[data-v14-apply="percentage"]').click();
  await f.waitForFunction(()=>window.RWARenkoV14.settings.method==='percentage'&&Math.abs(Number(window.RWARenkoV14.settings.percentage)-.0025)<1e-12&&!window.RWARenkoV14.state.building,{timeout:60000});
  ...
  const atr=f.locator('#v14AtrLength');await atr.fill('21');await f.waitForTimeout(1100);if(await atr.inputValue()!=='21')fail('ATR draft rewrote itself');await f.locator('[data-v14-apply="atr"]').click();await f.waitForFunction(()=>window.RWARenkoV14.settings.method==='atr'&&Number(window.RWARenkoV14.settings.atrLength)===21&&!window.RWARenkoV14.state.building,{timeout:60000});
  const a=await f.evaluate(()=>({box:RWARenkoV14.state.box,atr:RWARenkoV14.state.atrValue,ok:RWARenkoV14MethodProfiles.verify('atr')}));if(!a.ok||!(a.box>0)||!(a.atr>0))fail(`ATR Apply mismatch ${JSON.stringify(a)}`);
  ...
  const before=await f.evaluate(()=>({first:Number(RWARenkoV14.state.bars[0]?.[0]),bars:RWARenkoV14.state.bars.length,bricks:RWARenkoV14.state.data.length}));await f.evaluate(()=>RWARenkoV14.loadOlder(2));await f.waitForFunction(()=>!window.RWARenkoV14.state.loading&&!window.RWARenkoV14.state.building,{timeout:90000});const after=await f.evaluate(()=>({first:Number(RWARenkoV14.state.bars[0]?.[0]),bars:RWARenkoV14.state.bars.length,bricks:RWARenkoV14.state.data.length,projection:RWARenkoV14.state.projection.length,contract:{pct:RWARenkoV14.percentageRule,real:RWARenkoV14.realtimeBehavior,source:RWARenkoV14.historicalConfirmation}}));if(!(after.first<before.first)||!(after.bars>before.bars))fail(`History did not extend ${JSON.stringify({before,after})}`);
  if(after.contract.pct!=='symbol-load-ltp-fixed-across-bars'||after.contract.real!=='projection-until-source-bar-closes')fail('Final TradingView parity contracts missing');
  const fatal=errors.filter(x=>/RENKO V14|renko-v14/i.test(x));if(fatal.length)fail(fatal.slice(0,5).join(' | '));
  console.log('RENKO V14 FUNCTIONAL PASS',JSON.stringify(after));await page.close();
