...
try{
  ...
  await page.waitForFunction(()=>window.RWARenkoTV?.state?.status==='live'&&window.RWARenkoSwitchFast?.version==='1.2.0-history-scroll'&&window.RWARenkoScrollZeroLock?.version==='1.0.0'&&window.RWARenkoTraditionalControl?.version&&document.querySelectorAll('.pair-row[data-symbol]').length>=50,null,{timeout:90000});
...
const criticalConsole=consoleErrors.filter(x=>!/favicon|ERR_FAILED|Failed to load resource|WebSocket connection .* failed: Ping received after close/i.test(x));
