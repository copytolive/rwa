...
let crashed=false,closedUnexpectedly=false;
...
try{
  ...
  await page.waitForFunction(()=>window.RWARenkoTV?.state?.symbol==='XAUTUSDT'&&window.RWARenkoTV?.state?.status==='live'&&window.RWARenkoTV?.state?.closedBars?.length>100&&window.RWARenkoSwitchFast?.version&&window.RWARenkoATRFixed1s?.version&&window.RWARenkoATRFixed1s.workerActive===false&&document.documentElement.dataset.atrMatrixReady==='idle',null,{timeout:90000});
  await page.waitForFunction(()=>document.querySelectorAll('.pair-row').length>=80,null,{timeout:60000});
  initial=await page.evaluate(()=>({symbol:RWARenkoTV.state.symbol,status:RWARenkoTV.state.status,bars:RWARenkoTV.state.closedBars.length,box:RWARenkoTV.state.box,last:RWARenkoTV.state.lastPrice,switchVersion:RWARenkoSwitchFast.version,atrVersion:RWARenkoATRFixed1s.version,atrWorkerActive:RWARenkoATRFixed1s.workerActive,atrEntryCount:RWARenkoATRFixed1s.entryCount,chartEmptyDisplay:getComputedStyle(document.getElementById('chartEmpty')).display,canvases:document.querySelectorAll('#chartHost canvas').length,pairRows:document.querySelectorAll('.pair-row').length}));
  const available=await page.evaluate(()=>[...document.querySelectorAll('.pair-row')].map(r=>(r.querySelector('.pair-main b')?.textContent||'').replace(/\s*\/\s*/g,'').toUpperCase()).filter(Boolean));
  const availableSet=new Set(available),selected=[];for(const s of preferred)if(availableSet.has(s)&&!selected.includes(s))selected.push(s);for(const s of available)if(s.endsWith('USDT')&&!selected.includes(s))selected.push(s);symbols=selected.slice(0,50);
  if(symbols.length!==50)throw new Error(`Need 50 available pair rows, got ${symbols.length}`);
  await gc();startMetrics=await metrics();
  for(let i=0;i<symbols.length;i++){
    const symbol=symbols[i];
    const first=await page.evaluate(symbol=>{
      const rows=[...document.querySelectorAll('.pair-row')],row=rows.find(r=>(r.querySelector('.pair-main b')?.textContent||'').replace(/\s*\/\s*/g,'').toUpperCase()===symbol);
...
const returnPass=!!returnXaut&&retFirst.firstFrameMs<=16&&retFirst.chartEmptyDisplay==='none'&&retFirst.overlayVisible&&retFirst.canvasCount>0&&!retFirst.workerActive&&returnXaut.symbol==='XAUTUSDT'&&returnXaut.status==='live'&&returnXaut.bars>50&&returnXaut.box>0&&returnXaut.last>0&&returnXaut.chartEmptyDisplay==='none'&&!returnXaut.workerActive&&returnXaut.entryCount===0;
const status=!fatal&&httpStatus>=200&&httpStatus<400&&initial.symbol==='XAUTUSDT'&&!initial.atrWorkerActive&&initial.atrEntryCount===0&&initial.chartEmptyDisplay==='none'&&initial.canvases>0&&rows.length===50&&rows.every(r=>r.pass)&&returnPass&&!crashed&&!closedUnexpectedly&&!pageErrors.length&&!criticalConsole.length&&summary.firstFrameMaxMs<=16&&summary.loadP95Ms<6000&&summary.loadMaxMs<12000&&summary.longTaskMaxMs<120&&summary.heapEndMB<256&&heapGrowthMB<96?'PASS':'FAIL';
const report={schema:'renko-50pair-zero-ms-switch-browser-v1',generatedAt:new Date().toISOString(),base,url,status,contract:'Starting from live XAUT, click 50 different real pair rows. The synchronous first frame must acknowledge the target in <=16ms, keep the prior chart canvas visible, never expose the full-screen loader, keep the million-row XAUT ATR worker idle, complete each switch live, avoid browser crash, and remain memory-bounded. Then return to XAUT with the same rules.',httpStatus,initial,symbols,summary,startMetrics,endMetrics,returnXaut,returnPass,crashed,closedUnexpectedly,pageErrors,consoleErrors:criticalConsole,requestFailures:criticalRequests.slice(0,100),requestFailureCount:criticalRequests.length,fatal,rows};
