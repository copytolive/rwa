...
for(let i=0;i<symbols.length;i++){
  ...
  await page.waitForFunction(symbol=>document.documentElement.dataset.atrControlStatus==='active'&&document.documentElement.dataset.atrControlSymbol===symbol&&Number(document.documentElement.dataset.atrAppliedLength)===100&&RWARenkoTV?.settings?.method==='atr'&&Number(RWARenkoTV?.settings?.atrLength)===100&&Number(RWARenkoTV?.state?.atrAppliedLength)===100,symbol,{timeout:15000});
  const row=await page.evaluate(symbol=>{
    const T=RWARenkoTV,s=T.state,raw=Number(s.atrRaw),box=Number(s.box),tol=Math.max(1e-12,Math.abs(raw)*1e-10);
    return {symbol,actualSymbol:s.symbol,status:s.status,method:T.settings.method,length:Number(T.settings.atrLength),appliedLength:Number(s.atrAppliedLength),historySatisfied:!!s.atrHistorySatisfied,sourceCount:Number(s.atrHistorySourceCount)||0,rawAtr:raw,box,rawEqualsBox:Number.isFinite(raw)&&raw>0&&Number.isFinite(box)&&Math.abs(raw-box)<=tol,badge:document.querySelector('.method[data-method="atr"] .method-title span')?.textContent||'',input:document.getElementById('atrLength')?.value||'',controlStatus:document.documentElement.dataset.atrControlStatus||'',workerActive:!!window.RWARenkoATRFixed1s?.workerActive,entryCount:Number(window.RWARenkoATRFixed1s?.entryCount)||0,exactBoxOwn:Object.prototype.hasOwnProperty.call(T.settings,'_exactBox'),interval:T.settings.interval,selectorExists:!!document.querySelector('#intervalSelect'),loadText:document.getElementById('tvLoadState')?.textContent||''};
  },symbol);
  row.pass=row.actualSymbol===symbol&&row.status==='live'&&row.method==='atr'&&row.length===100&&row.appliedLength===100&&row.historySatisfied&&row.sourceCount>=100&&row.rawEqualsBox&&row.badge.includes('ACTIVE')&&row.input==='100'&&row.controlStatus==='active'&&!row.workerActive&&row.entryCount===0&&!row.exactBoxOwn&&row.interval==='1s'&&!row.selectorExists;
  rows.push(row);
  await page.screenshot({path:path.join(out,'screens',`${String(i+1).padStart(2,'0')}-${symbol}-atr100.png`),fullPage:true});
}
const report={schema:'renko-atr-control-50pair-browser-v1',generatedAt:new Date().toISOString(),base,mode,viewport,httpStatus:response?.status(),count:rows.length,passCount:rows.filter(r=>r.pass).length,status:response?.ok()&&rows.length===50&&rows.every(r=>r.pass)&&!errors.length?'PASS':'FAIL',contract:'Physical ATR Apply button must work for ATR length 100 on every non-XAUT launch pair using that pair own fixed-1s Binance bars. The XAUT deep-matrix worker must remain idle on these pairs.',rows,errors,consoleErrors,requestFailures:failed.slice(0,50)};
