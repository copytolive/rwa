...
for(let i=0;i<symbols.length;i++){
  ...
  const row=await page.evaluate(symbol=>{
    ...
    const visibleAtr=(document.getElementById('currentAtr')?.textContent||'').trim();
    const visibleBox=(document.getElementById('brickValue')?.textContent||'').trim();
    const parseVisible=v=>Number(String(v).replace(/,/g,''));
    const pricePrecision=Number(document.documentElement.dataset.renkoPricePrecision||0);
    const requiredPrecision=typeof T.precisionFor==='function'?Number(T.precisionFor(s.lastPrice,s.tickSize,raw,box))||0:0;
    return {symbol,actualSymbol:s.symbol,status:s.status,method:T.settings.method,length:Number(T.settings.atrLength),appliedLength:Number(s.atrAppliedLength),historySatisfied:!!s.atrHistorySatisfied,sourceCount:Number(s.atrHistorySourceCount)||0,rawAtr:raw,box,exactBox:exact,rawEqualsBox:Number.isFinite(raw)&&raw>0&&Number.isFinite(box)&&Math.abs(raw-box)<=tol,exactEqualsRaw:Number.isFinite(exact)&&Math.abs(raw-exact)<=tol,badge:document.querySelector('.method[data-method="atr"] .method-title span')?.textContent||'',input:document.getElementById('atrLength')?.value||'',controlStatus:document.documentElement.dataset.atrControlStatus||'',stableBox:document.documentElement.dataset.renkoAtrStableBox||'',workerActive:!!window.RWARenkoATRFixed1s?.workerActive,entryCount:Number(window.RWARenkoATRFixed1s?.entryCount)||0,exactBoxOwn:Object.prototype.hasOwnProperty.call(T.settings,'_exactBox'),interval:T.settings.interval,selectorExists:!!document.querySelector('#intervalSelect'),loadText:document.getElementById('tvLoadState')?.textContent||'',visibleAtr,visibleBox,visibleAtrPositive:parseVisible(visibleAtr)>0,visibleBoxPositive:parseVisible(visibleBox)>0,pricePrecision,requiredPrecision,pricePrecisionPass:pricePrecision>=requiredPrecision};
  },symbol);
  row.pass=row.actualSymbol===symbol&&row.status==='live'&&row.method==='atr'&&row.length===100&&row.appliedLength===100&&row.historySatisfied&&row.sourceCount>=100&&row.rawEqualsBox&&row.exactEqualsRaw&&row.badge.includes('ACTIVE')&&row.input==='100'&&row.controlStatus==='active'&&row.stableBox==='true'&&!row.workerActive&&row.entryCount===0&&row.exactBoxOwn&&row.interval==='1s'&&!row.selectorExists&&row.visibleAtrPositive&&row.visibleBoxPositive&&row.pricePrecisionPass;
  rows.push(row);
  await page.screenshot({path:path.join(out,'screens',`${String(i+1).padStart(2,'0')}-${symbol}-atr100.png`),fullPage:true});
}
const report={schema:'renko-atr-control-50pair-browser-v3-precision-safe',generatedAt:new Date().toISOString(),base,mode,viewport,httpStatus:response?.status(),count:rows.length,passCount:rows.filter(r=>r.pass).length,status:response?.ok()&&rows.length===50&&rows.every(r=>r.pass)&&!errors.length?'PASS':'FAIL',contract:'Physical ATR Apply must work for length 100 on every non-XAUT launch pair using that pair own fixed-1s Binance bars. Raw ATR must remain the stable _exactBox between closes, XAUT deep matrix must remain idle, and every positive ATR/box including tiny-price SHIB/PEPE values must remain visibly positive with enough chart price precision instead of rendering as zero.',rows,errors,consoleErrors,requestFailures:failed.slice(0,50)};
fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));
console.log('RENKO_ATR_CONTROL_50PAIR '+JSON.stringify({status:report.status,mode,count:report.count,passCount:report.passCount,failed:rows.filter(r=>!r.pass).map(r=>({symbol:r.symbol,visibleAtr:r.visibleAtr,visibleBox:r.visibleBox,pricePrecision:r.pricePrecision,requiredPrecision:r.requiredPrecision})),errors}));
await browser.close();
