...
try{
  ...
  await page.waitForFunction(()=>window.RWARenko?.version==='5.3.0'&&window.RWARenkoV5Auto?.version==='5.4.0'&&window.RWARenkoV3?.state?.symbols?.length>=650&&window.RWARenkoV3?.state?.tickCount>=9,{timeout:15000});
  ...
  const archive=await page.evaluate(async()=>{
    ...
    const w=new Worker('archive-worker-v5.js?v=54');
  ...
  console.log(JSON.stringify({ok:true,contract:'rwa-renko-v5.4-owned-visible-history-ui',desktop,preview,archive,mobile},null,2));
  await context.close();
