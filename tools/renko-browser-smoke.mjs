...
async function desktop(browser){
  ...
  await page.waitForFunction(()=>window.RWARenko?.version==='2.0.0'&&window.RWARenko.state.symbols.length>=650&&window.RWARenko.state.tickCount>=9,{timeout:10000});
  const universe=await page.evaluate(()=>({count:window.RWARenko.state.symbols.length,rows:document.querySelectorAll('.pair-row').length,source:window.RWARenko.source,method:window.RWARenko.method,reversal:window.RWARenko.reversalBoxes,selected:window.RWARenko.state.selected,last:window.RWARenko.state.lastPrice,health:document.querySelector('#tickHealth')?.textContent}));
  assert.ok(universe.count>=650,`full universe too small ${universe.count}`);assert.equal(universe.rows,500,'default list must render top 500');assert.equal(universe.source,'raw-trade-ticks-only');assert.equal(universe.method,'traditional-fixed-box');assert.equal(universe.reversal,2);assert.equal(universe.selected,'BTCUSDT');assert.equal(universe.last,112);
  const engine=await page.evaluate(()=>{const r=window.RWARenko,s=r.state;s.historyTicks=[];s.bricks=[];s.tickCount=0;s.lastPrice=NaN;s.lastTickTime=0;s.lastTradeId=null;s.lastClose=NaN;s.direction=0;s.anchor=NaN;s.pan=0;s.box=10;[100,110,120,111,100,90,110].forEach((price,i)=>r.applyTick({id:i+1,price,time:1700000000000+i*1000},true,false));return{bricks:s.bricks.map(b=>({o:b.open,c:b.close,d:b.direction})),tickCount:s.tickCount}});
  assert.equal(engine.tickCount,7);assert.deepEqual(engine.bricks,[{o:100,c:110,d:1},{o:110,c:120,d:1},{o:110,c:100,d:-1},{o:100,c:90,d:-1},{o:100,c:110,d:1}]);
  ...
  const layout=await page.evaluate(()=>({scrollWidth:document.documentElement.scrollWidth,innerWidth,canvas:document.querySelector('#renkoCanvas').getBoundingClientRect().toJSON(),easy:!!document.querySelector('#quickPairs'),auto:!!document.querySelector('#autoBox'),pairTotal:document.querySelector('#pairTotal')?.textContent}));
  assert.ok(layout.scrollWidth<=layout.innerWidth+2,`desktop overflow ${layout.scrollWidth}/${layout.innerWidth}`);assert.ok(layout.canvas.width>600&&layout.canvas.height>300);assert.equal(layout.easy,true);assert.equal(layout.auto,true);assert.match(layout.pairTotal,/markets/);assert.equal(errors.length,0,errors.join(' | '));await context.close();return{universe,engine,layout};
}

async function mobile(browser){
  const context=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true});await setup(context);const page=await context.newPage();const errors=[];page.on('pageerror',e=>errors.push(String(e)));await page.goto(BASE,{waitUntil:'domcontentloaded',timeout:20000});await page.waitForFunction(()=>window.RWARenko?.state.symbols.length>=650&&window.RWARenko.state.tickCount>=9,{timeout:10000});
  const before=await page.evaluate(()=>({scrollWidth:document.documentElement.scrollWidth,innerWidth,markets:getComputedStyle(document.querySelector('.markets')).display,button:getComputedStyle(document.querySelector('#openPairs')).display,canvas:document.querySelector('#renkoCanvas').getBoundingClientRect().toJSON(),health:document.querySelector('#tickHealth')?.textContent}));
  assert.ok(before.scrollWidth<=392);assert.equal(before.markets,'none');assert.notEqual(before.button,'none');assert.ok(before.canvas.width<=390&&before.canvas.height>=300);
...
try{const d=await desktop(browser),m=await mobile(browser);console.log(JSON.stringify({ok:true,contract:'rwa-renko-v2-top500-live-tick-ux',desktop:d,mobile:m},null,2))}finally{await browser.close()}
