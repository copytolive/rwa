...
function parseCsv(buf,streams){const text=buf.toString('utf8');for(const line of text.split(/\r?\n/)){if(!line)continue;const c=line.split(','),id=Number(c[0]),price=Number(c[1]),time=Number(c[5]);if(!Number.isFinite(id)||!Number.isFinite(price)||!Number.isFinite(time))continue;for(const s of streams)applyTrade(s,id,price,time)}}
async function loadArchive(url,label,streams){const r=await fetch(url);if(r.status===404){console.log('PRELOAD_MISSING',label);return{loaded:0,bytes:0}}if(!r.ok)throw new Error(`${label} HTTP ${r.status}`);const ab=await r.arrayBuffer();parseCsv(unzip(ab),streams);console.log('PRELOAD_ARCHIVE',label,'bytes',ab.byteLength,'maxBricks',streams.at(-1)?.bricks.length||0);return{loaded:1,bytes:ab.byteLength}}
...
const result={version:2,contract:'tick-derived-first-frame-preview-then-exact-background',symbol:SYMBOL,tickSize:tick,ltpAtBuild:ltp,generatedAt:new Date().toISOString(),shortLookbackDays:DAYS,longLookbackMonths:MONTHS,archivesLoaded:loaded,archiveBytes:bytes,targetVisible:46,seeds:outSeeds};
