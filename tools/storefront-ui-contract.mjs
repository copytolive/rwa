...
const [registryText,storefront,safety,draftUi,draftCss,uiCss,quick,tradePolish,tradeCss,tradeCfg,assetsText]=await Promise.all([
  read('rwa-commerce-registry.json'),read('rwa-storefront.js'),read('rwa-storefront-safety-patch.js'),read('rwa-store-draft-ui.js'),read('rwa-store-draft-ui.css'),read('rwa-ui-pro.css'),read('quick-actions.js'),read('trade/ui-polish.js'),read('trade/ui-polish.css'),read('trade/config.js'),read('rwa-assets.json')
]);
...
const uiRuntime=storefront+safety+draftUi+tradePolish;
...
ok(quick.includes("loadScript('rwa-storefront.js?v=1')")&&quick.includes("loadScript('rwa-storefront-safety-patch.js?v=1')")&&quick.includes("loadScript('rwa-store-draft-ui.js?v=1')")&&quick.includes("runtime:'premium-shell-storefront-v1'"),'root shell does not load the complete storefront UI stack');
...
if(fail.length){console.error(JSON.stringify({ok:false,contract:'rwa-physical-commerce-ui-v2',fail},null,2));process.exit(1)}
console.log(JSON.stringify({ok:true,contract:'rwa-physical-commerce-ui-v2',policy:registry.policy,liveStores:registry.stores.length,verifiedAssets:assets.verified.length,frontend:'desktop+mobile+shop+cart+command-palette+physical-store-onboarding',checkout:'BACKEND_GATED'},null,2));
