...
try{
 ...
 const root=await context.newPage();const rootErrors=[];root.on('pageerror',e=>rootErrors.push(String(e)));await root.goto(base,{waitUntil:'domcontentloaded'});await root.locator('.institutional').waitFor({state:'visible',timeout:15000});assert.equal((await root.locator('.institutional').textContent()).trim(),'Create RWA');await root.click('.institutional');const iframe=root.locator('[data-global-rwa-factory] iframe');await iframe.waitFor({state:'attached',timeout:15000});assert.match(await iframe.getAttribute('src'),/rwa-8-engines\/\?embed=1/);assert.ok(await root.locator('[data-global-rwa-factory]').count()===1,'single internal factory mount required');assert.deepEqual(rootErrors,[],`root page errors: ${rootErrors.join(' | ')}`);
