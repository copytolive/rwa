...
try{
 ...
 await page.click('#addAsset');const rows=page.locator('[data-asset-row]');assert.equal(await rows.count(),2);const second=rows.nth(1);await second.locator('[data-asset="assetType"]').selectOption({label:'Receivables / Invoice'});await second.locator('[data-asset="name"]').fill('Export invoice pool');await second.locator('[data-asset="declaredValue"]').fill('750000');await second.locator('[data-asset="currency"]').fill('USD');await page.click('button.primary');await page.waitForFunction(()=>{try{return JSON.parse(document.getElementById('resultJson')?.textContent||'{}').registry?.assets?.length===2}catch{return false}});const multi=JSON.parse(await page.locator('#resultJson').textContent());assert.equal(multi.registry.assets.length,2,'multi-asset registry required');
 await page.selectOption('select[name="mode"]','FINANCE');await page.click('button.primary');await page.waitForFunction(()=>{try{return JSON.parse(document.getElementById('resultJson')?.textContent||'{}').mode==='FINANCE'}catch{return false}});const finance=JSON.parse(await page.locator('#resultJson').textContent());assert.equal(finance.ready.finance,false,'finance must fail closed without professional approvals');assert.equal(finance.factory.status,'BLOCKED');assert.ok(finance.blockers.includes('LICENSED_LEGAL_REVIEW_REQUIRED'));assert.ok(finance.blockers.includes('SANCTIONS_PEP_SCREENING_REQUIRED'));
 assert.deepEqual(errors,[],`factory page errors: ${errors.join(' | ')}`);
 const root=await context.newPage();const rootErrors=[];root.on('pageerror',e=>rootErrors.push(String(e)));await root.goto(base,{waitUntil:'domcontentloaded'});await root.waitForFunction(()=>window.RWASuperApp?.version==='5.0.0',{timeout:20000});await root.evaluate(()=>window.RWASuperApp.navigate('institutional'));const issuer=root.locator('[data-v5-action="issuer"]').first();await issuer.waitFor({state:'visible',timeout:15000});assert.match((await issuer.textContent()).trim(),/issuer/i,'issuer workspace CTA missing');await issuer.click();const iframe=root.locator('[data-global-rwa-factory] iframe');await iframe.waitFor({state:'attached',timeout:15000});assert.match(await iframe.getAttribute('src'),/rwa-8-engines\/\?embed=1/);assert.ok(await root.locator('[data-global-rwa-factory]').count()===1,'single internal factory mount required');assert.deepEqual(rootErrors,[],`root page errors: ${rootErrors.join(' | ')}`);
