...
const minute=60000,latestStart=1700000000000,olderStart=latestStart-1000*minute,oldestStart=olderStart-1000*minute;
...
const latest=makeBars(latestStart,1000,100),older=makeBars(olderStart,1000,-1100),oldest=makeBars(oldestStart,1000,-2300);
async function setup(context){
  ...
  await context.route('**/api/v3/klines?**',r=>{const u=new URL(r.request().url()),end=Number(u.searchParams.get('endTime')||0);let rows=latest;if(end&&end<olderStart)rows=oldest;else if(end&&end<latestStart)rows=older;r.fulfill({status:200,contentType:'application/json',body:JSON.stringify(rows)})});
}
...
try{
  ...
  const before=await page.evaluate(()=>({bars:window.RWARenkoV6.state.bars.length,bricks:window.RWARenkoV6.state.bricks.length,offset:window.RWARenkoV6.state.offset,generation:window.RWARenkoV6.state.generation,first:window.RWARenkoV6.state.bars[0]?.[0]}));
  const load=await page.evaluate(async()=>{const result=await window.RWARenkoV6.loadOlder();for(let i=0;i<8;i++)window.RWARenkoV6.moveOlder();await new Promise(r=>setTimeout(r,150));return {result,bars:window.RWARenkoV6.state.bars.length,bricks:window.RWARenkoV6.state.bricks.length,offset:window.RWARenkoV6.state.offset,generation:window.RWARenkoV6.state.generation,first:window.RWARenkoV6.state.bars[0]?.[0],lastLoad:window.RWARenkoV6.state.lastLoad};});
  assert.equal(load.result,true,JSON.stringify({before,load}));assert.equal(load.generation,before.generation);assert.ok(load.bars>before.bars,JSON.stringify({before,load}));assert.ok(load.bricks>=before.bricks);assert.ok(load.offset>before.offset);assert.ok(load.first<before.first);assert.ok(load.lastLoad?.added>0);
  ...
  const cache=await page.evaluate(()=>Object.keys(localStorage).filter(k=>k.startsWith('rwa_renko_v61_seed_')).length);assert.ok(cache>=1,'50-brick synchronous cache was not written');
  assert.equal(errors.length,0,errors.join(' | '));
  console.log(JSON.stringify({ok:true,contract:'renko-v6.1-generation-safe-lazy-50-all-binance-spot-search',initial,search,before,load,olderState,live,back,cache},null,2));
  await context.close();
