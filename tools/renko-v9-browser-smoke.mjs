...
try{
  const context=await browser.newContext({viewport:{width:1440,height:900}});await setup(context);const page=await context.newPage(),errors=[];page.on('pageerror',e=>errors.push(String(e)));page.on('requestfailed',r=>{if(!r.url().includes('/api/v3/'))console.warn('request failed',r.url(),r.failure()?.errorText)});
  await page.goto(BASE,{waitUntil:'domcontentloaded',timeout:30000});
  await page.waitForFunction(()=>window.LightweightCharts&&window.RWARenkoV9?.version==='9.1.0'&&window.RWARenkoV3?.state?.symbols?.length>600,{timeout:30000});
  try{await page.waitForFunction(()=>window.RWARenkoV9?.state?.data?.length>=50,{timeout:30000})}catch(e){console.error('initial state',await page.evaluate(()=>({v9:window.RWARenkoV9?.state,v3:{selected:window.RWARenkoV3?.state?.selected,box:window.RWARenkoV3?.state?.box,ticks:window.RWARenkoV3?.state?.historyTicks?.length},body:document.body.innerText.slice(0,1000)})));throw e}
  console.log('smoke: initial chart ready');
  ...
  assert.equal(initial.version,'9.1.0');assert.equal(initial.mode,'native-lightweight-charts');assert.equal(initial.renderer,'tradingview-lightweight-charts-5.1');assert.equal(initial.source,'binance-1m-close-background-worker');assert.equal(initial.live,'binance-individual-@trade');assert.ok(initial.data>=50);assert.ok(initial.markets>600);assert.equal(initial.rows,80);assert.ok(initial.host.width>800&&initial.host.height>400);assert.equal(initial.oldCanvas,'hidden');assert.equal(initial.oldControls,'none');assert.ok(!initial.text.includes('one-minute bars currently loaded'));assert.match(String(initial.attribution),/TradingView Lightweight Charts/i);assert.ok(initial.width>=48&&initial.width<=56,`initial logical width ${initial.width}`);

  const applyStart=Date.now();await page.fill('#brickSize','2');await page.click('#applyBrick');await page.waitForFunction(()=>RWARenkoV9.state.box===2&&RWARenkoV9.state.data.length>=50,{timeout:15000});assert.ok(Date.now()-applyStart<15000,'Apply blocked the UI');
  console.log('smoke: apply ready');

  const wrap=page.locator('#chartWrap'),br=await wrap.boundingBox();assert.ok(br);
  const beforeZoom=await page.evaluate(()=>RWARenkoV9.chart.timeScale().getVisibleLogicalRange());await page.mouse.move(br.x+br.width*.56,br.y+br.height*.5);for(let i=0;i<8;i++)await page.mouse.wheel(0,65);await page.waitForTimeout(180);const afterZoom=await page.evaluate(()=>RWARenkoV9.chart.timeScale().getVisibleLogicalRange());assert.ok((afterZoom.to-afterZoom.from)>(beforeZoom.to-beforeZoom.from)+2,'native wheel zoom-out did not widen logical range');
  console.log('smoke: zoom ready');
  ...
  console.log('smoke: lazy history ready');

  await page.click('#tvLive');await page.waitForTimeout(100);const liveState=await page.evaluate(()=>({range:RWARenkoV9.chart.timeScale().getVisibleLogicalRange(),n:RWARenkoV9.state.data.length}));assert.ok(liveState.range.to>=liveState.n-2,`LIVE did not return to latest bricks: ${JSON.stringify(liveState)}`);
  const beforeButtons=await page.evaluate(()=>RWARenkoV9.chart.timeScale().getVisibleLogicalRange());await page.click('#tvPanOlder');await page.waitForTimeout(80);const afterOlderButton=await page.evaluate(()=>RWARenkoV9.chart.timeScale().getVisibleLogicalRange());assert.ok(afterOlderButton.from<beforeButtons.from-2,'older button did not pan');await page.click('#tvPanNewer');await page.click('#tvZoomIn');await page.click('#tvZoomOut');await page.click('#tvReset');
  ...
  console.log(JSON.stringify({ok:true,contract:'renko-v9-native-lightweight-charts',initial,beforeZoom,afterZoom,beforeDrag,afterDrag,beforeOlder,afterOlder,liveState,mobile:mm},null,2));
