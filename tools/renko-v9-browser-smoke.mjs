...
try{
  ...
  await page.click('#tvLive');await page.waitForTimeout(100);const liveState=await page.evaluate(()=>({range:RWARenkoV9.chart.timeScale().getVisibleLogicalRange(),n:RWARenkoV9.state.data.length}));assert.ok(liveState.range.to>=liveState.n-2,'LIVE did not return to latest bricks');
