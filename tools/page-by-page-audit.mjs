...
const intel=page.locator('[data-rwa-target-nav="analytics"]').first();if(await intel.count()){await intel.click();await page.waitForSelector('#rwaTradingWorkspace:not([hidden])',{timeout:12000});await snap('02-intelligence-desktop');await page.locator('[data-workspace-close]').click();await page.waitForTimeout(200)}
...
const rewards=page.locator('[data-rwa-target-nav="rewards"]').first();if(await rewards.count()){await rewards.click();await page.waitForSelector('#rwaTradingWorkspace:not([hidden])',{timeout:10000});await snap('06-rewards-desktop');await page.locator('[data-workspace-close]').click()}
...
const mc=page.locator('#rwaMultiChainLaunch').first();if(await mc.count()){await mc.click();await page.waitForFunction(()=>window.RWAMultiChain?.status?.().open===true,{timeout:12000});await snap('07-multichain-desktop');await page.locator('#rwaMultiChainPanel .rwa-mc-close').click()}
...
await ctx.close();
for(const [w,h,n] of [[390,844,'08-markets-mobile-390'],[430,932,'09-markets-mobile-430']]){
 const c=await browser.newContext({viewport:{width:w,height:h},deviceScaleFactor:1,serviceWorkers:'block'});const p=await c.newPage();p.on('pageerror',e=>errors.push(n+': '+String(e?.message||e)));await p.goto(base,{waitUntil:'domcontentloaded',timeout:50000});await p.waitForFunction(()=>window.RWALiveHome?.version==='4.0.0'&&window.RWAMarketRuntime?.state?.().pairs?.length>50,{timeout:35000});await p.waitForTimeout(600);await p.screenshot({path:proof+'/'+n+'.png',fullPage:false});await c.close();
}
