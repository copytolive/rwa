...
for(const [w,h,n] of [[390,844,'07-markets-mobile-390'],[430,932,'08-markets-mobile-430']]){
 const c=await browser.newContext({viewport:{width:w,height:h},deviceScaleFactor:1,serviceWorkers:'block'});const p=await c.newPage();p.on('pageerror',e=>errors.push(n+': '+String(e?.message||e)));await p.goto(base,{waitUntil:'domcontentloaded',timeout:50000});await p.waitForFunction(()=>window.RWALiveHome?.version==='3.1.0'&&window.RWAMarketRuntime?.state?.().pairs?.length>50,{timeout:35000});await p.waitForTimeout(600);await p.screenshot({path:proof+'/'+n+'.png',fullPage:false});await c.close();
}
