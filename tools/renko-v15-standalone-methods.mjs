...
async function applyFast(method,expectBox,maxMs=6000){const before=await sig(),start=Date.now();await page.locator(`[data-v15-apply="${method}"]`).click({timeout:1500});await page.waitForFunction(({m,b})=>{const root=document.querySelector('#v15BoxCard');return window.RWARenkoV15MethodProfiles?.version==='1.6.0'&&RWARenkoV15.settings.method===m&&!RWARenkoV15.state.building&&RWARenkoV15.state.data.length>0&&root?.dataset.lastApplied===m&&root?.dataset.applyVerified==='1'&&(!Number.isFinite(b)||Math.abs(Number(RWARenkoV15.state.box)-b)<Math.max(.02,Math.abs(b)*1e-9))},{m:method,b:expectBox},{timeout:10000});const wall=Date.now()-start;if(wall>maxMs)fail(`${method}: ${wall}ms > ${maxMs}ms`);if(await active()!==method)fail(`${method}: ACTIVE mismatch`);const out=await page.evaluate(()=>({box:RWARenkoV15.state.box,bricks:RWARenkoV15.state.data.length,applyMs:Number(document.querySelector('#v15BoxCard')?.dataset.applyMs||0),verified:RWARenkoV15MethodProfiles.verify(RWARenkoV15.settings.method),audit:RWARenkoV15.uniformAudit()}));if(!out.verified||!out.audit?.ok||out.bricks<=0||out.applyMs>maxMs)fail(`${method}: invalid ${JSON.stringify(out)}`);return{before,after:await sig(),wall,out}}
try{
  await page.goto(`${BASE}renko/?symbol=BTC&qa=157`,{waitUntil:'domcontentloaded',timeout:45000});
  await page.waitForFunction(()=>window.RWARenkoV15?.version==='15.1.0'&&document.documentElement.dataset.renkoMethodBootstrap==='157'&&window.RWARenkoV15MethodProfiles?.version==='1.6.0',null,{timeout:90000});
  ...
  await page.locator('#v15AtrLength').click({timeout:1000});await page.locator('#v15AtrLength').fill('14');
