...
async function apply(method,expectBox){
  ...
  await page.waitForFunction(({m,b})=>{
    ...
    return window.RWARenkoV15MethodProfiles?.version==='1.3.0'&&RWARenkoV15.settings.method===m&&!RWARenkoV15.state.building&&!RWARenkoV15.state.loading&&root?.dataset.lastApplied===m&&root?.dataset.applyVerified==='1'&&(!Number.isFinite(b)||Math.abs(Number(RWARenkoV15.state.box)-b)<Math.max(.02,Math.abs(b)*1e-9));
  },{m:method,b:expectBox},{timeout:120000});
  if(await active()!==method)fail(`${method}: ACTIVE badge mismatch`);
  const out=await page.evaluate(()=>({method:RWARenkoV15.settings.method,box:RWARenkoV15.state.box,summary:document.querySelector('#v11MethodLabel')?.textContent,verified:RWARenkoV15MethodProfiles.verify(RWARenkoV15.settings.method),audit:RWARenkoV15.uniformAudit(),changed:document.querySelector('#v15BoxCard')?.dataset.chartChanged,controller:RWARenkoV15MethodProfiles.switchContract}));
  if(!out.verified||!out.audit?.ok||out.controller!=='verified-only-active-latest-click-wins')fail(`${method}: verification failed ${JSON.stringify(out)}`);
...
try{
  ...
  // Exact user case: Traditional box 10 must rebuild and become verified ACTIVE.
  ...
  // Percentage 1% must switch away from Traditional and rebuild with its own profile.
  ...
  if(!(p.out.box>0)||p.before===p.after)fail(`Percentage 1% did not rebuild ${JSON.stringify(p.out)}`);

  // ATR 140 from the screenshot must switch back and complete a full rebuild.
  ...
  if(!(a.out.box>0)||a.before===a.after)fail(`ATR 140 did not rebuild ${JSON.stringify(a.out)}`);

  // Same method value change must also impact the chart, not only the ACTIVE badge.
  ...
  const result=await page.evaluate(()=>({controller:RWARenkoV15MethodProfiles.version,method:RWARenkoV15.settings.method,box:RWARenkoV15.state.box,bricks:RWARenkoV15.state.data.length,ticks:RWARenkoV15.state.ticks.length,active:document.querySelector('[data-v15-profile].active')?.dataset.v15Profile,lastApplied:document.querySelector('#v15BoxCard')?.dataset.lastApplied,verified:document.querySelector('#v15BoxCard')?.dataset.applyVerified,audit:RWARenkoV15.uniformAudit()}));
  console.log('RENKO V15.3 STANDALONE METHOD PASS',JSON.stringify(result));
