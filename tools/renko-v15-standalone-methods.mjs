...
try{
  ...
  // Delay controller-owned aggregate-trade fetches so the HISTORY/LOADING state is
  // physically pending while every control in the Traditional card is exercised.
  await page.evaluate(()=>{
  ...
  // 100,000 is deliberately far beyond BTC's recent range, so controller hydration
  // is guaranteed to remain pending. This reproduces the user's HISTORY 1/8 state.
  await page.locator('#v15TraditionalBox').fill('100000');
  await page.locator('[data-v15-apply="traditional"]').click({timeout:1200});
  await page.waitForFunction(()=>{const r=document.querySelector('#v15BoxCard'),s=document.querySelector('[data-v15-profile="traditional"] .v15-state')?.textContent||'';return r?.dataset.pendingHistory==='1'&&r?.dataset.hydratingMethod==='traditional'&&/LOADING|APPLYING|QUEUED/.test(s)&&RWARenkoV15.state.data.length>0},null,{timeout:3000});
  const pendingSnapshot=await page.evaluate(()=>({pending:document.querySelector('#v15BoxCard')?.dataset.pendingHistory,method:document.querySelector('#v15BoxCard')?.dataset.hydratingMethod,state:document.querySelector('[data-v15-profile="traditional"] .v15-state')?.textContent,bricks:RWARenkoV15.state.data.length}));
  if(pendingSnapshot.pending!=='1'||pendingSnapshot.method!=='traditional'||pendingSnapshot.bricks<=0)fail(`hydration not really pending ${JSON.stringify(pendingSnapshot)}`);
  ...
  await page.waitForFunction(n=>Number(document.querySelector('#v15BoxCard')?.dataset.historyAborts||0)>n,beforeAbort,{timeout:1200});
  ...
  if(edited.box!=='10'||edited.wicks!==false||edited.confirm!=='1'||edited.aborts<=beforeAbort||edited.pending!=='0'||edited.state!=='CHANGED')fail(`controls did not abort/edit during hydration ${JSON.stringify(edited)}`);
  ...
  // A separate slow background state must not delay a method rebuild.
  ...
  // Start another guaranteed pending history job, then touch ATR. The obsolete fetch
  // must be aborted immediately and ATR must not wait on it.
  await page.locator('#v15Percentage').fill('10');
  await page.locator('[data-v15-apply="percentage"]').click({timeout:1200});
  await page.waitForFunction(()=>document.querySelector('#v15BoxCard')?.dataset.pendingHistory==='1'&&document.querySelector('#v15BoxCard')?.dataset.hydratingMethod==='percentage',null,{timeout:3000});
  ...
  await page.waitForFunction(n=>Number(document.querySelector('#v15BoxCard')?.dataset.historyAborts||0)>n,abort2,{timeout:1200});
  ...
  console.log('RENKO V15.8 EXACT HISTORY-CONTROLS PASS',JSON.stringify(result));
