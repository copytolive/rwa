...
const proof=process.env.RWA_PROOF_DIR||'proof/multichain-v4-pilot';
...
for(const vp of [{name:'desktop',width:1440,height:1000},{name:'mobile',width:390,height:844}]){
  ...
  await page.waitForFunction(()=>document.querySelector('#status')?.textContent?.includes('5-10 USDC'));
...
await browser.close();
const report={contract:'rwa-multichain-real-receipt-pilot-v1',url,ok:true,results};
