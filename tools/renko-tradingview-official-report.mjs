...
async function runViewport(label,viewport){
  ...
  await page.waitForFunction(()=>window.RWARenkoTV?.version==='1.0.0'&&window.RWARenkoATRParity?.version==='2.0.0'&&window.RWARenkoFetchGuard?.version==='2.0.0'&&window.RWARenkoTV?.state?.closedBars?.length>=100&&window.RWARenkoTV?.state?.box>0&&document.querySelector('#tvLoadState')?.textContent?.includes('LIVE'),null,{timeout:60000});
  ...
  const contract=await page.evaluate(()=>{
    ...
    const atrParityScriptPass=window.RWARenkoATRParity?.version==='2.0.0',fetchGuardPass=window.RWARenkoFetchGuard?.version==='2.0.0'&&/single-cors-safe-binance-rest/.test(window.RWARenkoFetchGuard?.rule||'');
...
const report={generatedAt:new Date().toISOString(),url:`${BASE}/renko/`,reference:'TradingView public Renko documentation',scope:'documented-contract parity plus first-load responsiveness; not proprietary byte-for-byte output identity',required:{historicalSource:'chart resolution Close/OHLC',realtime:'projection until source interval closes',methods:['ATR','Traditional','Percentage (LTP)'],atr:'Wilder ATR from ordinary source OHLC; large ATR values must preserve the requested length and load only the history needed',performance:'chart first; duplicate/CORS fallback requests deduped; full market universe loaded on demand; browser remains responsive while large ATR history loads',largeAtrRegression:'ATR 14 -> 2000 must preserve 2000 and materially rebuild; a look-back beyond resident history must move coverage start backward'},status:results.every(x=>x.pass)?'PASS':'FAIL',results};
