...
for(const [label,width,height] of [['root-1672x941',1672,941],['root-mobile-390x844',390,844]]){
  ...
  try{
    ...
    if(width>680){const expectedDock=width>=1600?460:440,expectedLeft=width>=1600?291:(width<=1400?260:286),expectedOrder=width>=1600?239:(width<=1400?220:236);if(Math.abs(Number(audit.topbar?.height||0)-62)>2||!audit.skeletonVisible||Math.abs(Number(audit.skeleton?.width||0)-expectedDock)>2||!/BACKEND LOCKED/i.test(audit.skeletonText))failures.push({label,reason:'desktop root first-paint shell geometry/truth failed',audit});if(Math.abs(Number(audit.left?.width||0)-expectedLeft)>2||Math.abs(Number(audit.order?.width||0)-expectedOrder)>2)failures.push({label,reason:'first-paint desktop grid geometry mismatch',audit});if(width===1672&&Math.abs(Number(audit.main?.width||0)-682)>2)failures.push({label,reason:'1672 target center column is not 682px',audit});if(Math.abs(audit.innerWidth-audit.clientWidth)>1)failures.push({label,reason:'first-paint viewport has scrollbar gutter',audit});}
    else if(!audit.skeletonVisible||Math.abs(Number(audit.skeleton?.width||0)-width)>2||!/BACKEND LOCKED/i.test(audit.skeletonText))failures.push({label,reason:'mobile root first-paint contextual shell failed',audit});
...
await browser.close();const out={ok:failures.length===0,contract:'ecommerce-root-first-paint-0ms-v1.4',base,publicMode,results,failures};await writeFile(`${proof}/browser-result.json`,JSON.stringify(out,null,2));console.log(JSON.stringify(out,null,2));if(!out.ok)process.exit(1);
