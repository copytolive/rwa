...
for(let i=0;i<symbols.length;i++){
  ...
  const row=await page.evaluate(symbol=>{
    ...
    return {symbol,actual:s.symbol,status:s.status,method:T.settings.method,box,tick,input,visible,visiblePositive:parseVisible(visible)>0,total,rendered:Number(s.confirmed?.length||0),sourceBars:Number(s.closedBars?.length||0),audit,interval:T.settings.interval,selector:!!document.querySelector('#intervalSelect'),exactOwn:Object.prototype.hasOwnProperty.call(T.settings,'_exactBox'),workerActive:!!window.RWARenkoATRFixed1s?.workerActive,profile:{...(window.RWARenkoTraditionalControl?.profiles?.[symbol]||{})},source:document.documentElement.dataset.renkoTraditionalBoxSource||'',fixed:document.documentElement.dataset.renkoTraditionalFixed||'',suggestionRevision:document.documentElement.dataset.renkoTraditionalSuggestionRevision||'',precision,required,precisionPass:precision>=required,tickMultiple:Number.isFinite(ratio)&&Math.abs(ratio-Math.round(ratio))<=1e-6};
  },symbol);
  row.prepared=prepared;row.applyMs=Number(applyMs.toFixed(1));
  const tol=Math.max(1e-12,Math.abs(row.box)*1e-10),p=prepared.profile||{},atLegalTickFloor=Math.abs(row.box-row.tick)<=tol,tickLimited=atLegalTickFloor&&row.total<TARGET_MIN;
  row.firstFrame={targetMin:TARGET_MIN,target:TARGET,expectedBricks:Number(p.expectedBricks||0),preparedMaxAtMinTick:Number(p.maxAtMinTick||0),atLegalTickFloor,tickLimited};
  row.firstFramePass=row.total>=TARGET_MIN||tickLimited;
...
const limited=rows.filter(r=>r.firstFrame.tickLimited),attainable=rows.filter(r=>!r.firstFrame.tickLimited);
const report={schema:'renko-traditional-50pair-browser-v5',generatedAt:new Date().toISOString(),base,mode,viewport,httpStatus:response?.status(),count:rows.length,passCount:rows.filter(r=>r.pass).length,status:response?.ok()&&rows.length===50&&rows.every(r=>r.pass)&&!errors.length?'PASS':'FAIL',target:{minimumConfirmed:TARGET_MIN,idealConfirmed:TARGET,maxApplyMs:MAX_APPLY_MS,attainableCount:attainable.length,limitedAtLegalMinimumTick:limited.length},contract:'Traditional uses a positive minimum-tick-safe absolute box. Its first unsaved per-symbol suggestion is calibrated locally toward about 60 confirmed bricks. The actual applied first frame must contain at least 46 confirmed bricks unless the applied box has already reached the exchange minimum tick; at that legal floor there is no smaller valid Traditional box, so the real positive frame is accepted rather than fabricating an illegal sub-tick box. Any pair whose applied box remains above one tick must still meet the 46-brick floor. User Apply owns the saved per-symbol box, ATR _exactBox never leaks into Traditional, the XAUT ATR worker stays dormant on the 50 Binance launch pairs, fixed 1s remains permanent, and tiny-price boxes remain visibly positive.',rows,errors,consoleErrors};
fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));
console.log('RENKO_TRADITIONAL_50PAIR '+JSON.stringify({status:report.status,mode,count:report.count,passCount:report.passCount,attainable:attainable.length,limitedAtLegalTick:limited.length,minAttainable:attainable.length?Math.min(...attainable.map(r=>r.total)):null,maxApplyMs:rows.length?Math.max(...rows.map(r=>r.applyMs)):null,limitedPairs:limited.map(r=>({symbol:r.symbol,box:r.box,tick:r.tick,total:r.total})),failed:rows.filter(r=>!r.pass).map(r=>({symbol:r.symbol,box:r.box,tick:r.tick,total:r.total,sourceBars:r.sourceBars,visible:r.visible,source:r.source,expected:r.firstFrame.expectedBricks,preparedMaxAtMinTick:r.firstFrame.preparedMaxAtMinTick,atLegalTickFloor:r.firstFrame.atLegalTickFloor,tickLimited:r.firstFrame.tickLimited,firstFramePass:r.firstFramePass,applyMs:r.applyMs,latencyPass:r.latencyPass,exact:r.exactOwn,worker:r.workerActive})),errors}));
await browser.close();
