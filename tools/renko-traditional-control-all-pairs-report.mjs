...
const report={schema:'renko-traditional-50pair-browser-v3',generatedAt:new Date().toISOString(),base,mode,viewport,httpStatus:response?.status(),count:rows.length,passCount:rows.filter(r=>r.pass).length,status:response?.ok()&&rows.length===50&&rows.every(r=>r.pass)&&!errors.length?'PASS':'FAIL',target:{minimumConfirmed:TARGET_MIN,idealConfirmed:TARGET,maxApplyMs:MAX_APPLY_MS,attainableCount:attainable.length,limitedByCurrentTickGeometry:limited.length},contract:'Traditional uses a positive minimum-tick-safe absolute box. Its first unsaved per-symbol suggestion is calibrated locally against already-loaded fixed-1s history toward about 60 confirmed bricks. At Apply-time the exact same current source window is re-audited at one minimum tick: at least 46 confirmed bricks are required whenever that geometry can produce 46; if one tick itself currently produces fewer than 46, one tick is the mathematically smallest legal box and sub-tick/fake bricks are forbidden. User Apply owns the saved per-symbol box, ATR _exactBox never leaks into Traditional, the XAUT ATR worker stays dormant on the 50 Binance launch pairs, fixed 1s remains permanent, and tiny-price boxes remain visibly positive.',rows,errors,consoleErrors};
fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));
console.log('RENKO_TRADITIONAL_50PAIR '+JSON.stringify({status:report.status,mode,count:report.count,passCount:report.passCount,attainable:attainable.length,limitedByCurrentTick:limited.length,minAttainable:attainable.length?Math.min(...attainable.map(r=>r.total)):null,maxApplyMs:rows.length?Math.max(...rows.map(r=>r.applyMs)):null,failed:rows.filter(r=>!r.pass).map(r=>({symbol:r.symbol,box:r.box,tick:r.tick,total:r.total,visible:r.visible,source:r.source,expected:r.firstFrame.expectedBricks,preparedMaxAtMinTick:r.firstFrame.preparedMaxAtMinTick,currentMinTickCapacity:r.firstFrame.currentMinTickCapacity,currentAttainable:r.firstFrame.currentAttainable,firstFramePass:r.firstFramePass,applyMs:r.applyMs,latencyPass:r.latencyPass,exact:r.exactOwn,worker:r.workerActive})),errors}));
await browser.close();
