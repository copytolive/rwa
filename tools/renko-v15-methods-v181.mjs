...
async function typeActiveValue(selector,value,method){const result=await page.evaluate(({selector,value,method})=>new Promise((resolve,reject)=>{const el=document.querySelector(selector);if(!el)return reject(new Error('input missing'));const start=performance.now(),tm=setTimeout(()=>reject(new Error('auto apply timeout')),3000);const on=e=>{if(e.detail?.method!==method)return;clearTimeout(tm);window.removeEventListener('renko:v15-method-applied',on);resolve({wallMs:performance.now()-start,...e.detail})};window.addEventListener('renko:v15-method-applied',on);el.focus();el.value=value;el.dispatchEvent(new Event('input',{bubbles:true}));}),{selector,value,method});return result}
try{
  await page.goto(`${BASE}renko/?symbol=SOL&qa=183`,{waitUntil:'domcontentloaded',timeout:45000});
  await page.waitForFunction(()=>window.RWARenkoV15?.version==='15.1.0'&&window.RWARenkoV15MethodProfiles?.version==='1.8.3'&&document.documentElement.dataset.renkoMethodBootstrap==='183',null,{timeout:90000});
  ...
  if(boot.numericAutoApply!=='75')fail(`numeric auto apply debounce missing ${JSON.stringify(boot)}`);
  ...
  if(!hotEdit.hotReplay)fail(`Traditional keyboard edit did not use hot replay ${JSON.stringify(hotEdit)}`);
  if(hotEdit.wallMs>800||hotEdit.localMs>500)fail(`Traditional hot replay too slow ${JSON.stringify(hotEdit)}`);
  ...
  if(!hotBack.hotReplay||hotBack.wallMs>800)fail(`Traditional second hot replay too slow ${JSON.stringify(hotBack)}`);
  ...
  if(!pctHot.hotReplay||pctHot.wallMs>800||pctHot.localMs>500)fail(`Percentage keyboard hot replay too slow ${JSON.stringify(pctHot)}`);
  ...
  console.log('RENKO V15.15 INSTANT HOT-REPLAY + 60-BRICK + WICKS PASS',JSON.stringify({boot,atr,atrOff,atrOn,traditional,hotEdit,hotSnap,hotBack,tradReady,tradOff,tradOn,percentage,pctHot,pctHotSnap,pctOff,pctOn,manual100}));
