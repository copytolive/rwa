...
const BASE=process.env.RWA_BASE_URL||'http://127.0.0.1:4173/rwa/';

async function openLazyFundamentals(page){
  await page.goto(BASE,{waitUntil:'domcontentloaded',timeout:30000});
  await page.waitForFunction(()=>window.RWASuperApp?.version==='5.0.0',{timeout:20000});
  const before=await page.evaluate(()=>({
    fundScript:[...document.scripts].some(x=>(x.getAttribute('src')||'').includes('rwa-fundamentals.js')),
    fundStyle:[...document.querySelectorAll('link[rel="stylesheet"]')].some(x=>(x.getAttribute('href')||'').includes('rwa-fundamentals.css')),
    path:location.pathname
  }));
  assert.equal(before.fundScript,false,'fundamentals JS must remain lazy on first paint');
  assert.equal(before.fundStyle,false,'fundamentals CSS must remain lazy on first paint');
  await page.evaluate(()=>window.RWASuperApp.navigate('asset/BTC'));
  await page.waitForSelector('[data-asset-tab="fundamentals"]',{state:'visible',timeout:10000});
  await page.click('[data-asset-tab="fundamentals"]');
  await page.waitForSelector('[data-v5-action="legacy-fundamentals"]',{state:'visible',timeout:10000});
  await page.click('[data-v5-action="legacy-fundamentals"]');
  await page.waitForFunction(()=>window.RWAFundamentals?.version==='1.0.0'&&document.getElementById('rwaFundamentals')?.classList.contains('open'),{timeout:15000});
  const after=await page.evaluate(()=>({
    fundScript:[...document.scripts].some(x=>(x.getAttribute('src')||'').includes('rwa-fundamentals.js')),
    fundStyle:[...document.querySelectorAll('link[rel="stylesheet"]')].some(x=>(x.getAttribute('href')||'').includes('rwa-fundamentals.css')),
    path:location.pathname
  }));
  assert.equal(after.fundScript,true,'lazy fundamentals JS did not load');
  assert.equal(after.fundStyle,true,'lazy fundamentals CSS did not load');
  assert.equal(after.path,before.path,'lazy fundamentals must remain on canonical root');
  return after.path;
}
...
async function desktop(browser){
  ...
  const before=await openLazyFundamentals(page);
  ...
  await page.evaluate(()=>window.RWAFundamentals.openAsset('BTC'));
  ...
  assert.equal(new URL(page.url()).pathname,before,'close must stay on canonical root');
  await ctx.close();
  return{tabs:base.tabs,draftStatus:draft.status,canonicalPath:before,lazy:true};
}

async function mobile(browser){
  const ctx=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true});
  const page=await ctx.newPage();
  const path=await openLazyFundamentals(page);
  ...
  assert.equal(await page.locator('#rwaFundamentals').getAttribute('aria-hidden'),'true');
  await ctx.close();
  return{width:box.width,path,lazy:true};
}

const browser=await chromium.launch({headless:true});
try{
  const d=await desktop(browser),m=await mobile(browser);
  console.log(JSON.stringify({ok:true,contract:'rwa-lazy-income-fundamentals-v2',desktop:d,mobile:m},null,2));
