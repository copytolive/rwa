...
async function run(label,width,height){
  ...
  try{
    ...
    const before=await rect(page,'.layout'),alreadyOpen=await page.locator('#rwaShopScreen.open').isVisible().catch(()=>false),launcher=page.locator('#rwaSeablueprintCommerceLaunch,[data-rwa-seablueprint-commerce="1"]').filter({visible:true}).first();
    if(!alreadyOpen){if(!(await launcher.isVisible().catch(()=>false)))throw new Error('Ecommerce neither open nor launchable');await launcher.click()}
    await page.waitForFunction(()=>document.querySelector('#rwaShopScreen')?.classList.contains('open'),{timeout:12000});await stabilize(page);
