...
async function run(label,width,height){
  ...
  try{
    ...
    await page.waitForFunction(()=>window.RWASeablueprintCommerceBridge?.version==='1.5.0'&&window.RWATargetDashboardV2?.version==='2.2.0'&&window.RWAEcommerceTargetController?.version==='2.3.0'&&window.RWATargetTopbarCleanup?.version==='1.6.0'&&window.RWAEcommerceProductionVisualV1?.version==='1.6.0',{timeout:publicMode?30000:20000});
    ...
    const a=await page.evaluate(()=>({bridge:window.RWASeablueprintCommerceBridge.audit(),target:window.RWAEcommerceTargetController.audit(),visual:window.RWAEcommerceProductionVisualV1.audit(),topbar:window.RWATargetTopbarCleanup.audit(),path:location.pathname,hash:location.hash,heading:document.querySelector('.rwa-ecom-head-title')?.innerText||'',tabs:[...document.querySelectorAll('[data-ecom-target-tab]')].map(x=>x.textContent.trim()),store:document.querySelector('.rwa-ecom-store h3')?.textContent||'',source:[...document.querySelectorAll('[data-seablueprint-source]')].map(x=>x.textContent||'').join(' '),transitions:[...document.querySelectorAll('#rwaCommerceDock,[data-ecom-target-tab],[data-ecom-action]')].slice(0,40).map(x=>({t:getComputedStyle(x).transitionDuration,a:getComputedStyle(x).animationDuration}))}));
...
for(const s of sizes)await run(...s);await browser.close();const out={ok:failures.length===0,contract:'ecommerce-target-parity-v3.1',base,publicMode,results,failures};await writeFile(`${proof}/browser-result.json`,JSON.stringify(out,null,2));console.log(JSON.stringify(out,null,2));if(!out.ok)process.exit(1);
