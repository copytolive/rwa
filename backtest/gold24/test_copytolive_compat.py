...
def frame(close, high=None, low=None):
    ...
    return pd.DataFrame(
        {"open":close,"high":np.asarray(high,float),"low":np.asarray(low,float),"close":close},
        index=idx,
    )


def padded(values, fill=100.0, n=12):
    out=[fill]*n
...
class CopyToLiveCompatTests(unittest.TestCase):
    def test_fixed_risk_and_tp_ratio(self):
        df=frame(padded([100,100,100]),high=padded([100,103,103]),low=padded([100,99.5,99.5]))
        sig=np.zeros(len(df),dtype=np.int8); sig[0]=1
    ...
    def test_stop_first_when_same_bar_hits_both(self):
        df=frame(padded([100,100]),high=padded([100,103]),low=padded([100,98]))
        sig=np.zeros(len(df),dtype=np.int8); sig[0]=1
        cfg=CopyToLiveExecutionConfig(sl_pct=0.01,tp_ratio=2.0,fee=0.0)
        r=run_copytolive_backtest(df,sig,cfg)
        self.assertEqual(len(r["trades"]),1)
    ...
    def test_no_same_bar_exit(self):
        df=frame(padded([100,100]),high=padded([103,100]),low=padded([98,100]))
        sig=np.zeros(len(df),dtype=np.int8); sig[0]=1
        cfg=CopyToLiveExecutionConfig(sl_pct=0.01,tp_ratio=2.0,fee=0.0)
        r=run_copytolive_backtest(df,sig,cfg)
    ...
    def test_fee_matches_production_contract(self):
        df=frame(padded([100,100]),high=padded([100,103]),low=padded([100,99.5]))
        sig=np.zeros(len(df),dtype=np.int8); sig[0]=1
    ...
    def test_short_side(self):
        df=frame(padded([100,100]),high=padded([100,100.5]),low=padded([100,97]))
        sig=np.zeros(len(df),dtype=np.int8); sig[0]=-1
        cfg=CopyToLiveExecutionConfig(sl_pct=0.01,tp_ratio=2.0,fee=0.0)
        r=run_copytolive_backtest(df,sig,cfg)
    ...
    def test_single_position_ignores_new_signal_until_closed(self):
        df=frame(padded([100,100,100]),high=padded([100,100.5,103]),low=padded([100,99.5,99]))
        sig=np.zeros(len(df),dtype=np.int8); sig[0]=1; sig[1]=-1; sig[2]=-1
        cfg=CopyToLiveExecutionConfig(sl_pct=0.01,tp_ratio=2.0,fee=0.0)
        r=run_copytolive_backtest(df,sig,cfg)
        self.assertEqual(len(r["trades"]),1)
    ...
    def test_metrics(self):
        trades=[{"profit":100.0},{"profit":100.0},{"profit":-50.0},{"profit":-50.0}]
    ...
    def test_digest_is_stable(self):
        df=frame(padded([100,100,100]),high=padded([100,103,103]),low=padded([100,99,99]))
        sig=np.zeros(len(df),dtype=np.int8); sig[0]=1
        cfg=CopyToLiveExecutionConfig(sl_pct=0.01,tp_ratio=2.0,fee=0.0)
