...
def main() -> int:
    ...
    ap.add_argument('--candidate-count',type=int,default=20000)
    ap.add_argument('--workers',type=int,default=8)
    ap.add_argument('--seed',type=int,default=2026090411)
    a=ap.parse_args()
    state=Path(a.state_dir)
    dataset=state/'XAUUSD_MT5_NATIVE_H4_PRIMARY.csv'
    receipt=state/'gate_a_h4_receipt.json'
    d,audit=audit_dataset(dataset,receipt,'H4')
    rec=json.loads(receipt.read_text())

    families=sorted(FAMILIES); rng=random.Random(a.seed)
    generated=[]; seen=set(); family_counts=Counter(); attempts=0
    while len(generated)<a.candidate_count and attempts<a.candidate_count*50:
        attempts+=1
        family=families[len(generated)%len(families)] if rng.random()<0.70 else rng.choice(families)
        c=base._candidate(rng,family)
    ...
    exact=[]; bars={}
    for r in pre:
        row,bp,tp=impl._exact_row(d,audit,r['candidate'],'BROKER_NATIVE_H4')
    ...
    payload={
        'schema':'gold10b-broker-native-h4-hardpass-discovery-v1','status':'PASS',
        'dataset':{
            'provider':rec.get('primary_provider'),'broker_server':rec.get('primary_broker_server'),'symbol':rec.get('primary_symbol'),
            'logical_symbol':'GOLD','timeframe':'H4','rows':audit['rows'],'start_utc':audit['start_utc'],'end_utc':audit['end_utc'],
            'dataset_sha256':audit['dataset_sha256'],'cost_model':'core.py COST_FLOOR_RT=0.0032','quantity_gold_units':100.0,
            'starting_equity_usd':10000.0,'construction':'DIRECT_BROKER_NATIVE_H4_NO_RESAMPLING','crosscheck_provider':audit['crosscheck_provider']},
        ...
        'candidate_evaluated_unique':len(generated),'generated_family_counts':dict(sorted(family_counts.items())),
        'primitive_diagnostics':dict(sorted(primitive.items())),'primitive_survivors':len(pre),'full_pre_corr_hardpass':len(exact),
        'native_h4_hardpass_kept':len(selected),'native_h4_corr_rejected':len(rejected),'hard_pass_rows':selected,
        'top50_primitive_near_miss':near,'global_cross_timeframe_corr_status':'PENDING_IF_H4_HARDPASS_EXISTS','portfolio_readiness':'NOT_READY',
        'note':'Direct broker-server H4 bars only; no lower-timeframe aggregation/resampling. Global logical-GOLD correlation against D1 remains mandatory.'}
    Path(a.out).parent.mkdir(parents=True,exist_ok=True); Path(a.out).write_text(json.dumps(payload,indent=2)+'\n')
    print(json.dumps({k:payload[k] for k in ['status','candidate_evaluated_unique','primitive_diagnostics','primitive_survivors','full_pre_corr_hardpass','native_h4_hardpass_kept']},indent=2))
    return 0
