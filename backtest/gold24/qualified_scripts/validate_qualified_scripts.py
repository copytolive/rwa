...
MULTI=['multi_rank01_donchian_f3_s89_off4_75_exp3','multi_rank02_candle_engulfing_f20_s144_off2_exp5','multi_rank03_candle_engulfing_f26_s144_off2_exp8','multi_rank04_candle_engulfing_f89_s144_off4_5_exp5','multi_rank05_candle_engulfing_f55_s144_off2_exp6','multi_rank06_candle_engulfing_f34_s144_off1_exp8']
ALL=STRICT+MULTI
...
def check_wrapper(stem,m):
    text=(HERE/f'{stem}.mq5').read_text(encoding='utf-8');pref='MM' if stem.startswith('multi_') else 'QM';c=m.CANDIDATE
...
def main():
    ...
    payload={'status':'PASS','strict_count':len(STRICT),'multi_count':len(MULTI),'total_method_pairs':len(ALL),'checks':results,'contract':{'python':'exact canonical core.py qty=100 parity','mt5':'native MetaEditor clean compile plus custom-symbol Strategy Tester operational certification; wrapper config must match Python candidate','broker_pnl_note':'broker-specific spread/commission/swap/tick sequence can differ from canonical stressed cost model'}}
