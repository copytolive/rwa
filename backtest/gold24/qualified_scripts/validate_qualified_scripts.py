...
MULTI=[
'multi_rank01_donchian_f3_s89_off4_75_exp3',
'multi_chart_pattern_f3_s34_off1_25_exp8',
'multi_bollinger_reversion_v2_f3_s14_off1_25_exp4',
'multi_candle_engulfing_f21_s144_off5_exp8',
'multi_candle_engulfing_f34_s144_off1_75_exp10',
'multi_volume_f50_s100_off1_exp5',
'multi_bollinger_reversion_v2_f8_s13_off0_5_exp5',
'multi_chart_pattern_f3_s26_off4_exp3',
'multi_volume_f13_s144_off4_75_exp8',
'multi_volatility_regime_f5_s13_off2_25_exp10',
'multi_chart_pattern_f5_s20_off3_exp4',
'multi_volatility_regime_f5_s7_off0_5_exp2',
'multi_rank08_candle_engulfing_f26_s144_off1_75_exp8',
'multi_adaptive_trend_f20_s144_off5_exp5',
'multi_rank07_candle_engulfing_f55_s100_off3_25_exp8',
]
ALL=STRICT+MULTI
...
def check_wrapper(stem,m):
    ...
    family_codes={'DONCHIAN':'1','CANDLE_ENGULFING':'2','CHART_PATTERN':'3','ADAPTIVE_TREND':'4','BOLLINGER_REVERSION_V2':'5','VOLUME':'6','VOLATILITY_REGIME':'7'}
    if c.family not in family_codes:
        return [f'unsupported selected MT5 family mapping: {c.family}']
    expected={
        f'{pref}_FAMILY_CODE':family_codes[c.family],
        f'{pref}_FAST':str(c.fast),f'{pref}_SLOW':str(c.slow),
        f'{pref}_P1':str(float(c.p1)),f'{pref}_P2':str(float(c.p2)),f'{pref}_P3':str(float(c.p3)),
        f'{pref}_SL_USD':str(float(c.sl)),f'{pref}_TP_USD':str(float(c.tp)),
        f'{pref}_OFFSET_USD':str(float(c.offset)),f'{pref}_EXPIRY_BARS':str(c.expiry),
...
def main():
    ...
    payload={'status':'PASS','strict_count':len(STRICT),'multi_count':len(MULTI),'total_method_pairs':len(ALL),'checks':results,'contract':{'python':'exact canonical core.py qty=100 parity','mt5':'native MetaEditor clean compile plus custom-symbol Strategy Tester operational certification; wrapper config/hash must match Python candidate','family_parameter_note':'all selected wrappers carry exact p1/p2/p3, entry method and config-hash macros; DONCHIAN/CANDLE keep identity-only p values while CHART_PATTERN, ADAPTIVE_TREND, BOLLINGER_REVERSION_V2, VOLUME and VOLATILITY_REGIME consume their family parameters','broker_pnl_note':'broker-specific spread/commission/swap/tick sequence can differ from canonical stressed cost model'}}
