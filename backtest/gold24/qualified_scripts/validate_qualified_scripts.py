...
if not MANIFEST.exists():
    raise RuntimeError('generated_multi_manifest.json missing: run generate_from_runtime.py first')
_manifest=json.loads(MANIFEST.read_text())
MULTI=[str(x['stem']) for x in _manifest.get('rows',[])]
if int(_manifest.get('count',0))!=len(MULTI) or not MULTI:
    raise RuntimeError('generated multi manifest invalid/empty')
ALL=STRICT+MULTI
...
def check_wrapper(stem,m):
    ...
    if c.family in {'CHART_PATTERN','ADAPTIVE_TREND','BOLLINGER_REVERSION_V2','VOLUME','VOLATILITY_REGIME'}:
    ...
    if c.family=='VOLATILITY_REGIME' and '#define MM_VOLREG_SPECIAL' not in text:errors.append('VOLATILITY_REGIME wrapper missing MM_VOLREG_SPECIAL')
...
def main():
    ...
    for stem,m in modules.items():
        ...
        if a.state_dir:
            ...
            record['canonical_python_parity']='PASS' if not errs else 'FAIL';record['dataset_rows']=audit.get('rows');record['execution_hash']=res.get('execution_hash')
        if errs:raise SystemExit(json.dumps(record,indent=2))
        results.append(record)
    payload={'status':'PASS','strict_count':len(STRICT),'multi_count':len(MULTI),'total_method_pairs':len(ALL),'checks':results,'contract':{'python':'exact canonical core.py qty=100 parity','mt5':'native MetaEditor clean compile plus custom-symbol Strategy Tester operational certification; wrapper config/hash must match Python candidate','family_parameter_note':'current selected families include DONCHIAN, CANDLE_ENGULFING, CHART_PATTERN, ADAPTIVE_TREND, BOLLINGER_REVERSION_V2, VOLUME and VOLATILITY_REGIME; parameter-bearing families carry exact p1/p2/p3 macros','broker_pnl_note':'broker-specific spread/commission/swap/tick sequence can differ from canonical stressed cost model'}}
