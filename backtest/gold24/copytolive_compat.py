...
"""CopyToLive-compatible execution model for GOLD24 research.

This module intentionally mirrors the *execution semantics* used by the
CopyToLive production research pipeline without copying private strategy
implementations into this public repository.

Compatibility contract:
- initial deposit: USD 10,000
- risk budget: USD 200 per trade
- stop distance: entry_price * sl_pct
- target distance: stop_distance * tp_ratio
- position size: risk_usd / stop_distance
- one open position at a time
- signal enters at the current close; exits are evaluated from the next bar
- stop is evaluated before target on the same bar (conservative/worst case)
- stressed fee: 0.0016 * entry_price * quantity, charged on close
- 70/30 walk-forward helper with 100/50/300 train/test/full trade floors

The legacy GOLD24 fixed-dollar SL/TP engine in core.py is kept unchanged for
historical reproducibility. New CopyToLive-compatible discovery should use
this module.
"""
...
def _time_index(df: pd.DataFrame) -> pd.Index:
    ...
    for name in ("Date", "date", "time", "timestamp", "datetime"):
        if name in df.columns:
            return pd.DatetimeIndex(pd.to_datetime(df[name], utc=True, errors="coerce"))
...
def _as_signals(signals: Iterable[int], n: int) -> np.ndarray:
    ...
    out = np.sign(out).astype(np.int8)
    return out
...
def run_copytolive_backtest(
    ...
    """Run the CopyToLive-compatible single-position execution model."""
    config.validate()
    if len(df) < 2:
    ...
    for i in range(len(df)):
        if pos is None:
            ...
            # Production semantics: a newly opened position is not evaluated
            # for SL/TP on the same bar.
            continue
        ...
        # Conservative precedence: SL first if the bar contains both levels.
        if side == 1:
            if low[i] <= entry - sl:
                exit_price = entry - sl
                exit_type = "SL"
    ...
    canonical = [
        {
            ...
            "quantity": round(float(t["quantity"]), 10),
...
def adapt_core_candidate_signals(df: pd.DataFrame, candidate) -> np.ndarray:
    """Reuse GOLD24 public signal families while switching only execution semantics."""
