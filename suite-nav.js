...
function addCss(href,key){if(document.querySelector(`link[data-rwa-${key}]`))return;const l=document.createElement('link');l.rel='stylesheet';l.href=href;l.dataset[`rwa${key.replace(/(^|-)(\w)/g,(_,a,b)=>b.toUpperCase())}`]='1';document.head.appendChild(l)}
addCss('ops.css?v=1','ops-css');
addCss('human-operability-v1.css?v=1','human-operability');
...
function returnToMarkets(){hideSuite();if(window.RWASuperApp?.navigate)window.RWASuperApp.navigate('markets');else location.hash='markets';requestAnimationFrame(()=>document.getElementById('terminal')?.scrollIntoView({behavior:'auto',block:'start'}))}
function installGlobalRwaFactory(){const panel=document.querySelector('[data-suite-panel="rwa"]');if(!panel||panel.querySelector('[data-global-rwa-factory]'))return false;const legacy=panel.querySelector('.suite-grid');const wrap=document.createElement('section');wrap.className='suite-card span-12';wrap.dataset.globalRwaFactory='1';wrap.setAttribute('aria-label','Create My RWA workspace');wrap.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;border-bottom:1px solid #263142"><button type="button" class="rwa-human-factory-back" data-rwa-factory-back aria-label="Back to markets">←</button><div style="min-width:0;flex:1"><small style="letter-spacing:.12em;color:#8ab4ff;font-weight:800">P21 · GLOBAL RWA FACTORY · 8 ENGINES</small><h3 style="margin:3px 0 0">Create My RWA</h3></div><span style="font-size:11px;color:#60d394">REGISTER → VERIFY → VALUE → FINANCE → TOKENIZE → TRADE</span></div><iframe title="Global RWA Factory" src="rwa-8-engines/?embed=1" loading="lazy" style="display:block;width:100%;border:0;background:#080b10" allow="clipboard-write"></iframe>';if(legacy)panel.insertBefore(wrap,legacy);else panel.appendChild(wrap);return true}
function suite(tab){if(!window.RWASuite)return;if(tab==='rwa')installGlobalRwaFactory();window.RWASuite.open(tab);if(tab==='rwa')setTimeout(installGlobalRwaFactory,80);const s=document.getElementById('suite');if(innerWidth>899&&s)setTimeout(()=>s.scrollIntoView({behavior:'auto',block:'start'}),20)}
document.addEventListener('click',e=>{if(e.target.closest('[data-rwa-factory-back]')){e.preventDefault();returnToMarkets();return}if(e.target.closest('[data-suite-close]')){e.preventDefault();returnToMarkets()}});
for(const b of document.querySelectorAll('.topnav button')){const t=b.textContent.trim().toLowerCase();if(t==='markets')b.onclick=returnToMarkets;if(t==='intelligence')b.onclick=()=>suite('intel');if(t==='assets')b.onclick=()=>suite('rwa');if(t==='research')b.onclick=()=>suite('feed');if(t==='company')b.onclick=()=>suite('profile')}
