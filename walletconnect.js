...
async function init(projectId){
 ...
 booting=(async()=>{
  ...
  app.subscribeProvider?.(s=>{if(s?.provider){window.RWAProvider=s.provider}});app.subscribeProviders?.(s=>{if(s?.eip155)window.RWAProvider=s.eip155});sync();return app
...
async function connect(projectId=localStorage.getItem(KEY)||''){if(!projectId)throw Error('Reown Project ID is required');localStorage.setItem(KEY,projectId);const a=await init(projectId);await a.open({view:'Connect',namespace:'eip155'});const p=await waitProvider(a);window.RWAProvider=p;const accounts=await p.request({method:'eth_accounts'});const address=String(accounts?.[0]||'').toLowerCase();if(!/^0x[a-f0-9]{40}$/.test(address))throw Error('Wallet address unavailable');const ts=Date.now(),message=`RWA Network wallet login\nWallet: ${address}\nTime: ${ts}`;const {BrowserProvider}=await import('https://esm.sh/ethers@6.15.0');const signer=await new BrowserProvider(p).getSigner();const signature=await signer.signMessage(message);localStorage.setItem(SESSION,JSON.stringify({wallet:address,message,signature,ts,via:'walletconnect'}));sessionStorage.setItem('rwa_wallet_just_logged_in','1');toast('WalletConnect login verified');location.reload()}
