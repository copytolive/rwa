...
await page.waitForFunction(()=>document.querySelector('#chartHost canvas')&&window.RWARenkoTV?.state?.symbol==='XAUUSD'&&window.RWARenkoGoldTotalHistory&&window.RWARenkoGoldWheelPanLock?.revision==='2.1.2-microtask-prepaint-scale-lock'&&window.RWARenkoGoldExplicitZoom?.revision==='1.2.2-deep-threshold-demand'&&window.RWARenkoGoldManualViewport?.revision==='4.5.0-range-event-scale'&&document.documentElement.dataset.renkoGoldRecent==='true',{timeout:120000});
...
const driftSamples=samples.filter(s=>Math.abs(Number(s.spacing)-Number(baseline))>1e-9);
const final=samples.at(-1);const maxPaint=Number(final?.pan?.stats?.maxPaintBarSpacingDelta||0);
const report={token:'RENKO_GOLD_FIRST_MIXED_NO_VISIBLE_SCALE_DRIFT_PASS',base,baseline,driftSamples,maxPaintBarSpacingDelta:maxPaint,samples};
await fs.writeFile(path.join(out,'trace.json'),JSON.stringify(report,null,2));
if(driftSamples.length)throw new Error(`first mixed-wheel sampled scale drift ${JSON.stringify(driftSamples.map(x=>({dt:x.dt,spacing:x.spacing})))}`);
if(maxPaint>1e-9)throw new Error(`first mixed-wheel pre-paint residual drift ${maxPaint}`);
