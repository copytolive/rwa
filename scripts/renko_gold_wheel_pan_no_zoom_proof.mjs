...
await page.waitForFunction(()=>document.querySelector('#chartHost canvas')&&window.RWARenkoTV?.state?.symbol==='XAUUSD'&&window.RWARenkoGoldTotalHistory&&window.RWARenkoGoldWheelPanLock?.version==='1.2.0-scroll-position-spacing-lock'&&document.documentElement.dataset.renkoGoldRecent==='true',{timeout:120000});
...
await page.click('#tvLive');await page.waitForTimeout(650);
...
for(const [dx,dy] of candidates){await page.click('#tvLive');await page.waitForTimeout(650);await page.mouse.move(box.x+box.width*.55,box.y+box.height*.45);const a=await getState();await page.mouse.wheel(dx,dy);await page.waitForTimeout(650);const b=await getState();assertScale(a,b,`calibration ${dx},${dy}`);const move=Number(b.logical?.from)-Number(a.logical?.from);calibration.push({dx,dy,move,beforeSpacing:a.barSpacing,afterSpacing:b.barSpacing,beforeWidth:a.width,afterWidth:b.width,beforeScroll:a.scrollPosition,afterScroll:b.scrollPosition});if(Number.isFinite(move)&&move<best){best=move;vector=[dx,dy]}}
if(!vector)throw new Error(`mixed/real wheel could not pan older: ${JSON.stringify(calibration)}`);
await page.click('#tvLive');await page.waitForTimeout(650);await page.mouse.move(box.x+box.width*.55,box.y+box.height*.45);
...
for(let cycle=1;cycle<=3;cycle++){
  ...
  for(let n=0;n<1800;n++){
    ...
    if(n%12===0){await page.waitForTimeout(45);const mid=await getState();assertScale(baseline,mid,`cycle ${cycle} wheel ${n}`)}else await page.waitForTimeout(18);
  }
  if(!triggered){await page.waitForTimeout(650);triggered=(await getState()).prepends>=target}
  if(!triggered)throw new Error(`back-scroll failed to prepend cycle ${cycle}`);
  await page.waitForFunction(n=>!window.RWARenkoGoldTotalHistory.busy&&window.RWARenkoGoldTotalHistory.stats.prepends>=n,target,{timeout:180000});await page.waitForTimeout(650);
  const after=await getState();assertScale(baseline,after,`cycle ${cycle} settled`);if(after.method!=='traditional'||delta(after.box,10)>1e-9)throw new Error(`Traditional 10 changed during scroll cycle ${cycle}`);if(after.panOnly!=='true'||after.wheelVersion!=='1.2.0-scroll-position-spacing-lock')throw new Error('wheel pan-only guard not active');rows.push({cycle,beforeSpacing:baseline.barSpacing,afterSpacing:after.barSpacing,beforeWidth:baseline.width,afterWidth:after.width,beforeScroll:baseline.scrollPosition,afterScroll:after.scrollPosition,prepends:after.prepends,corrections:after.wheelStats.corrections,maxBarSpacingDelta:after.wheelStats.maxBarSpacingDelta});
}
await page.screenshot({path:path.join(outDir,'AFTER.png'),fullPage:true});
const final=await getState();if(pageErrors.length)throw new Error(`page errors ${JSON.stringify(pageErrors)}`);const report={token:'RENKO_GOLD_WHEEL_PAN_NO_ZOOM_PASS',sha:deployedSha||expectedSha||null,baseUrl,traditionalBox:10,vector,calibration,rows,final};await fs.writeFile(path.join(outDir,'report.json'),JSON.stringify(report,null,2));console.log(report.token,JSON.stringify(report));await browser.close();
