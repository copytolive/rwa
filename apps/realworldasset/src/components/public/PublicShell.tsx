...
export function RoutePlaceholder({title,path}:{title:string;path:string}){
  ...
  return <PublicShell authenticated><main className="rwa-workspace"><header className="rwa-workspace__head"><div><span className="rwa-workspace__eyebrow">RWA.MS · {section} · DEMO</span><h1>{title}</h1><p>Production-style interface preview with deterministic demo data. Read-only navigation is active; backend writes and financial execution remain fail-closed until live services and evidence gates are connected.</p></div><div className="rwa-workspace__actions"><Link className="rwa-link-button rwa-link-button--primary" href="/discover">Discover Assets</Link><Link className="rwa-link-button" href="/home">Dashboard</Link></div></header><section className="rwa-workspace__kpis"><article><small>Demo Total Value</small><strong>$1.28B</strong><em>Sample dataset</em></article><article><small>Demo 24H Volume</small><strong>$64.7M</strong><em>Sample dataset</em></article><article><small>Sample Asset Profiles</small><strong>1,248</strong><em>Not verified-asset count</em></article><article><small>Environment</small><strong>Demo</strong><em>Backend offline · safe mode</em></article></section><section className="rwa-workspace__panel"><nav className="rwa-workspace__tabs" aria-label="Workspace views">{["Overview","Activity","Analytics","Details"].map(x=><button type="button" key={x} data-active={tab===x?"true":undefined} onClick={()=>setTab(x)}>{x}</button>)}</nav><div className="rwa-workspace__chart"><div><small>{tab} · 30D · DEMO</small><strong>$1.28B <em>Sample</em></strong></div><svg viewBox="0 0 900 220" preserveAspectRatio="none" aria-label={`${title} demo trend`}><defs><linearGradient id="workspace-fill" x1="0" y1="0" x2="0" y2="1"><stop stopColor="#2978ff" stopOpacity=".34"/><stop offset="1" stopColor="#2978ff" stopOpacity="0"/></linearGradient></defs><path d="M0 190 L65 176 120 184 185 145 240 151 300 118 365 128 430 86 495 103 560 70 625 82 690 48 760 62 825 31 900 42 L900 220 L0 220Z" fill="url(#workspace-fill)"/><polyline points="0,190 65,176 120,184 185,145 240,151 300,118 365,128 430,86 495,103 560,70 625,82 690,48 760,62 825,31 900,42" fill="none" stroke="#4290ff" strokeWidth="3"/></svg></div></section><section className="rwa-workspace__lower"><article className="rwa-workspace__panel"><header><div><small>DEMO MARKET SNAPSHOT</small><h2>Sample opportunities</h2></div><Link href="/markets">View markets →</Link></header><div className="rwa-workspace__table"><div className="head"><span>Asset</span><span>Demo Price</span><span>Demo 24H</span><span>Demo Volume</span></div>{demoRows.map(([name,price,change,volume])=><div key={name}><b>{name}</b><span>{price}</span><em>{change}</em><span>{volume}</span></div>)}</div></article><aside className="rwa-workspace__panel"><small>WORKSPACE STATUS</small><h2>Demo interface ready</h2><dl><div><dt>Route</dt><dd>{path}</dd></div><div><dt>Navigation</dt><dd>Active</dd></div><div><dt>UI assets</dt><dd>Static-audit gated</dd></div><div><dt>Backend</dt><dd className="warn">Offline</dd></div></dl><Link className="rwa-link-button" href="/businesses">Browse Businesses</Link><Link className="rwa-link-button" href="/rwa">Browse RWA</Link></aside></section></main></PublicShell>;
}
