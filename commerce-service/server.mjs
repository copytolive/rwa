...
import {CommerceError,buildAuthoritativeQuote,mapMidtransNotification,pricingConfig,requireIdempotencyKey,transitionForPaymentEvent,jsonHash} from './commerce-core.mjs';
...
const errorStatus=e=>{if(Number(e?.statusCode)||Number(e?.status))return Number(e?.statusCode)||Number(e?.status);const m=String(e?.message||'');if(m==='order_not_found'||m==='product_not_found'||m==='quote_not_found')return 404;if(m==='quote_expired')return 410;if(m==='quote_consumed'||m.startsWith('insufficient_stock')||m.startsWith('invalid_transition')||m==='inventory_below_reserved')return 409;if(m.startsWith('registry_'))return 409;return 500};
...
async function route(req,res){
  ...
  if(method==='POST'&&path==='/v1/orders'){
    ...
    const order=db.createOrder({quote:q,wallet:s.wallet,contact:b.contact||{},notes:b.notes||'',paymentMode:String(b.payment_mode||'MIDTRANS').toUpperCase(),paymentReference:''});const response={ok:true,data:order};idemWrite(scope,key,b,201,response);return out(201,response)
  }
  ...
  m=match(path,/^\/v1\/orders\/([^/]+)\/cancel$/);if(method==='POST'&&m){const s=auth.requireSession(req),id=decodeURIComponent(m[1]),o=ensureOwner(db.order(id),s.wallet);if(o.status!=='AWAITING_PAYMENT')throw new CommerceError('cancel_not_allowed',409,{status:o.status});return out(200,{ok:true,data:db.transitionOrder(id,'CANCELLED',{actor:s.wallet})})}
  m=match(path,/^\/v1\/orders\/([^/]+)\/payment$/);if(method==='POST'&&m){
    ...
    const p=await payment.createForOrder(o);store.setPaymentReference(id,p.token||p.order_id,s.wallet);const response={ok:true,data:p};idemWrite(scope,key,idemRequest,201,response);return out(201,response)
  }
  ...
  m=match(path,/^\/v1\/admin\/refunds\/([^/]+)\/process$/);if(method==='POST'&&m){const a=auth.requireAdmin(req),rid=decodeURIComponent(m[1]),r=store.refund(rid);if(!r)throw new CommerceError('refund_not_found',404);if(!['REQUESTED','FAILED'].includes(r.status))throw new CommerceError('refund_not_processable',409,{status:r.status});const o=db.order(r.order_id);store.updateRefund(rid,'PROCESSING');try{const result=await payment.refund(o,{amountCents:r.amount_cents,reason:r.reason});store.updateRefund(rid,'SUCCEEDED',{providerReference:String(result.refund_key||result.transaction_id||''),detail:result});store.markOrderRefunded(o.id,a.actor);return out(200,{ok:true,data:store.refund(rid),order:db.order(o.id)})}catch(e){store.updateRefund(rid,'FAILED',{detail:{error:e.message}});throw e}}
  if(method==='GET'&&path==='/v1/seller/stores'){const s=auth.requireSession(req);return out(200,{ok:true,data:store.sellerStores(s.wallet)})}
  if(method==='GET'&&path==='/v1/seller/products'){const s=auth.requireSession(req),token=String(u.searchParams.get('store')||'').trim().toUpperCase();if(!token)throw new CommerceError('store_required',400);if(!store.canManageStore(token,s.wallet))throw new CommerceError('forbidden',403);return out(200,{ok:true,data:db.products(token)})}
...
const expiry=setInterval(()=>{try{for(const id of store.expiredAwaiting(Date.now()-priceConfig.paymentTtlMs)){try{db.transitionOrder(id,'CANCELLED',{actor:'system:payment-expiry'})}catch(e){console.warn('[commerce] expiry',id,e.message)}}}catch(e){console.warn('[commerce] expiry scan',e.message)}},60_000);expiry.unref();
