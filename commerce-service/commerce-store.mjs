...
export class CommerceStore {
  ...
  expiredAwaiting(beforeTs){
    // Only release an order locally when no provider payment has been created.
    // Once a Midtrans reference exists, its signed webhook is authoritative for
    // settlement/expiry. Releasing stock earlier could allow a still-payable link
    // to settle after inventory was returned to availability.
  ...
  requestRefund(order,requester,reason,amountCents=null){
    ...
    // Item-level/partial-return accounting is intentionally not invented. Until
    // that ledger exists, fail closed and support only a full-order refund.
  ...
  markOrderRefunded(orderId,actor='admin'){
    return this.db.transaction(()=>{const o=this.db.order(orderId);if(!o)throw new CommerceError('order_not_found',404);if(o.status==='REFUNDED')return o;if(o.status==='CANCELLED')throw new CommerceError('order_cancelled',409);
      ...
      // reservation. A completed order has already left on-hand inventory; do not
      // fabricate a physical restock without a real returned-goods receipt.
