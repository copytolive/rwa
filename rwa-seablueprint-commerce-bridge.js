...
function tradeCoin(anchor){
  const preset=String(anchor?.dataset?.seablueprintTrade||'').replace(/[^A-Za-z0-9]/g,'').toUpperCase();if(preset)return preset;
...
function decorate(){
  ...
  qa('#rwaShopScreen a[href]').forEach(a=>{a.removeAttribute('target');a.removeAttribute('rel');const coin=tradeCoin(a);if(coin){a.dataset.seablueprintTrade=coin;a.setAttribute('href',`#trade/${encodeURIComponent(coin)}`)}});
  restoreSingleShell();
}
...
document.addEventListener('click',e=>{
  ...
  const coin=tradeCoin(a);if(coin){e.preventDefault();e.stopImmediatePropagation();close({restore:false});const route=`trade/${coin}`;try{history.replaceState({...(history.state||{}),rwaRoute:route},'',`${state.rootPath}${location.search}#${route}`)}catch{location.hash=`#${route}`}queueMicrotask(()=>{if(window.RWASuperApp?.navigate)window.RWASuperApp.navigate(route,{replace:true})});return}
