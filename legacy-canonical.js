...
window.RWALegacyCanonical={version:'7.0.0',canonical:'https://copytolive.github.io/rwa/',renkoStandalone:true,renkoController:'153'};
try{
  const u=new URL(location.href),p=u.pathname.replace(/\/+$/,'/'),base='/rwa/';
  if(p.startsWith(base+'renko/')){
    document.documentElement.dataset.rwaRenkoStandalone='tick-native';
    const boot=()=>{
      if(document.querySelector('script[data-renko-bootstrap="153"]'))return;
      const s=document.createElement('script');s.src='renko-v15-controller-bootstrap.js?v=153';s.async=false;s.dataset.renkoBootstrap='153';document.body.appendChild(s);
    };
