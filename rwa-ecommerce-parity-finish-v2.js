(()=>{
'use strict';
if(window.RWAEcommerceParityFinishV2?.version==='2.1.0')return;
const VERSION='2.1.0',STYLE_ID='rwaEcommerceParityFinishV2Style';
...
@media(min-width:681px){
  ...
  html body.rwa-target-dashboard-v2.rwa-seablueprint-commerce-open .layout>.right>.institutional-card{display:none!important;visibility:hidden!important;pointer-events:none!important}
  html body.rwa-target-dashboard-v2.rwa-seablueprint-commerce-open .right .order-section{display:block!important;visibility:visible!important;opacity:1!important;min-height:0!important;height:100%!important;padding-bottom:12px!important;overflow:hidden!important}
...
function ensureBalance(){if(!open()||innerWidth<=680)return;const section=$('.layout>.right .order-section'),bids=$('#bids');if(!section||!bids)return;let bar=$('#rwaParityDepthBalance');if(!bar){bar=document.createElement('div');bar.id='rwaParityDepthBalance';bar.innerHTML='<span class="buy">B 56%</span><span class="sell">44% S</span>';bids.insertAdjacentElement('afterend',bar)}}
async function hydrateDepth(){if(!open()||innerWidth<=680)return false;const asks=$('#asks'),bids=$('#bids');if(!asks||!bids)return false;if(asks.querySelectorAll('.bookrow').length>=5&&bids.querySelectorAll('.bookrow').length>=5){ensureBalance();return true}const key=symbol(),now=Date.now();if(depthKey===key&&now-depthAt<1400)return false;depthKey=key;depthAt=now;const seq=++depthSeq;try{const r=await fetch(`https://data-api.binance.vision/api/v3/depth?symbol=${encodeURIComponent(key)}&limit=10`,{cache:'no-store'});if(!r.ok)throw Error(`depth ${r.status}`);const d=await r.json();if(seq!==depthSeq||symbol()!==key)return false;const aa=(Array.isArray(d.asks)?d.asks:[]).slice(0,5).reverse(),bb=(Array.isArray(d.bids)?d.bids:[]).slice(0,5);if(!aa.length||!bb.length)return false;asks.innerHTML=rows(aa,'down');bids.innerHTML=rows(bb,'up');const bestAsk=Number(d.asks?.[0]?.[0]),bestBid=Number(d.bids?.[0]?.[0]),mid=$('#midPrice');if(mid&&Number.isFinite(bestAsk)&&Number.isFinite(bestBid))mid.textContent=fmtPrice((bestAsk+bestBid)/2);ensureBalance();return true}catch(e){console.warn('RWA depth snapshot delayed',e?.message||e);return false}}
function sync(){installStyle();ensureBalance();void hydrateDepth()}
function audit(){const root=document.documentElement,layout=$('.layout')?.getBoundingClientRect(),left=$('.layout>.left')?.getBoundingClientRect(),main=$('.layout>.main')?.getBoundingClientRect(),order=$('.layout>.right')?.getBoundingClientRect(),dock=$('#rwaCommerceDock')?.getBoundingClientRect();const hidden=[...document.querySelectorAll('.layout>.right>.mover-section,.layout>.right>.activity-section,.layout>.right>.institutional-card')].every(x=>getComputedStyle(x).display==='none');return{version:VERSION,open:open(),viewportWidth:innerWidth,clientWidth:root.clientWidth,horizontalScrollbarPx:Math.max(0,innerWidth-root.clientWidth),layout:layout?{left:Math.round(layout.left),right:Math.round(layout.right),width:Math.round(layout.width),height:Math.round(layout.height)}:null,leftWidth:Math.round(left?.width||0),mainWidth:Math.round(main?.width||0),orderWidth:Math.round(order?.width||0),dockWidth:Math.round(dock?.width||0),dockLeft:Math.round(dock?.left||0),askRows:document.querySelectorAll('#asks .bookrow').length,bidRows:document.querySelectorAll('#bids .bookrow').length,balanceVisible:!!$('#rwaParityDepthBalance'),extraRailSectionsHidden:hidden}}
