...
function closeCommerceForNavigation(next){const r=String(next||'').replace(/^#/,'');if(!r||r==='shop'||r.startsWith('shop/'))return false;const screen=$('#rwaShopScreen'),open=!!(screen?.classList.contains('open')||document.body.classList.contains('rwa-seablueprint-commerce-open'));if(!open)return false;try{window.RWASeablueprintCommerceBridge?.close?.({restore:false})}catch{}screen?.classList.remove('open');document.body.classList.remove('rwa-shop-open','rwa-seablueprint-commerce-open');return true}
...
function patchSuper(){
  ...
  api.navigate=(next,...rest)=>{closeCommerceForNavigation(next);closeForNavigation();const out=nav(next,...rest);queueMicrotask(()=>{releaseStaleWorkspaceSuspend();mountMarketplace();syncFundVisual();syncLegacyRight()});return out};patchedSuper=true;mountMarketplace();return true
}
