...
async function prepareFundamentals(){
  ...
  document.body.classList.remove('rwa-super-workspace-open','rwa-super-asset-workspace');
}
function patchSuper(){
...
function audit(){
  const market=$('#rwaMarketplaceLaunch'),workspace=$('#rwaSuperWorkspace'),fund=$('#rwaFundamentals'),workspaceOpen=isWorkspaceOpen(),wr=visible(workspace)?workspace.getBoundingClientRect():null,fr=visible(fund)&&fund.classList.contains('open')?fund.getBoundingClientRect():null,root=document.documentElement;
  const findings=[];if(!visible(market)&&!workspaceOpen)findings.push('MARKETPLACE_NOT_VISIBLE');if(workspaceOpen&&isFundOpen())findings.push('MULTIPLE_CONTEXT_SURFACES');if(intersection(wr,fr)>4)findings.push('CONTEXT_OVERLAP');if(root.scrollWidth-root.clientWidth>4)findings.push('ROOT_HORIZONTAL_OVERFLOW');const top=$('.topbar')?.getBoundingClientRect();if(top&&(top.right>innerWidth+2||top.left<-2))findings.push('TOPBAR_OUT_OF_VIEWPORT');
  return{ok:findings.length===0,version:'15.0.0',route:window.RWASuperApp?.route?.()||location.hash,marketplaceVisible:visible(market),workspaceOpen,fundamentalsOpen:isFundOpen(),contextOverlapPx:Math.round(intersection(wr,fr)),horizontalOverflowPx:Math.max(0,root.scrollWidth-root.clientWidth),findings};
}
function boot(){
