...
VIEWPORT={"width":1672,"height":941};ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"visual-audit";PREVIEW=ROOT/".visual-preview-chat11";SITE=PREVIEW/"realworldasset"
SURFACES={"36-settings":"/settings/","37-security-sessions":"/settings/security/","38-pro-explanation":"/pro/","54-help-center":"/help/","55-system-status":"/status/"}
def prepare():
...
def audit(page,base,name):
    ...
    if fail:raise RuntimeError("CHAT11 browser button audit failed: "+"; ".join(f'{x["surface"]} #{x["index"]} {x["label"]}: {x.get("error") or "no observable action"}' for x in fail[:30]))
    print(f"CHAT11 button audit PASS: {name} — {len(rows)} visible buttons checked");return rows
def verify_flow(page,base):
    goto(page,base,"/settings/");page.get_by_role("button",name="Manage Security →",exact=True).click();page.wait_for_timeout(180)
    if "/settings/security" not in page.url:raise RuntimeError("Settings -> Security failed")
    page.get_by_role("button",name="◇ Revoke All Other Sessions",exact=True).click();page.wait_for_timeout(100)
    if page.locator('[role="dialog"]').count()!=1:raise RuntimeError("Session confirmation dialog failed")
    page.get_by_role("button",name="Cancel",exact=True).click()
    goto(page,base,"/help/");page.get_by_role("button",name="System Status",exact=True).click();page.wait_for_timeout(180)
    if not page.url.endswith('/status/'):raise RuntimeError("Help -> Status failed")
    goto(page,base,"/pro/");page.get_by_role("button",name="Switch to PRO",exact=True).click();page.wait_for_timeout(100)
    if "PRO mode enabled" not in page.locator("body").inner_text():raise RuntimeError("PRO switch state failed")
    print("CHAT11 flow PASS: Settings -> Security confirmation; Help -> Status; PRO mode state")
def main():
    prepare();server=subprocess.Popen(["python3","-m","http.server","4181","--bind","127.0.0.1","--directory",str(PREVIEW)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);time.sleep(1);base="http://127.0.0.1:4181/realworldasset";records=[];audits=[]
    try:
    ...
    (OUT/"chat11-capture.json").write_text(json.dumps(records,indent=2));(OUT/"chat11-button-audit.json").write_text(json.dumps(audits,indent=2));print(f"Captured {len(records)} CHAT11 surfaces at 1672x941; {sum(x['status'].startswith('PASS') for x in audits)} controls passed/current-state")
