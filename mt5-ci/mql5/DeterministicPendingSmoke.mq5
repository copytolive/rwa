...
bool Place(ENUM_ORDER_TYPE type,double price,double sl,double tp,string comment){
 MqlTradeRequest r={}; MqlTradeResult z={};
 r.action=TRADE_ACTION_PENDING;r.magic=Magic;r.symbol=_Symbol;r.volume=Lots;r.type=type;r.price=price;r.sl=sl;r.tp=tp;r.deviation=0;r.type_time=ORDER_TIME_GTC;r.type_filling=ORDER_FILLING_FOK;r.comment=comment;
 if(!OrderSend(r,z)){Print("OrderSend false err=",GetLastError());return false;}
 Print("OrderSend retcode=",z.retcode," order=",z.order," comment=",z.comment);
 return (z.retcode==TRADE_RETCODE_DONE || z.retcode==TRADE_RETCODE_PLACED);
}
void Buy(){MqlTick q;if(!SymbolInfoTick(_Symbol,q))return;double e=NormalizeDouble(q.ask+DistancePoints*_Point,_Digits),sl=NormalizeDouble(e-RiskPoints*_Point,_Digits),tp=NormalizeDouble(e+RewardPoints*_Point,_Digits);if(Place(ORDER_TYPE_BUY_STOP,e,sl,tp,"SMOKE_BUY"))buy_placed=true;}
void Sell(){MqlTick q;if(!SymbolInfoTick(_Symbol,q))return;double e=NormalizeDouble(q.bid-DistancePoints*_Point,_Digits),sl=NormalizeDouble(e+RiskPoints*_Point,_Digits),tp=NormalizeDouble(e-RewardPoints*_Point,_Digits);if(Place(ORDER_TYPE_SELL_STOP,e,sl,tp,"SMOKE_SELL"))sell_placed=true;}
...
void ExportLedger(){
 ...
 int h=FileOpen("mt5_smoke_deals.csv",FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON,',');if(h==INVALID_HANDLE){Print("ledger FileOpen err=",GetLastError());return;}FileWrite(h,"position_id","entry","type","time_msc","price","profit");
...
void OnDeinit(const int reason){ExportLedger();}
