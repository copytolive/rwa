...
}catch{Diagnostics;if(!(Test-Path (Join-Path $out 'final_status.json'))){[ordered]@{status='FAIL';error=$_.Exception.Message;utc=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json|Set-Content -Encoding UTF8 (Join-Path $out 'final_status.json')};Write-Host $_.Exception.Message;throw}
