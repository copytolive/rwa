...
contract RWADistributionVault is AccessControl {
    ...
    function isClaimed(uint256 distributionId,uint256 index) public view returns(bool){uint256 wordIndex=index/256,bitIndex=index%256;uint256 word=_claimedBitMap[distributionId][wordIndex];uint256 mask=(1<<bitIndex);return word&mask==mask;}
    function _setClaimed(uint256 distributionId,uint256 index) private{uint256 wordIndex=index/256,bitIndex=index%256;_claimedBitMap[distributionId][wordIndex]|=(1<<bitIndex);}
