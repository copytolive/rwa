...
/* RWA_TARGET_DASHBOARD_V2_AND_SEABLUEPRINT_CONTEXT_PANEL */
(async()=>{try{for(const[key,href]of[['v2','rwa-target-dashboard-v2.css?v=2.0.0'],['hardening','rwa-target-dashboard-v2-hardening.css?v=2.1.0']]){if(document.querySelector(`link[data-rwa-target-${key}]`))continue;const l=document.createElement('link');l.rel='stylesheet';l.href=href;l.dataset[`rwaTarget${key[0].toUpperCase()+key.slice(1)}`]='1';document.head.appendChild(l)}if(!window.RWATargetDashboardV2)await import('./rwa-target-dashboard-v2.js?v=2.0.0');if(!window.RWASeablueprintCommerceBridge)await import('./rwa-seablueprint-commerce-bridge.js?v=1.5.0');if(!window.RWATargetDashboardV2Hardening)await import('./rwa-target-dashboard-v2-hardening.js?v=2.1.0')}catch(e){console.error('RWA target/commerce context panel failed to load',e)}})();
