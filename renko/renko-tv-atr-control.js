...
async function applyLocal(value,source='control'){
  ...
  load(pass?`LIVE · fixed 1s · ATR ${n.toLocaleString()} · ${available.toLocaleString()} Binance 1s bars`:`ATR ${n.toLocaleString()} could not produce a valid box`,pass);
  window.dispatchEvent(new CustomEvent('renko:atr-control-applied',{detail:{symbol,length:n,pass,rawAtr:raw,box,available,source}}));
  return pass;
}
...
window.RWARenkoATRControl={version:'1.0.0',rule:'xaut-deep-matrix-only-other-pairs-local-fixed1s-atr',applyLocal,ensureHistory,isLocal};
})();
