...
function clear(reason='user'){
  if(hold)stats.clears++;hold=null;stats.lastReason=reason;root.dataset.renkoGoldViewportAuthorityHold='false';
}
function drift(){
  if(!hold)return false;const t=ts(),r=timeRange(t?.getVisibleRange?.()),o=options(t);if(!r)return true;
  return Math.abs(r.from-hold.time.from)>1||Math.abs(r.to-hold.time.to)>1||(finite(hold.barSpacing)&&finite(o.barSpacing)&&Math.abs(o.barSpacing-hold.barSpacing)>1e-9)||(finite(hold.rightOffset)&&finite(o.rightOffset)&&Math.abs(o.rightOffset-hold.rightOffset)>1e-9);
}
function apply(reason='range-drift'){
  if(!hold||applying||M.inHistoryMutation)return false;const t=ts();if(!t)return false;
  applying=true;try{
    const o={};if(finite(hold.barSpacing))o.barSpacing=hold.barSpacing;if(finite(hold.rightOffset))o.rightOffset=hold.rightOffset;if(Object.keys(o).length)t.applyOptions?.(o);
    t.setVisibleRange(hold.time);followingOff();stats.repairs++;stats.lastReason=reason;root.dataset.renkoGoldViewportAuthorityRepairs=String(stats.repairs);return true;
  }catch(_){return false}finally{applying=false}
}
function check(reason='range-event'){
  queued=false;if(!hold||M.inHistoryMutation)return;stats.repairChecks++;if(drift())apply(reason);else followingOff();
}
function schedule(reason='range-event'){
  if(!hold||queued||applying)return;queued=true;requestAnimationFrame(()=>check(reason));
}
function arm(result){
  const t=timeRange(result?.targetTime||result?.beforeTime);if(!t)return clear('no-target');
  hold={id:result?.id??null,time:t,barSpacing:finite(result?.barSpacingBefore)?Number(result.barSpacingBefore):null,rightOffset:finite(result?.rightOffsetBefore)?Number(result.rightOffsetBefore):null};
  stats.holds++;stats.lastHold={...hold,time:{...hold.time}};stats.lastReason='history-end';root.dataset.renkoGoldViewportAuthorityHold='true';root.dataset.renkoGoldViewportAuthorityId=String(hold.id??'');followingOff();schedule('history-end');
}
...
function bind(){
  if(bound)return true;const t=ts();if(!t?.subscribeVisibleLogicalRangeChange)return false;bound=true;t.subscribeVisibleLogicalRangeChange(()=>{stats.rangeEvents++;if(hold&&!M.inHistoryMutation&&!applying)schedule('programmatic-range')});root.dataset.renkoGoldViewportAuthorityBound='true';return true;
}
const userStart=e=>{if(e.target?.closest?.('#chartWrap,#chartHost'))clear(e.type)};
document.addEventListener('wheel',userStart,{capture:true,passive:true});
document.addEventListener('pointerdown',userStart,{capture:true,passive:true});
document.addEventListener('touchstart',userStart,{capture:true,passive:true});
...
for(const ev of ['renko:chart-ready','renko:tv-ready'])window.addEventListener(ev,bind);
const poll=setInterval(()=>{if(bind())clearInterval(poll)},50);setTimeout(()=>clearInterval(poll),5000);
