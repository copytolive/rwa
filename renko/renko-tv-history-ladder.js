...
function ladder(interval){
  const hi=highestDocumented(interval),a=ORDER.indexOf(interval),b=ORDER.indexOf(hi);
  if(a<0)return[interval];if(b<a)return[interval];
...
async function extend(T=window.RWARenkoTV,{targetBars=0,maxPagesPerTier=MAX_PAGES_PER_TIER,rebuild=true}={}){
  if(!T||busy||!T.state?.closedBars?.length)return false;busy=true;
  ...
  try{
    ...
    for(const interval of tiers){
      let pages=0,stagnant=0,cursor=boundary-1;
      while(pages<maxPagesPerTier){
        if(generation!==T.state.generation||T.settings.interval!==selected)return false;
        if(targetBars>0&&bars.length>=targetBars)break;
        const rows=await page(symbol,interval,cursor),older=rows.filter(b=>b.closeTime<boundary);
        pages++;
        if(!older.length){stagnant++;if(stagnant>=1)break;continue}
        stagnant=0;
        const before=bars.length;bars=mergeOlder(older,bars,boundary);const added=bars.length-before;addedTotal+=Math.max(0,added);
        const oldest=num(older[0]?.openTime);if(!(oldest<cursor))break;cursor=oldest-1;boundary=num(bars[0]?.openTime);
        await new Promise(r=>setTimeout(r,0));
      }
      if(interval!==selected&&addedTotal>0&&!used.includes(interval))used.push(interval);
      if(targetBars>0&&bars.length>=targetBars)break;
    }
    if(generation!==T.state.generation)return false;
    T.state.closedBars=bars;T.state.historyPages=Math.max(Number(T.state.historyPages)||1,5)+addedTotal/1000;
    stamp(T,used);
    if(rebuild&&addedTotal>0)T.rebuild({fit:false});
    return addedTotal>0;
...
window.addEventListener('renko:tv-ready',()=>{const T=window.RWARenkoTV;if(!T)return;stamp(T,[T.settings.interval]);setTimeout(()=>void extend(T,{maxPagesPerTier:2,rebuild:true}),250)},{once:true});
window.RWARenkoHistoryLadder={version:'1.0.0',rule:'selected-finest-then-higher-timeframes-on-history-exhaustion',highestDocumented,ladder,extend,coverage,get busy(){return busy}};
})();