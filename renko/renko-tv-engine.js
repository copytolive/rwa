...
function emit(s,out,open,close,dir,box,sourceTime,wicks,isReversal,sourceKind){let high=Math.max(open,close),low=Math.min(open,close);if(wicks){if(dir>0&&finite(s.pendingLow))low=Math.min(low,s.pendingLow);if(dir<0&&finite(s.pendingHigh))high=Math.max(high,s.pendingHigh)}out.push({open,high,low,close,direction:dir,box,isReversal:!!isReversal,sourceTime:num(sourceTime)||0,sourceKind:sourceKind||'historical'});s.lastClose=close;s.direction=dir;s.lastSourceTime=num(sourceTime)||s.lastSourceTime;resetPending(s,close)}
function processPrice(s,p,out,opt={}){p=num(p);const box=num(opt.box),tm=num(opt.sourceTime)||0;if(!Number.isFinite(p)||!(box>0))return 0;touch(s,p);const before=out.length;let guard=0;while(guard++<20000){const lc=num(s.lastClose),d=num(s.direction)||0;if(d===0){if(p>=lc+box){emit(s,out,lc,lc+box,1,box,tm,opt.wicks!==false,false,opt.sourceKind);continue}if(p<=lc-box){emit(s,out,lc,lc-box,-1,box,tm,opt.wicks!==false,false,opt.sourceKind);continue}break}if(d>0){if(p>=lc+box){emit(s,out,lc,lc+box,1,box,tm,opt.wicks!==false,false,opt.sourceKind);continue}if(p<=lc-2*box){emit(s,out,lc-box,lc-2*box,-1,box,tm,opt.wicks!==false,true,opt.sourceKind);continue}break}if(p<=lc-box){emit(s,out,lc,lc-box,-1,box,tm,opt.wicks!==false,false,opt.sourceKind);continue}if(p>=lc+2*box){emit(s,out,lc+box,lc+2*box,1,box,tm,opt.wicks!==false,true,opt.sourceKind);continue}break}return out.length-before}
function trueRange(bar,prevClose){const h=num(bar?.high),l=num(bar?.low),pc=num(prevClose);if(!Number.isFinite(h)||!Number.isFinite(l))return NaN;if(!Number.isFinite(pc))return h-l;return Math.max(h-l,Math.abs(h-pc),Math.abs(l-pc))}
function atrSeries(bars,length=14){length=Math.max(1,Math.floor(num(length)||14));const src=(Array.isArray(bars)?bars:[]).filter(b=>finite(b?.high)&&finite(b?.low)&&finite(b?.close));const out=new Array(src.length).fill(NaN),trs=[];let prev=NaN,atr=NaN;for(let i=0;i<src.length;i++){const tr=trueRange(src[i],prev);trs.push(tr);prev=num(src[i].close);if(i===length-1){atr=trs.slice(0,length).reduce((a,b)=>a+b,0)/length;out[i]=atr}else if(i>=length){atr=((atr*(length-1))+tr)/length;out[i]=atr}}if(src.length&&src.length<length){const a=trs.filter(Number.isFinite);if(a.length)out[src.length-1]=a.reduce((x,y)=>x+y,0)/a.length}return out}
function latestAtr(bars,length=14,_tick=0){const a=atrSeries(bars,length);for(let i=a.length-1;i>=0;i--)if(Number.isFinite(a[i]))return a[i];return NaN}
function latestClose(bars){for(let i=(bars?.length||0)-1;i>=0;i--){const c=num(bars[i]?.close);if(Number.isFinite(c))return c}return NaN}
function atrBox(bars,length,tick=0){const raw=latestAtr(bars,length);if(Number.isFinite(raw)&&raw>0)return raw;const t=num(tick);return t>0?t:Number.EPSILON}
function computeBox(bars,settings={},tickSize=0){const exact=num(settings?._exactBox);if(Number.isFinite(exact)&&exact>0)return exact;const method=String(settings.method||'atr').toLowerCase(),tick=num(tickSize)>0?num(tickSize):0;if(method==='traditional')return roundTick(Math.max(Number.EPSILON,num(settings.boxSize)||num(settings.traditionalBox)||1),tick);if(method==='percentage'){const pct=Math.max(.000001,num(settings.percentage)||.01),c=latestClose(bars);return percentageLtpStableRound(c*pct,tick)}return atrBox(bars,settings.atrLength||14,tick)}
