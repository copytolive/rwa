...
const MAX_RENDER_BRICKS=6000,PREFIX_BRICKS=2500,VIEW_BUFFER=250;
...
function visibleOldIndex(oldData){
  const r=TS()?.getVisibleRange?.();if(!r||!oldData?.length)return 0;const from=num(r.from);let lo=0,hi=oldData.length;while(lo<hi){const m=(lo+hi)>>1;if(num(oldData[m]?.time)<from)lo=m+1;else hi=m}return Math.max(0,Math.min(oldData.length-1,lo));
}
function decoratePrefix(rows,suffixStartTime){
  const out=rows.map(b=>({...b,_canonicalSourceTime:canonicalTime(b)}));
  let last=0;for(const b of out){let t=Math.max(1,Math.floor(canonicalTime(b)/1000));if(t<=last)t=last+1;last=t;b.sourceTime=t*1000}
...
function stitch(prepared,oldBricks,oldData,oldBase,tick){
  ...
  const oldBox=num(oldBase?.box),newBox=num(prepared?.box),tol=Math.max(1e-12,Math.abs(oldBox||newBox||1)*1e-10);
  if(finite(oldBox)&&finite(newBox)&&Math.abs(oldBox-newBox)>tol){stats.boxMismatches++;return null}
  const workerMap=new Map();for(let i=0;i<wb.length;i++)workerMap.set(brickKey(wb[i],tick),i);
  const visible=visibleOldIndex(oldData),limit=Math.min(oldBricks.length-1,Math.max(0,visible-VIEW_BUFFER));
  let oi=-1,wi=-1;
  ...
  const suffixStart=finite(suffixData[0]?.time)?num(suffixData[0].time):null,prefix=decoratePrefix(prefixRaw,suffixStart);
  let bricks=[...prefix,...suffix];if(bricks.length>MAX_RENDER_BRICKS)bricks=bricks.slice(0,MAX_RENDER_BRICKS);
...
function install(){
  const t=T();if(wrapped||!t?.rebuild||!t?.engine?.build)return false;
  const prior=t.rebuild.bind(t);
  t.rebuild=(opts={})=>{
    const h=H(),active=!!h?.busy&&t.state?.symbol==='XAUUSD'&&Array.isArray(t.state?.confirmed)&&t.state.confirmed.length&&Array.isArray(t.state?.confirmedData)&&t.state.confirmedData.length;
    if(!active)return prior(opts);
    const outerBuild=t.engine.build,oldBricks=t.state.confirmed.slice(),oldData=t.state.confirmedData.slice(),oldBase=t.state.base,tick=num(t.state.tickSize)||0.001;
    let prepared;try{prepared=outerBuild(t.state.closedBars,t.settings,t.state.tickSize)}catch(_){return prior({...opts,fit:false})}
    const result=stitch(prepared,oldBricks,oldData,oldBase,tick);
    if(!result){stats.misses++;stats.last={ok:false,oldBricks:oldBricks.length,workerBricks:prepared?.bricks?.length||0};return prior({...opts,fit:false})}
...
poll=setInterval(()=>{if(install()){clearInterval(poll);poll=0}},25);setTimeout(()=>{if(poll)clearInterval(poll)},10000);
for(const ev of ['renko:tv-ready','renko:chart-ready','renko:gold-recent'])window.addEventListener(ev,install);
window.RWARenkoGoldHistoryGeometryStitch={version:'1.0.0-immutable-visible-suffix',rule:'GOLD prepend adds bounded worker prefix while preserving already-visible Renko suffix geometry',stats,install};
})();
