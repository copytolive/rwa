...
const stats={applies:0,wheels:0,syntheticPans:0,prependRequests:0,prependSuccesses:0,postPrependScaleWrites:0,corrections:0,lockStarts:0,lockClears:0,maxBarSpacingDelta:0,last:null};
...
function publish(){Object.assign(root.dataset,{renkoGoldWheelPanOnly:'true',renkoGoldWheelPanApplies:String(stats.applies),renkoGoldWheelPanEvents:String(stats.wheels),renkoGoldWheelSyntheticPans:String(stats.syntheticPans),renkoGoldWheelPrependRequests:String(stats.prependRequests),renkoGoldWheelPrependSuccesses:String(stats.prependSuccesses),renkoGoldWheelTrustedPrependEdge:String(TRUSTED_PREPEND_EDGE_BARS),renkoGoldWheelPostPrependScaleWrites:String(stats.postPrependScaleWrites),renkoGoldWheelScaleCorrections:String(stats.corrections),renkoGoldWheelSizeLock:finite(sizeLockSpacing)?'true':'false',renkoGoldWheelSizeLockSpacing:finite(sizeLockSpacing)?String(sizeLockSpacing):'',renkoGoldWheelMaxSpacingDelta:String(stats.maxBarSpacingDelta)})}
...
async function maybePrependOlder(dx,expectedSpacing,rangeHint=null){
  ...
  const H=window.RWARenkoGoldTotalHistory,T=window.RWARenkoTV,r=rangeHint||logical(ts());
  if(!H?.prependOlder||H.busy||T?.state?.symbol!=='XAUUSD'||!r||Number(r.from)>TRUSTED_PREPEND_EDGE_BARS)return false;
  const oldest=Math.floor(Number(T.state?.closedBars?.[0]?.openTime||0)/1000);if(!oldest||oldest===requestedOldest)return false;
  requestedOldest=oldest;stats.prependRequests++;if(finite(expectedSpacing))holdSize(Number(expectedSpacing),4200);publish();
  try{
    const ok=await H.prependOlder({panOlder:false});
    if(ok){stats.prependSuccesses++;if(finite(expectedSpacing)){forceSpacing(Number(expectedSpacing));holdSize(Number(expectedSpacing),1800);requestAnimationFrame(()=>forceSpacing(Number(expectedSpacing)));setTimeout(()=>forceSpacing(Number(expectedSpacing)),120);setTimeout(()=>forceSpacing(Number(expectedSpacing)),420)}}
    else requestedOldest=0;
    publish();return !!ok;
  }catch(e){requestedOldest=0;publish();console.warn('[RENKO GOLD wheel prepend]',e);return false}
}
function onWheel(e){if(!e.isTrusted)return;if(!e.target?.closest?.('#chartWrap,#chartHost'))return;enforce();const t=ts(),beforeSpacing=finite(sizeLockSpacing)?Number(sizeLockSpacing):spacing(t),dx=horizontalDelta(e);stats.wheels++;if(e.cancelable)e.preventDefault();e.stopImmediatePropagation();holdSize(beforeSpacing);dispatchHorizontal(e,dx);if(window.RWARenkoTV?.state)window.RWARenkoTV.state.following=false;requestAnimationFrame(()=>{const afterSpacing=spacing(t),afterLogical=logical(t),d=finite(beforeSpacing)&&finite(afterSpacing)?Math.abs(afterSpacing-beforeSpacing):0;stats.maxBarSpacingDelta=Math.max(stats.maxBarSpacingDelta,d);stats.last={inputDeltaX:Number(e.deltaX)||0,inputDeltaY:Number(e.deltaY)||0,sanitizedDeltaX:dx,barSpacingBefore:beforeSpacing,barSpacingAfter:afterSpacing,spacingDelta:d,logicalAfter:afterLogical};publish();void maybePrependOlder(dx,beforeSpacing,afterLogical)});publish()}
...
enforce();
window.RWARenkoGoldWheelPanLock={version:'1.3.0-full-prepend-size-lock',implementation:'1.7.0-preblank-edge-preload',legacyRemoved:'scrollToPosition',rule:'Trusted mixed trackpad wheel is intercepted before Lightweight Charts and re-dispatched as bounded horizontal-only deltaY=0; native wheel scale is disabled; strict finite spacing is locked through prepend and delayed settle; trusted older-directed wheel preloads history before the rendered viewport reaches blank left-edge overscroll while programmatic observer fallback remains stricter',stats,enforce,horizontalDelta,dispatchHorizontal,maybePrependOlder,forceSpacing,holdSize,clearSizeLock,trustedPrependEdgeBars:TRUSTED_PREPEND_EDGE_BARS,get sizeLockSpacing(){return sizeLockSpacing}};
})();
