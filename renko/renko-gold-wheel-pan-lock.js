...
function publish(){Object.assign(root.dataset,{renkoGoldWheelPanOnly:'true',renkoGoldWheelPanApplies:String(stats.applies),renkoGoldWheelPanEvents:String(stats.wheels),renkoGoldWheelSyntheticPans:String(stats.syntheticPans),renkoGoldWheelPrependRequests:String(stats.prependRequests),renkoGoldWheelPrependSuccesses:String(stats.prependSuccesses),renkoGoldWheelPostPrependScaleWrites:String(stats.postPrependScaleWrites),renkoGoldWheelScaleCorrections:String(stats.corrections),renkoGoldWheelSizeLock:finite(sizeLockSpacing)?'true':'false',renkoGoldWheelSizeLockSpacing:finite(sizeLockSpacing)?String(sizeLockSpacing):'',renkoGoldWheelMaxSpacingDelta:String(stats.maxBarSpacingDelta)})}
...
async function maybePrependOlder(dx,expectedSpacing){
...
enforce();
window.RWARenkoGoldWheelPanLock={version:'1.3.0-full-prepend-size-lock',implementation:'1.6.0-post-prepend-exact-spacing',legacyRemoved:'scrollToPosition',rule:'Trusted mixed trackpad wheel is intercepted before Lightweight Charts and re-dispatched as horizontal-only deltaY=0; native wheel scale is disabled; strict finite spacing is locked through the history mutation and explicitly re-applied after prepend completion and delayed settle; only trusted older-directed wheel at the real left edge may request total-history prepend',stats,enforce,horizontalDelta,dispatchHorizontal,maybePrependOlder,forceSpacing,holdSize,clearSizeLock,get sizeLockSpacing(){return sizeLockSpacing}};
})();
