...
window.RWARenkoFixed1s={version:'1.5.0',interval:'1s',selectorAllowed:false,okxCandleWebSocket:'business'};
})();
 ...
 * mathematically produces millions/billions of bricks therefore never allocates
 * millions/billions of JS objects and does not change the exact final state.
 */
(()=>{
'use strict';
...
function transition(s,p,opt={}){
  ...
  const desc={lc,count,dir,sign,reversal,box,tm,wicks:opt.wicks!==false,sourceKind:opt.sourceKind||'historical',pendingHigh,pendingLow};
  s.lastClose=reversal?lc+sign*(count+1)*box:lc+sign*count*box;s.direction=dir;s.lastSourceTime=tm||s.lastSourceTime;s.pendingHigh=s.lastClose;s.pendingLow=s.lastClose;
  return desc;
}
function materialize(descriptors,limit,projection=false){
  ...
  for(let i=descriptors.length-1;i>=0&&remaining>0;i--){const d=descriptors[i],take=Math.min(d.count,remaining);chosen.push({d,start:d.count-take+1});remaining-=take}
  chosen.reverse();const out=[];
  for(const {d,start} of chosen){for(let j=start;j<=d.count;j++){let open,close,isRev=false;if(d.reversal){open=d.lc+d.sign*j*d.box;close=d.lc+d.sign*(j+1)*d.box;isRev=j===1}else{open=d.lc+d.sign*(j-1)*d.box;close=d.lc+d.sign*j*d.box}out.push(brick(open,close,d.dir,d.box,d.tm,d.wicks,isRev,d.sourceKind,d.pendingHigh,d.pendingLow,j===1,projection))}}
  return out;
}
/* Compatibility helper for any diagnostic caller. Main build/project do not use
 * it, so the production hot path stays O(source prices + final render tail). */
...
window.RWARenkoBrickBudget={version:'1.3.0',rule:'exact-box-exact-final-state-o1-source-transition-bounded-tail-materialization',defaultLimit:DEFAULT_LIMIT,projectionLimit:PROJECTION_LIMIT,transition,materialize,processTail,buildBudget,projectBudget,originalBuild,originalProject,syncUi};
})();
