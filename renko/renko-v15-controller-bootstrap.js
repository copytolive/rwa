(()=>{
'use strict';
if(document.documentElement.dataset.renkoMethodBootstrap==='171')return;
document.documentElement.dataset.renkoMethodBootstrap='171';
const old=document.getElementById('v15BoxCard');
if(old){
  const clean=old.cloneNode(true);
  clean.dataset.controller='v15.11-fill-active-history';
...
let style=document.getElementById('v171InteractionStyle');
if(!style){
  style=document.createElement('style');style.id='v171InteractionStyle';
...
s.src='renko-v15-method-controller-v170.js?v=171';
s.async=false;
s.dataset.renkoController='171';
s.onerror=()=>console.error('[RENKO] history-fill controller failed to load');
