...
async function chooseBase(bases,symbol){const y=2017,m=8,path=monthlyPath(symbol==='BTCUSDT'?symbol:'BTCUSDT',y,m);for(const base of bases){const p=await safeHead(base+path);if(p.ok)return base}throw Error('No browser-accessible raw tick archive source. Controlled CORS proxy is required.')}
async function discover(base,symbol){
  ...
  if(seen||symbol==='BTCUSDT'){for(let day=1;day<=cd;day++)plan.push({kind:'daily',key:`D:${cy}-${pad(cm)}-${pad(day)}`,path:dailyPath(symbol,cy,cm,day),size:0,required:true})}
...
function engine(box){return {box,lastClose:NaN,direction:0,bricks:[],ticks:0,firstTime:0,lastTime:0,firstId:null,lastId:null}}
