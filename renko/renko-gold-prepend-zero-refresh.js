...
const stats={rebuilds:0,immediateRestores:0,reinforcedRestores:0,restoreFailures:0,longTasks:0,tbtMs:0,maxBlockingMs:0,lastBefore:null,lastAfter:null,lastLogicalBefore:null,lastLogicalAfter:null,lastApplyMs:0};
let wrapped=false,poll=0,windowStart=0,windowEnd=0;
...
function install(){
  ...
  t.rebuild=(opts={})=>{
    ...
    windowStart=started;windowEnd=started+250;
    ...
    publish();
    // Reinforce against library-internal deferred scale bookkeeping without ever
    // changing the requested range or zoom.
    ...
    queueMicrotask(reinforce);requestAnimationFrame(reinforce);requestAnimationFrame(()=>requestAnimationFrame(reinforce));setTimeout(reinforce,0);setTimeout(reinforce,80);
    return out;
  };
...
window.RWARenkoGoldPrependZeroRefresh={version:'1.0.0-before-paint-time-range',rule:'canonical-history-prepend-restores-exact-visible-time-range-synchronously-before-paint-no-logical-index-reuse-no-autofit-no-zoom-reset',stats,ready,sameRange};
})();