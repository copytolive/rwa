...
const create=L.createChart.bind(L),stats={rangeCallbacks:0,leftEdgeClamps:0,explicitOlderRequests:0};
function wrapTs(ts){return new Proxy(ts,{get(t,p,r){if(p==='subscribeVisibleLogicalRangeChange')return cb=>t.subscribeVisibleLogicalRangeChange(range=>{stats.rangeCallbacks++;if(!range)return cb(range);const from=Number(range.from);if(Number.isFinite(from)&&from<8){stats.leftEdgeClamps++;document.documentElement.dataset.renkoScrollEdge='true';return cb({...range,from:8})}document.documentElement.dataset.renkoScrollEdge='false';return cb(range)});const v=Reflect.get(t,p,r);return typeof v==='function'?v.bind(t):v}})}
...
window.RWARenkoScrollZeroLock={version:'1.1.0-total-history',rule:'wheel-pan-render-only-no-auto-history-fetch-reanchor-explicit-io-outside-wheel',stats};
})();