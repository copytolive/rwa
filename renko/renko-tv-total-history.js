 ...
 * Binance Spot launch pairs: official /api/v3/klines 1s + Data Vision archive checksum discovery.
...
const ORIGIN_BARS=6000,DB='renko-total-history-v2',STORE='originWindows';
...
async function discoverBinance(symbol){
  ...
  try{const saved=JSON.parse(localStorage.getItem(`renko-history-manifest:${symbol}`)||'null');if(saved?.symbol===symbol&&saved?.interval==='1s'&&saved?.archiveFirstMonthMs){manifest.set(symbol,saved);return saved}}catch{}
  ...
  let hi=months.length-1;while(hi>=0&&!(await monthExists(symbol,months[hi])))hi--;
  ...
  let first=months[lo];for(let i=Math.max(0,lo-2);i<lo;i++)if(await monthExists(symbol,months[i])){first=months[i];break}
  const checksum=(await textOk(checksumUrl(symbol,first))||'').trim();
  if(checksum.length<20)throw Error(`${symbol} origin checksum unavailable`);
  const span=Math.max(1,Math.round((months[hi]-first)/2629800000)+1);
  const m={symbol,provider:'Binance Spot',source:'Binance Public Data + Data API',interval:'1s',archive:'Binance Public Data / Data Vision',archiveFirstMonth:monthKey(first),archiveFirstMonthMs:first,archiveLastVerifiedMonth:monthKey(months[hi]),archiveLastVerifiedMonthMs:months[hi],archiveMonthSpan:span,totalSourceChunks:span,integrityChecksum:checksum,discoveredAt:Date.now()};
  manifest.set(symbol,m);try{localStorage.setItem(`renko-history-manifest:${symbol}`,JSON.stringify(m))}catch{}return m
}
async function fetchKlines(symbol,startTime,limit=1000){const q=new URLSearchParams({symbol,interval:'1s',limit:String(Math.min(1000,limit)),startTime:String(Math.floor(startTime))});const r=await fetch(`${BINANCE_API}/api/v3/klines?${q}`,{cache:'no-store',credentials:'omit'});if(!r.ok)throw Error(`Binance 1s HTTP ${r.status}`);const a=await r.json();return(Array.isArray(a)?a:[]).map(x=>({openTime:normTs(x[0]),open:Number(x[1]),high:Number(x[2]),low:Number(x[3]),close:Number(x[4]),volume:Number(x[5])||0,closeTime:normTs(x[6])})).filter(b=>Number.isFinite(b.openTime)&&Number.isFinite(b.closeTime)&&[b.open,b.high,b.low,b.close].every(Number.isFinite))}
async function fetchOriginWindow(symbol,m){
  ...
  const v={key,symbol,provider:'Binance Spot',source:'Binance Data API historical 1s',interval:'1s',bars,earliestAvailableMs:bars[0].openTime,originWindowNewestMs:bars.at(-1).closeTime,archiveFirstMonth:m.archiveFirstMonth,integrityChecksum:m.integrityChecksum,savedAt:Date.now()};
...
function tickRows(j){
  if(!j||!Array.isArray(j.times)||!Array.isArray(j.bids)||!Number.isFinite(j.timestamp)||!Number.isFinite(j.multiplier)||!(j.multiplier>0)||typeof j.bid!=='number')return[];
...
async function goldOrigin(){
  const key='dukascopy:XAU-USD:2003-05-05:s1:origin-v2',cached=mem.get(key)||await dbGet(key);if(cached?.bars?.length){mem.set(key,cached);stats.originCacheHits++;return{...cached,cacheHit:true}}
  const all=[];let minMove=NaN;
  for(let h=0;h<24&&all.length<ORIGIN_BARS*4;h++){
    const u=`${DUKA}/ticks/${GOLD_CODE}/2003/5/5/${h}`,r=await fetch(u,{cache:'force-cache',credentials:'omit'});if(!r.ok)continue;const j=await r.json();if(Number(j?.multiplier)>0)minMove=Number.isFinite(minMove)?Math.min(minMove,Number(j.multiplier)):Number(j.multiplier);all.push(...tickRows(j))
  }
...
window.RWARenkoTotalHistory={version:'2.1.0-origin-chunks',rule:'authoritative-fixed-1s-origin-discovery-bounded-indexeddb-origin-windows-no-timeframe-substitution',stats,manifest,discoverBinance,fetchOriginWindow,jumpOrigin,goldOrigin,goLive,gold:{symbol:GOLD_SYMBOL,instrumentCode:GOLD_CODE,documentedEarliestS1Ms:GOLD_DOCUMENTED_EARLIEST_MS},get current(){return T.state.historyMeta||null}};
})();
