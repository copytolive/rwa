...
const clip=(v,max=280)=>String(v??'').trim().slice(0,max);
const safeWallet=v=>String(v||'').toLowerCase();
const safeSymbol=v=>String(v||'').toUpperCase().replace(/[^A-Z0-9._-]/g,'').slice(0,32);
const opaqueToken=()=>{const a=crypto.getRandomValues(new Uint8Array(32));return Array.from(a,x=>x.toString(16).padStart(2,'0')).join('')};
function canonicalSession(p){return `RWA TERMINAL SESSION V1\nWallet: ${safeWallet(p.wallet)}\nIssued At: ${p.issuedAt}\nNonce: ${p.nonce}`}

export class RWAState {
  constructor(ctx,env){
    this.ctx=ctx;this.env=env;this.db={schema:4,users:{},events:[],nonces:{},sessions:{},alerts:{},social:{posts:[],comments:{},reactions:{},follows:{}},rewards:{ledger:{},entries:{}},holdersCache:{},metrics:{since:now(),events:{}},updated_at:0};this.control={...DEFAULT_CONTROL};this.controlAt=0;this.lastError='';this.cursor=0;
    ctx.blockConcurrencyWhile(async()=>{
      this.db=await ctx.storage.get('db')||this.db;
      this.db.schema=4;this.db.users=this.db.users||{};this.db.events=this.db.events||[];this.db.nonces=this.db.nonces||{};this.db.sessions=this.db.sessions||{};this.db.alerts=this.db.alerts||{};this.db.social=this.db.social||{posts:[],comments:{},reactions:{},follows:{}};this.db.social.posts=this.db.social.posts||[];this.db.social.comments=this.db.social.comments||{};this.db.social.reactions=this.db.social.reactions||{};this.db.social.follows=this.db.social.follows||{};this.db.rewards=this.db.rewards||{ledger:{},entries:{}};this.db.rewards.ledger=this.db.rewards.ledger||{};this.db.rewards.entries=this.db.rewards.entries||{};this.db.holdersCache=this.db.holdersCache||{};
  ...
  metrics(){const e=this.db.metrics?.events||{},active=Object.values(this.db.users||{}).filter(x=>x.copy?.enabled);return{since:this.db.metrics?.since||0,active_users:active.length,total_users:Object.keys(this.db.users||{}).length,terminal_sessions:Object.keys(this.db.sessions||{}).length,active_alerts:Object.values(this.db.alerts||{}).flat().filter(x=>!x.triggered).length,social_posts:(this.db.social?.posts||[]).length,reward_wallets:Object.keys(this.db.rewards?.ledger||{}).length,executed:n(e['copy.executed']),replay_confirmed:n(e['copy.replay_confirmed']),retry_pending:n(e['copy.retry_pending']),blocked:n(e['copy.blocked']),errors:n(e['worker.error']),max_loss_stops:n(e['copy.max_loss']),alerts_triggered:n(e['alert.triggered'])}}
  health(){return{ok:true,service:'rwa-agent-worker',runtime:'cloudflare-durable-object-free',version:'4.0.0',single_write_path:WORKER_SINGLE_WRITE_PATH,idempotency:WORKER_IDEMPOTENCY,control_enabled:!!this.control.enabled,kill_switch:!!this.control.kill_switch,production_ready:!!this.control.production_ready,mainnet_allowed:this.mainnetAllowed(),origin_bound:true,users:Object.values(this.db.users||{}).filter(x=>x.copy?.enabled).length,uptime_ms:now()-n(this.db.metrics?.since,now()),updated_at:this.db.updated_at}}
  ...
  cleanupSessions(){const t=now();for(const [k,v] of Object.entries(this.db.sessions||{}))if(!v||n(v.expiresAt)<=t)delete this.db.sessions[k]}
  auth(request){this.cleanupSessions();const token=String(request.headers.get('authorization')||'').replace(/^Bearer\s+/i,'').trim(),row=this.db.sessions?.[token];if(!token||!row||n(row.expiresAt)<=now())throw Error('terminal_session_required');return safeWallet(row.wallet)}
  async createTerminalSession(payload){
    const p={wallet:safeWallet(payload.wallet),issuedAt:String(payload.issuedAt||''),nonce:clip(payload.nonce,160),message:String(payload.message||''),signature:String(payload.signature||'')};
    if(!ADDRESS.test(p.wallet))throw Error('invalid_wallet');const expected=canonicalSession(p);if(p.message!==expected||!await validFreshSignature({master:p.wallet,message:p.message,signature:p.signature,issuedAt:p.issuedAt}))throw Error('terminal_session_signature_invalid');
    this.consumeNonce(p.wallet,p.nonce);const token=opaqueToken(),expiresAt=now()+7*24*60*60_000;this.db.sessions[token]={wallet:p.wallet,createdAt:now(),expiresAt};this.event('terminal.session',{wallet:p.wallet});await this.save();return{ok:true,token,expiresAt,wallet:p.wallet}
  }
  rewardProgram(){try{return{active:false,rates:{},...JSON.parse(String(this.env.RWA_REWARD_PROGRAM||'{}'))}}catch{return{active:false,rates:{}}}}
  creditReward(wallet,reason,ref){
    const program=this.rewardProgram(),rate=Math.max(0,n(program.rates?.[reason]));if(!program.active||!(rate>0))return;
    const id=`${reason}:${ref}`;if(this.db.rewards.entries[id])return;this.db.rewards.entries[id]={wallet,reason,points:rate,ts:now()};this.db.rewards.ledger[wallet]=n(this.db.rewards.ledger[wallet])+rate;this.event('reward.credit',{wallet,reason,points:rate,ref})
  }
  alertsFor(wallet){return (this.db.alerts[wallet]||[]).slice(-100)}
  async createAlert(wallet,payload){
    const symbol=safeSymbol(payload.symbol),price=n(payload.price),direction=String(payload.direction||'above').toLowerCase()==='below'?'below':'above';if(!symbol||!(price>0))throw Error('invalid_alert');
    const list=this.db.alerts[wallet]||[],id=crypto.randomUUID();list.push({id,wallet,symbol,price,direction,createdAt:now(),triggered:false,triggeredAt:0,triggerPrice:0});this.db.alerts[wallet]=list.slice(-100);this.event('alert.created',{wallet,symbol,price,direction,id});await this.save();return{ok:true,alert:list.at(-1)}
  }
  async deleteAlert(wallet,id){const list=this.db.alerts[wallet]||[],next=list.filter(x=>x.id!==String(id));if(next.length===list.length)throw Error('alert_not_found');this.db.alerts[wallet]=next;this.event('alert.deleted',{wallet,id:String(id)});await this.save();return{ok:true}}
  socialFeed(limit=50){const posts=(this.db.social.posts||[]).slice(-Math.max(1,Math.min(100,n(limit,50)))).reverse();return posts.map(p=>({...p,comments:(this.db.social.comments[p.id]||[]).length,reactions:Object.values(this.db.social.reactions[p.id]||{}).reduce((a,x)=>(a[x]=(a[x]||0)+1,a),{})}))}
  async socialPost(wallet,payload){
    const symbol=safeSymbol(payload.symbol),side=String(payload.side||'LONG').toUpperCase()==='SHORT'?'SHORT':'LONG',body=clip(payload.text,280);if(!symbol||body.length<2)throw Error('invalid_post');
    const post={id:crypto.randomUUID(),wallet,symbol,side,text:body,createdAt:now()};this.db.social.posts.push(post);this.db.social.posts=this.db.social.posts.slice(-1000);this.creditReward(wallet,'post',post.id);this.event('social.post',{wallet,id:post.id,symbol,side});await this.save();return{ok:true,post}
  }
  async socialDelete(wallet,id){const i=this.db.social.posts.findIndex(x=>x.id===String(id));if(i<0)throw Error('post_not_found');if(this.db.social.posts[i].wallet!==wallet)throw Error('forbidden');this.db.social.posts.splice(i,1);delete this.db.social.comments[id];delete this.db.social.reactions[id];this.event('social.delete',{wallet,id:String(id)});await this.save();return{ok:true}}
  async socialComment(wallet,payload){const id=String(payload.postId||''),text=clip(payload.text,240);if(!this.db.social.posts.some(x=>x.id===id))throw Error('post_not_found');if(text.length<1)throw Error('invalid_comment');const row={id:crypto.randomUUID(),wallet,text,createdAt:now()},list=this.db.social.comments[id]||[];list.push(row);this.db.social.comments[id]=list.slice(-500);this.creditReward(wallet,'comment',row.id);this.event('social.comment',{wallet,postId:id,id:row.id});await this.save();return{ok:true,comment:row}}
  async socialReact(wallet,payload){const id=String(payload.postId||''),reaction=clip(payload.reaction||'fire',24).toLowerCase();if(!this.db.social.posts.some(x=>x.id===id))throw Error('post_not_found');const map=this.db.social.reactions[id]||{};if(map[wallet]===reaction)delete map[wallet];else map[wallet]=reaction;this.db.social.reactions[id]=map;this.event('social.react',{wallet,postId:id,reaction:map[wallet]||'removed'});await this.save();return{ok:true,reactions:Object.values(map).reduce((a,x)=>(a[x]=(a[x]||0)+1,a),{})}}
  async socialFollow(wallet,payload){const target=safeWallet(payload.wallet);if(!ADDRESS.test(target)||target===wallet)throw Error('invalid_follow_target');const list=new Set(this.db.social.follows[wallet]||[]);if(list.has(target))list.delete(target);else list.add(target);this.db.social.follows[wallet]=[...list].slice(-500);this.event('social.follow',{wallet,target,following:list.has(target)});await this.save();return{ok:true,following:list.has(target),wallet:target}}
  rewardsSummary(wallet){const program=this.rewardProgram(),entries=Object.entries(this.db.rewards.entries||{}).filter(([,x])=>x.wallet===wallet).map(([id,x])=>({id,...x})).slice(-100).reverse();return{ok:true,active:!!program.active,program:clip(program.name||'RWA Rewards',80),points:n(this.db.rewards.ledger?.[wallet]),entries,claim:{enabled:false,reason:'onchain_claim_not_configured'}}}
  async holders(symbol){
    const key=safeSymbol(symbol),cache=this.db.holdersCache[key];if(cache&&now()-n(cache.ts)<60_000)return cache.data;
    let sources={};try{sources=JSON.parse(String(this.env.RWA_HOLDER_SOURCES||'{}'))}catch{}const url=String(sources[key]||'');if(!/^https:\/\//i.test(url))throw Error('holder_source_not_configured');
    const r=await fetch(url,{headers:{accept:'application/json'}});if(!r.ok)throw Error(`holder_source_http_${r.status}`);const raw=await r.json(),holders=Array.isArray(raw?.holders)?raw.holders:Array.isArray(raw)?raw:[];const data={ok:true,symbol:key,source:url,holders:holders.slice(0,200),updatedAt:now()};this.db.holdersCache[key]={ts:now(),data};await this.save();return data
  }
  async processAlerts(){
    const pending=Object.values(this.db.alerts||{}).flat().filter(x=>!x.triggered);if(!pending.length)return;
    const symbols=[...new Set(pending.map(x=>x.symbol))],prices={};
    try{const r=await fetch('https://data-api.binance.vision/api/v3/ticker/price');if(r.ok){for(const x of await r.json())if(symbols.includes(String(x.symbol)))prices[x.symbol]=n(x.price)}}catch{}
    const missing=symbols.filter(x=>!(prices[x]>0));if(missing.length)try{const r=await fetch('https://api.hyperliquid.xyz/info',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({type:'allMids'})});if(r.ok){const mids=await r.json();for(const symbol of missing){const coin=symbol.replace(/USDT$/,'');if(n(mids?.[coin])>0)prices[symbol]=n(mids[coin])}}}catch{}
    let changed=false;for(const a of pending){const px=n(prices[a.symbol]);if(!(px>0))continue;const hit=a.direction==='below'?px<=n(a.price):px>=n(a.price);if(!hit)continue;a.triggered=true;a.triggeredAt=now();a.triggerPrice=px;changed=true;this.creditReward(a.wallet,'alert_trigger',a.id);this.event('alert.triggered',{wallet:a.wallet,id:a.id,symbol:a.symbol,price:a.price,triggerPrice:px,direction:a.direction})}
  ...
  async tick(){await this.loadControl();try{await this.processAlerts()}catch(e){this.event('alert.worker_error',{error:String(e?.message||e)});await this.setLastError(e)}if(!this.control.enabled||this.control.kill_switch){await this.save();return}const active=Object.values(this.db.users||{}).filter(x=>x.copy?.enabled);if(!active.length){await this.save();return}const batch=[];for(let i=0;i<Math.min(MAX_USERS_PER_TICK,active.length);i++)batch.push(active[(this.cursor+i)%active.length]);this.cursor=(this.cursor+batch.length)%active.length;for(const rec of batch){try{await this.processUser(rec)}catch(e){this.event('worker.error',{master:rec.master,error:String(e?.message||e)});await this.setLastError(e)}}await this.save()}
  ...
  async fetch(request){
    ...
    const u=new URL(request.url);try{
      ...
      if(request.method==='GET'&&u.pathname==='/healthz')return json(this.health(),200,headers);
      if(request.method==='GET'&&u.pathname==='/terminal/readyz')return json({ok:true,service:'rwa-terminal-service',version:'1.0.0',sessions:true,alerts:true,social:true,rewards:true,holders:'source-gated',alarm_ms:LOOP_MS},200,headers);
      if(request.method==='GET'&&u.pathname==='/readyz'){const r=this.ready();return json(r,r.ok?200:503,headers)}
      if(request.method==='GET'&&u.pathname==='/status')return json(this.redacted(),200,headers);
      if(request.method==='GET'&&u.pathname==='/metrics')return json(this.metrics(),200,headers);
      if(request.method==='GET'&&u.pathname==='/v1/social/feed')return json({ok:true,posts:this.socialFeed(u.searchParams.get('limit'))},200,headers);
      if(request.method==='GET'&&u.pathname==='/v1/holders')return json(await this.holders(u.searchParams.get('symbol')),200,headers);
      if(request.method==='POST'){
        if(!this.originAllowed(request))return json({error:'origin not allowed'},403,headers);const payload=await request.json();
        if(u.pathname==='/v1/session')return json(await this.createTerminalSession(payload),200,headers);
        if(u.pathname==='/v1/register')return json(await this.register(payload,request),200,headers);
        if(u.pathname==='/v1/stop')return json(await this.stop(payload,request),200,headers);
        const wallet=this.auth(request);
        if(u.pathname==='/v1/alerts/list')return json({ok:true,alerts:this.alertsFor(wallet)},200,headers);
        if(u.pathname==='/v1/alerts/create')return json(await this.createAlert(wallet,payload),200,headers);
        if(u.pathname==='/v1/alerts/delete')return json(await this.deleteAlert(wallet,payload.id),200,headers);
        if(u.pathname==='/v1/social/post')return json(await this.socialPost(wallet,payload),200,headers);
        if(u.pathname==='/v1/social/delete')return json(await this.socialDelete(wallet,payload.id),200,headers);
        if(u.pathname==='/v1/social/comment')return json(await this.socialComment(wallet,payload),200,headers);
        if(u.pathname==='/v1/social/react')return json(await this.socialReact(wallet,payload),200,headers);
        if(u.pathname==='/v1/social/follow')return json(await this.socialFollow(wallet,payload),200,headers);
        if(u.pathname==='/v1/rewards/summary')return json(this.rewardsSummary(wallet),200,headers);
      }
      return json({error:'not found'},404,headers)
    }catch(e){await this.setLastError(e);const msg=String(e?.message||e),status=/session_required|signature_invalid|invalid_wallet/.test(msg)?401:/not_configured/.test(msg)?503:400;return json({error:msg},status,headers)}
  }
}
